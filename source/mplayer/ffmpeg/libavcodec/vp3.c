...
int vp3_decode_end(AVCodecContext* avctx)
{
	Vp3DecodeContext* s = avctx->priv_data;
	int i;
	...
	for (inter = 0; inter < 2; inter++)
	{
		for (plane = 0; plane < 3; plane++)
		{
			...
			for (i = 0; i < 64; i++)
			{
				...
				s->qmat[qpi][inter][plane][s->dsp.idct_permutation[i]] =
...
#define PL 1
				...
				static void vp3_draw_horiz_band(Vp3DecodeContext* s, int y)
				{
					...
					)
					{
						...
						ff_thread_report_progress(&s->current_frame,
						                          y_flipped == s->avctx->height ? INT_MAX : y_flipped - 1, 0);
					}
							...
							for (sb_x = 0; sb_x < slice_width; sb_x++)
							{
								...
								for (j = 0; j < 16; j++)
								{
									...
									if (do_await && s->all_fragments[i].coding_method != MODE_INTRA)
										await_reference_row(s, &s->all_fragments[i], motion_val[fragment][1],
									...
									if (s->all_fragments[i].coding_method != MODE_COPY)
									{
											...
											(s->all_fragments[i].coding_method != MODE_USING_GOLDEN))
										{
											...
											if (src_x < 0 || src_y < 0 || src_x + 9 >= plane_width || src_y + 9 >=
												plane_height)
											{
												uint8_t* temp = s->edge_emu_buffer;
												if (stride < 0) temp -= 8 * stride;

												s->dsp.emulated_edge_mc(temp, motion_source, stride, 9, 9, src_x, src_y,
				...
				int allocate_tables(AVCodecContext* avctx)
				{
					Vp3DecodeContext* s = avctx->priv_data;
				...
				int vp3_decode_init(AVCodecContext* avctx)
				{
					Vp3DecodeContext* s = avctx->priv_data;
					int i, inter, plane;
					int c_width;
