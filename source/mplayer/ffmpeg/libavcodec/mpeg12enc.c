...
void put_qscale(MpegEncContext* s)
{
	if (s->q_scale_type)
	{
...
void mpeg1_encode_mb_internal(MpegEncContext* s,
                              DCTELEM block[6][64],
                              ...
                              int mb_block_count)
{
	...
	if (cbp == 0 && !first_mb && s->mv_type == MV_TYPE_16X16 &&
		...
		((s->pict_type == AV_PICTURE_TYPE_P && (motion_x | motion_y) == 0) ||
			(s->pict_type == AV_PICTURE_TYPE_B && s->mv_dir == s->last_mv_dir && (((s->mv_dir & MV_DIR_FORWARD)
					? ((s->mv[0][0][0] - s->last_mv[0][0][0]) | (s->mv[0][0][1] - s->last_mv[0][0][1]))
					: 0) |
				((s->mv_dir & MV_DIR_BACKWARD)
					 ? ((s->mv[1][0][0] - s->last_mv[1][0][0]) | (s->mv[1][0][1] - s->last_mv[1][0][1]))
					 : 0)) == 0)))
	{
		s->mb_skip_run++;
		s->qscale -= s->dquant;
