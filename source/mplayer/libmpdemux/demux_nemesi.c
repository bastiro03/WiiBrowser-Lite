...
static unsigned int get4CC(MIMEto4CC* supported_formats, const char* format)
{
	unsigned i;
	...
	for (i = 0; i < NMS_MAX_FORMATS; ++i)
	{
		if (!supported_formats[i].mime)
			return 0;
		if (strcmp(supported_formats[i].mime, format) == 0)
...
static demuxer_t* demux_open_rtp(demuxer_t* demuxer)
{
	...
	for (; media; media = media->next)
	{
		...
		const char* format_name = ptinfo ? ptinfo->name : NULL;
