                          ...
                          struct variant* var, AVIOContext* in)
{
	...
	while (!url_feof(in))
	{
		...
		if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr))
		{
			...
			is_variant = 1;
			ff_parse_key_value(ptr, handle_variant_args,
			                   &info);
		...
		else if (av_strstart(line, "#EXT-X-KEY:", &ptr))
		{
			...
			ff_parse_key_value(ptr, handle_key_args,
			                   &info);
			key_type = KEY_NONE;
			has_iv = 0;
...
restart:
	if (!v->input)
	{
		...
		int64_t reload_interval = v->n_segments > 0 ? v->segments[v->n_segments - 1]->duration : v->target_duration;
                         ...
                         int64_t timestamp, int flags)
{
	...
	c->seek_flags = flags;
	c->seek_timestamp = stream_index < 0
		                    ? timestamp
		                    : av_rescale_rnd(timestamp, AV_TIME_BASE,
		                                     s->streams[stream_index]->time_base.den,
		                                     flags & AVSEEK_FLAG_BACKWARD ? AV_ROUND_DOWN : AV_ROUND_UP);
	timestamp = av_rescale_rnd(timestamp, 1, stream_index >= 0 ? s->streams[stream_index]->time_base.den : AV_TIME_BASE,
	                           flags & AVSEEK_FLAG_BACKWARD ? AV_ROUND_DOWN : AV_ROUND_UP);
