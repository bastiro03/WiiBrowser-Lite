...
void DisplayMediaType(const char* label, const AM_MEDIA_TYPE* pmt)
{
	...
	Debug
		mp_msg(MSGT_LOADER, MSGL_DBG4, "Subtype:     %08x-%04x-%04x-%02x%02x-"
	...
	Debug
		mp_msg(MSGT_LOADER, MSGL_DBG4, "Format type: %08x-%04x-%04x-%02x%02x-"
		       "%02x%02x%02x%02x%02x%02x\n",
		       iid->f1, iid->f2, iid->f3,
		       (unsigned char)iid->f4[1], (unsigned char)iid->f4[0],
		       (unsigned char)iid->f4[2], (unsigned char)iid->f4[3],
	...
	if (pmt && memcmp(&pmt->formattype, &FORMAT_VideoInfo, 16) == 0 && pmt->pbFormat)
	{
		...
		{
			Debug mp_msg(MSGT_LOADER, MSGL_DBG4, "Vhdr: biComression 0x%08lx (%s)\n", Vhdr->bmiHeader.biCompression,
			             vo_format_name(Vhdr->bmiHeader.biCompression));
		}
		else
...
{
	return ((bWildcards && (IsEqualGUID(&pmt1->majortype, &GUID_NULL) || IsEqualGUID(&pmt2->majortype, &GUID_NULL))) ||
			IsEqualGUID(&pmt1->majortype, &pmt2->majortype)) &&
		((bWildcards && (IsEqualGUID(&pmt1->subtype, &GUID_NULL) || IsEqualGUID(&pmt2->subtype, &GUID_NULL))) ||
