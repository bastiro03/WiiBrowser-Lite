 ...
 */
static inline void restore_ac_coeffs(MpegEncContext* s, DCTELEM block[6][64], const int dir[6], uint8_t* st[6],
 ...
 */
static inline int decide_ac_pred(MpegEncContext* s, DCTELEM block[6][64], const int dir[6], uint8_t* st[6],
	...
	for (n = 0; n < 6; n++)
	{
		...
		if (dir[n])
		{
			...
			else
			{
				...
				{
					const int level = block[n][s->dsp.idct_permutation[i]];
					block[n][s->dsp.idct_permutation[i]] = level - ROUNDED_DIV(
						ac_val[i + 8] * qscale_table[xy], s->qscale);
					ac_val1[i] = block[n][s->dsp.idct_permutation[i << 3]];
					ac_val1[i + 8] = level;
				}
			}
		...
		else
		{
			...
			else
			{
				...
				{
					const int level = block[n][s->dsp.idct_permutation[i << 3]];
					block[n][s->dsp.idct_permutation[i << 3]] = level - ROUNDED_DIV(
						ac_val[i] * qscale_table[xy], s->qscale);
					ac_val1[i] = level;
					ac_val1[i + 8] = block[n][s->dsp.idct_permutation[i]];
				}
			}
...
void ff_clean_mpeg4_qscales(MpegEncContext* s)
{
	...
	if (s->pict_type == AV_PICTURE_TYPE_B)
	{
		...
		{
			int mb_xy = s->mb_index2xy[i];
			if (qscale_table[mb_xy] != qscale_table[s->mb_index2xy[i - 1]] && (s->mb_type[mb_xy] &
                                      ...
                                      uint8_t* scan_table, PutBitContext* dc_pb, PutBitContext* ac_pb)
{
		...
		if (level)
		{
			...
			else
			{
				...
				put_bits(ac_pb, 7 + 2 + 1 + 6 + 1 + 12 + 1,
				         (3 << 23) + (3 << 21) + (0 << 20) + (run << 14) + (1 << 13) + (((level - 64) & 0xfff) << 1) +
				         1);
			}
			last_non_zero = i;
		}
	...
	/*if(i<=last_index)*/
	{
		...
		else
		{
			//ESC3
			put_bits(ac_pb, 7 + 2 + 1 + 6 + 1 + 12 + 1,
			         (3 << 23) + (3 << 21) + (1 << 20) + (run << 14) + (1 << 13) + (((level - 64) & 0xfff) << 1) + 1);
		}
	}
}
                        ...
                        int motion_x, int motion_y)
{
	...
	if (!s->mb_intra)
	{
		...
		if (s->pict_type == AV_PICTURE_TYPE_B)
		{
			...
			if (s->next_picture.f.mbskip_table[s->mb_y * s->mb_stride + s->mb_x])
			{
				//FIXME avoid DCT & ...
				s->skip_count++;
				s->mv[0][0][0] =
			...
			else
			{
				...
				for (i = 0; i < 4; i++)
				{
					...
					ff_h263_encode_motion_vector(s, s->current_picture.f.motion_val[0][s->block_index[i]][0] - pred_x,
					                             s->current_picture.f.motion_val[0][s->block_index[i]][1] - pred_y,
					                             s->f_code);
				}
			}
