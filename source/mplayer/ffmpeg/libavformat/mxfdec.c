...
{
	static const uint8_t checkv[16] = {
		0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b, 0x43, 0x48, 0x55, 0x4b
	};
...
static int mxf_read_index_entry_array(AVIOContext* pb, MXFIndexTableSegment* segment)
{
	...
	if (!(segment->temporal_offset_entries = av_calloc(segment->nb_index_entries,
...
static const MXFCodecUL mxf_picture_essence_container_uls[] = {
	...
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x02, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x04, 0x60, 0x01}, 14,
		CODEC_ID_MPEG2VIDEO
	}, /* MPEG-ES Frame wrapped */
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x02, 0x41, 0x01}, 14,
		CODEC_ID_DVVIDEO
	}, /* DV 625 25mbps */
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x05, 0x00, 0x00}, 14,
		CODEC_ID_RAWVIDEO
...
static const MXFCodecUL mxf_intra_only_essence_container_uls[] = {
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x01, 0x00, 0x00}, 14,
		CODEC_ID_MPEG2VIDEO
...
static const MXFCodecUL mxf_intra_only_picture_essence_coding_uls[] = {
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x0A, 0x04, 0x01, 0x02, 0x02, 0x01, 0x32, 0x00, 0x00}, 14,
		CODEC_ID_H264
...
static const MXFCodecUL mxf_sound_essence_container_uls[] = {
	...
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x06, 0x01, 0x00}, 14,
		CODEC_ID_PCM_S16LE
	}, /* BWF Frame wrapped */
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x02, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x04, 0x40, 0x01}, 14,
		CODEC_ID_MP2
	}, /* MPEG-ES Frame wrapped, 0x40 ??? stream id */
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0D, 0x01, 0x03, 0x01, 0x02, 0x01, 0x01, 0x01}, 14,
		CODEC_ID_PCM_S16LE
	}, /* D-10 Mapping 50Mbps PAL Extended Template */
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x01, 0x01, 0x01, 0xFF, 0x4B, 0x46, 0x41, 0x41, 0x00, 0x0D, 0x4D, 0x4F}, 14,
		CODEC_ID_PCM_S16LE
...
static int mxf_get_sorted_table_segments(MXFContext* mxf, int* nb_sorted_segments,
	...
	*nb_sorted_segments = 0;
	...
	for (i = 0; i < nb_segments; i++)
	{
		...
		for (j = 0; j < nb_segments; j++)
		{
			 ...
			 */
			if ((i == 0 || s->body_sid > last_body_sid || s->index_sid > last_index_sid || s->index_start_position >
					last_index_start) &&
				(best == -1 || s->body_sid < best_body_sid || s->index_sid < best_index_sid || s->index_start_position <
					best_index_start ||
...
static int mxf_edit_unit_absolute_offset(MXFContext* mxf, MXFIndexTable* index_table, int64_t edit_unit,
                                         int64_t* edit_unit_out, int64_t* offset_out, int nag)
{
	int i;
	...
	for (i = 0; i < index_table->nb_segments; i++)
	{
		...
		if (edit_unit < s->index_start_position + s->index_duration)
		{
			...
			{
				av_log(mxf->fc, AV_LOG_ERROR,
				       "IndexSID %i segment at %"PRId64" missing EditUnitByteCount and IndexEntryArray\n",
				       index_table->index_sid, s->index_start_position);
				return AVERROR_INVALIDDATA;
			}
	...
	for (i = j = 0; j < mxf->nb_index_tables; i += mxf->index_tables[j++].nb_segments)
	{
		...
		if (sorted_segments[i]->index_start_position)
			av_log(mxf->fc, AV_LOG_WARNING,
			       "IndexSID %i starts at EditUnit %"PRId64" - seeking may not work as expected\n",
	...
	for (i = 0; i < material_package->tracks_count; i++)
	{
		...
		if (source_package->descriptor)
		{
			if (source_package->descriptor->type == MultipleDescriptor)
			{
				...
				{
					MXFDescriptor* sub_descriptor = mxf_resolve_strong_ref(
						mxf, &source_package->descriptor->sub_descriptors_refs[j], Descriptor);
...
static const MXFMetadataReadTableEntry mxf_metadata_read_table[] = {
	...
	},
	{
		...
		mxf_read_sequence, sizeof(MXFSequence), Sequence
	},
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01, 0x01, 0x01, 0x11, 0x00},
		mxf_read_source_clip, sizeof(MXFStructuralComponent), SourceClip
	},
	{
		{0x06, 0x0E, 0x2B, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01, 0x01, 0x01, 0x44, 0x00},
		mxf_read_generic_descriptor, sizeof(MXFDescriptor), MultipleDescriptor
	},
	{
	...
	}, /* AES3 */
	{
		...
		mxf_read_track, sizeof(MXFTrack), Track
	}, /* Static Track */
	{
	...
	},
	{
		...
		mxf_read_index_table_segment, sizeof(MXFIndexTableSegment), IndexTableSegment
	},
	{
		{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, NULL, 0,
		AnyType
	},
};

static int mxf_read_local_tags(MXFContext* mxf, KLVPacket* klv, MXFMetadataReadFunc* read_child, int ctx_size,
                               enum MXFMetadataSetType type)
{
	...
	while (avio_tell(pb) + 4 < klv_end && !url_feof(pb))
	{
		...
		if (avio_tell(pb) <= next) /* only seek forward, else this can loop for a long time */
...
static int mxf_read_header(AVFormatContext* s)
{
	...
	while (!url_feof(s->pb))
	{
		...
		if (!memcmp(klv.key, mxf_header_partition_pack_key, 13) &&
