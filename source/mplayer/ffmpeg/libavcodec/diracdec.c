...
int dirac_decode_init(AVCodecContext* avctx)
{
...
int dirac_decode_end(AVCodecContext* avctx)
{
...
static int dirac_unpack_prediction_parameters(DiracContext* s)
{
	...
	if (!s->plane[0].xbsep || !s->plane[0].ybsep || s->plane[0].xbsep < s->plane[0].xblen / 2 || s->plane[0].ybsep < s->
		plane[0].yblen / 2)
	{
...
static void interpolate_refplane(DiracContext* s, DiracFrame* ref, int plane, int width, int height)
{
	...
	s->dsp.draw_edges(ref->hpel[plane][0], ref->avframe.linesize[plane], width, height, edge, edge,
	...
	if (!ref->interpolated[plane])
	{
		...
		s->dsp.draw_edges(ref->hpel[plane][1], ref->avframe.linesize[plane], width, height, edge, edge,
		                  EDGE_TOP | EDGE_BOTTOM);
		s->dsp.draw_edges(ref->hpel[plane][2], ref->avframe.linesize[plane], width, height, edge, edge,
		                  EDGE_TOP | EDGE_BOTTOM);
		s->dsp.draw_edges(ref->hpel[plane][3], ref->avframe.linesize[plane], width, height, edge, edge,
		                  EDGE_TOP | EDGE_BOTTOM);
	}
...
static int dirac_decode_picture_header(DiracContext* s)
{
	...
	if (s->current_picture->avframe.reference)
	{
		...
		while (add_frame(s->ref_frames, MAX_REFERENCE_FRAMES, s->current_picture))
		{
			...
			remove_frame(s->ref_frames, s->ref_frames[0]->avframe.display_picture_number)->avframe.reference &=
				DELAYED_PIC_REF;
		}
	}
...
static int dirac_decode_frame(AVCodecContext* avctx, void* data, int* data_size, AVPacket* pkt)
{
	...
	*data_size = 0;
	...
	if (buf_size == 0)
		return get_delayed_pic(s, data, data_size);
