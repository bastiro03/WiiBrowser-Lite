...
int tta_decode_init(AVCodecContext* avctx)
{
	...
	if (show_bits_long(&s->gb, 32) == AV_RL32("TTA1"))
	{
		...
		switch (s->bps)
		{
		case 1: avctx->sample_fmt = AV_SAMPLE_FMT_U8;
			break;
		case 2:
		...
		{
			if (avctx->extradata_size < 26 + s->total_frames * 4 || tta_check_crc(
				s, avctx->extradata + 22, s->total_frames * 4))
				return AVERROR_INVALIDDATA;
		}
...
int tta_decode_close(AVCodecContext* avctx)
{
	TTAContext* s = avctx->priv_data;
