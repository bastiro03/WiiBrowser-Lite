...
int mpeg_decode_init(AVCodecContext* avctx)
{
	...
	if (s->mb_y == 0 && s->mb_x == 0 && (s->first_field || s->picture_structure == PICT_FRAME))
	{
		if (s->avctx->debug & FF_DEBUG_PICT_INFO)
		{
			av_log(s->avctx, AV_LOG_DEBUG,
			       ...
			       s->progressive_sequence ? "ps" : "", s->progressive_frame ? "pf" : "",
			       s->alternate_scan ? "alt" : "", s->top_field_first ? "top" : "",
	...
	for (;;)
	{
		...
		if (++s->mb_x >= s->mb_width)
		{
			...
			if (s->mb_y >= s->mb_height)
			{
				...
				int is_d10 = s->chroma_format == 2 && s->pict_type == AV_PICTURE_TYPE_I && avctx->profile == 0 && avctx
					->level == 5
		...
		if (start_code > 0x1ff)
		{
			if (s2->pict_type != AV_PICTURE_TYPE_B || avctx->skip_frame <= AVDISCARD_DEFAULT)
			{
				if (HAVE_THREADS && (avctx->active_thread_type & FF_THREAD_SLICE))
				{
					...
					avctx->execute(avctx, slice_decode_thread, &s2->thread_context[0], NULL, s->slice_count,
					               sizeof(void*));
				...
				else
				{
					...
					if (ret < 0)
					{
						...
						if (s2->resync_mb_x >= 0 && s2->resync_mb_y >= 0)
							ff_er_add_slice(s2, s2->resync_mb_x, s2->resync_mb_y, s2->mb_x, s2->mb_y,
							                ER_AC_ERROR | ER_DC_ERROR | ER_MV_ERROR);
					}
					else
					{
						ff_er_add_slice(s2, s2->resync_mb_x, s2->resync_mb_y, s2->mb_x - 1, s2->mb_y,
