...
static void init_layer3(int down_sample_sblimit)
{
	...
	{
		if (_has_mmx)
			gainpow2[i + 256] = 16384.0 * pow(2.0, -0.25 * (double)(i + 210));
		else
	...
	for (i = 0; i < 18; i++)
	{
		win[0][i] = win[1][i] = 0.5 * sin(M_PI / 72.0 * (double)(2 * (i + 0) + 1)) / cos(
			M_PI * (double)(2 * (i + 0) + 19) / 72.0);
		win[0][i + 18] = win[3][i + 18] = 0.5 * sin(M_PI / 72.0 * (double)(2 * (i + 18) + 1)) / cos(
			M_PI * (double)(2 * (i + 18) + 19) / 72.0);
	}
	for (i = 0; i < 6; i++)
	{
