...
static void print_help(void)
{
	...
	i_param_size = GetGlobalAudioPropertyArray(kAudioObjectSystemObject, kAudioHardwarePropertyDevices,
	                                           (void**)&devids);
...
static int init(int rate, int channels, int format, int flags)
{
	...
	ao->packetSize = inDesc.mBytesPerPacket = inDesc.mBytesPerFrame = inDesc.mFramesPerPacket * channels * (inDesc.
		mBitsPerChannel / 8);
	...
	err = AudioUnitSetProperty(ao->theOutputUnit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, 0, &inDesc,
	                           size);
	...
	size = sizeof(UInt32);
	err = AudioUnitGetProperty(ao->theOutputUnit, kAudioDevicePropertyBufferSize, kAudioUnitScope_Input, 0, &maxFrames,
	                           &size);
	...
	err = AudioUnitSetProperty(ao->theOutputUnit, kAudioOutputUnitProperty_CurrentDevice, kAudioUnitScope_Global, 0,
	                           &ao->i_selected_dev, sizeof(ao->i_selected_dev));
	...
	ao_msg(MSGT_AO, MSGL_V, "using %5d chunks of %d bytes (buffer len %d bytes)\n", (int)ao->num_chunks,
	       (int)ao->chunk_size, (int)ao->buffer_len);

	renderCallback.inputProc = theRenderProc;
	renderCallback.inputProcRefCon = 0;
	err = AudioUnitSetProperty(ao->theOutputUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Input, 0,
		...
		if (b_digital)
		{
			...
			for (j = 0; j < i_formats; ++j)
					...
					p_format_list[j].mFormat.mFormatID == kAudioFormatAC3)
				{
					...
					else if (i_backup_rate_format < 0 || p_format_list[j].mFormat.mSampleRate > p_format_list[
						i_backup_rate_format].mFormat.mSampleRate)
			...
			else ao->stream_format = p_format_list[i_backup_rate_format].mFormat;
