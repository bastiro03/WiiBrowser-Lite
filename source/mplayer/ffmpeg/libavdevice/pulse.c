...
int pulse_read_header(AVFormatContext* s)
{
	...
	const pa_sample_spec ss = {
		codec_id_to_pulse_format(codec_id),
		pd->sample_rate,
		pd->channels
	};
...
int pulse_close(AVFormatContext* s)
{
	PulseData* pd = s->priv_data;
	pa_simple_free(pd->s);
	return 0;
...
static const AVOption options[] = {
	...
	{
		"fragment_size", "buffering size, affects latency and cpu usage", OFFSET(fragment_size), AV_OPT_TYPE_INT,
		{.dbl = -1}, -1, INT_MAX, D
	},
	{NULL},
};
