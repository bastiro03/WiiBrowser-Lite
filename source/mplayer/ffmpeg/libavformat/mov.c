	...
	for (i = 0; i < len; i++)
	{
		...
		if (c < 0x80 && p < end)
			*p++ = c;
			...
			PUT_UTF8(mac_to_unicode[c - 0x80], t, 
		if (p < end) *p++ = t;
		)
	}
	*p = 0;
...
static int mov_read_udta_string(MOVContext* c, AVIOContext* pb, MOVAtom atom)
{
	...
	switch (atom.type)
	{
	case MKTAG(0xa9, 'n', 'a', 'm'): key = "title";
		break;
	...
	case MKTAG('a', 'A', 'R', 'T'): key = "album_artist";
		break;
	case MKTAG(0xa9, 'w', 'r', 't'): key = "composer";
		break;
	case MKTAG('c', 'p', 'r', 't'):
	case MKTAG(0xa9, 'c', 'p', 'y'): key = "copyright";
		break;
	case MKTAG(0xa9, 'g', 'r', 'p'): key = "grouping";
		break;
	case MKTAG(0xa9, 'l', 'y', 'r'): key = "lyrics";
		break;
	case MKTAG(0xa9, 'c', 'm', 't'):
	case MKTAG(0xa9, 'i', 'n', 'f'): key = "comment";
		break;
	case MKTAG(0xa9, 'a', 'l', 'b'): key = "album";
		break;
	case MKTAG(0xa9, 'd', 'a', 'y'): key = "date";
		break;
	case MKTAG(0xa9, 'g', 'e', 'n'): key = "genre";
		break;
	case MKTAG('g', 'n', 'r', 'e'): key = "genre";
		parse = mov_metadata_gnre;
		break;
	case MKTAG(0xa9, 't', 'o', 'o'):
	case MKTAG(0xa9, 's', 'w', 'r'): key = "encoder";
		break;
	case MKTAG(0xa9, 'e', 'n', 'c'): key = "encoder";
		break;
	case MKTAG('d', 'e', 's', 'c'): key = "description";
		break;
	case MKTAG('l', 'd', 'e', 's'): key = "synopsis";
		break;
	case MKTAG('t', 'v', 's', 'h'): key = "show";
		break;
	case MKTAG('t', 'v', 'e', 'n'): key = "episode_id";
		break;
...
enum CodecID ff_mov_get_lpcm_codec_id(int bps, int flags)
{
	if (flags & 1)
	{
		...
		if (flags & 2)
		{
			...
			if (bps == 64) return CODEC_ID_PCM_F64BE;
		}
		else
		{
			if (bps == 32) return CODEC_ID_PCM_F32LE;
			if (bps == 64) return CODEC_ID_PCM_F64LE;
		}
	}
	else
	{
		if (flags & 2)
		{
			...
			if (bps == 16) return CODEC_ID_PCM_S16BE;
			if (bps == 24) return CODEC_ID_PCM_S24BE;
			if (bps == 32) return CODEC_ID_PCM_S32BE;
		}
		else
		{
			if (bps == 8)
			...
			if (bps == 24) return CODEC_ID_PCM_S24LE;
...
int ff_mov_read_stsd_entries(MOVContext* c, AVIOContext* pb, int entries)
{
	...
	for (pseudo_stream_id = 0; pseudo_stream_id < entries; pseudo_stream_id++)
	{
		...
		if (st->codec->codec_tag &&
			...
			(c->fc->video_codec_id
				 ? ff_codec_get_id(ff_codec_movvideo_tags, format) != c->fc->video_codec_id
		...
		else if (st->codec->codec_type == AVMEDIA_TYPE_AUDIO)
		{
			...
			case CODEC_ID_PCM_S16BE:
				if (st->codec->bits_per_coded_sample == 8)
					...
					st->codec->codec_id =
						st->codec->codec_id == CODEC_ID_PCM_S16BE ? CODEC_ID_PCM_S24BE : CODEC_ID_PCM_S24LE;
				break;
		...
		sc->stts_count == 1 && sc->stts_data[0].duration == 1))
	{
		...
		for (i = 0; i < sc->chunk_count; i++)
		{
			...
			for (j = 0; j < sc->stsc_data[stsc_index].count; j++)
			{
				...
				if (!sc->keyframe_absent && (!sc->keyframe_count || current_sample + key_off == sc->keyframes[
					stss_index]))
				{
					keyframe = 1;
...
static int mov_read_tfhd(MOVContext* c, AVIOContext* pb, MOVAtom atom)
{
	...
	frag->base_data_offset = flags & MOV_TFHD_BASE_DATA_OFFSET ? avio_rb64(pb) : frag->moof_offset;
