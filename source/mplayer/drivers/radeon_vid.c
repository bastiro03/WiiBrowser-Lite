...
static void radeon_vid_display_video(void)
{
	...
	{
	case IMGFMT_RGB15:
	case IMGFMT_BGR15: bes_flags |= SCALER_SOURCE_15BPP;
		break;
	case IMGFMT_RGB16:
	case IMGFMT_BGR16: bes_flags |= SCALER_SOURCE_16BPP;
		break;
	case IMGFMT_RGB24:
	case IMGFMT_BGR24: bes_flags |= SCALER_SOURCE_24BPP;
		break;
	case IMGFMT_RGB32:
	case IMGFMT_BGR32: bes_flags |= SCALER_SOURCE_32BPP;
		break;
	/* 4:1:0*/
	case IMGFMT_IF09:
	case IMGFMT_YVU9: bes_flags |= SCALER_SOURCE_YUV9;
		break;
	/* 4:2:0 */
	case IMGFMT_IYUV:
	case IMGFMT_I420:
	case IMGFMT_YV12: bes_flags |= SCALER_SOURCE_YUV12;
		break;
	/* 4:2:2 */
	case IMGFMT_UYVY: bes_flags |= SCALER_SOURCE_YVYU422;
		break;
	case IMGFMT_YUY2:
	default: bes_flags |= SCALER_SOURCE_VYUY422;
		break;
	}
...
static int radeon_vid_init_video(mga_vid_config_t* config)
{
	...
	case IMGFMT_BGR15:
	case IMGFMT_RGB16:
	case IMGFMT_BGR16: pitch = ((src_w * 2) + 15) & ~15;
		break;
	case IMGFMT_RGB24:
	case IMGFMT_BGR24: pitch = ((src_w * 3) + 15) & ~15;
		break;
	case IMGFMT_RGB32:
	case IMGFMT_BGR32: pitch = ((src_w * 4) + 15) & ~15;
		break;
	}
...
static ssize_t radeon_vid_write(struct file* file, const char* buf, size_t count, loff_t* ppos)
{
	...
	else if (memcmp(buf, PARAM_SATURATION, min(count, strlen(PARAM_SATURATION))) == 0)
	{
		long saturation;
	...
	if (memcmp(buf, PARAM_BRIGHTNESS, min(count, strlen(PARAM_BRIGHTNESS))) == 0)
	{
		...
		else if (memcmp(buf, PARAM_DEINTERLACE, min(count, strlen(PARAM_DEINTERLACE))) == 0)
		{
			if (memcmp(&buf[strlen(PARAM_DEINTERLACE)], "on", 2) == 0) besr.deinterlace_on = 1;
			else besr.deinterlace_on = 0;
		}
		else if (memcmp(buf, PARAM_DEINTERLACE_PATTERN, min(count, strlen(PARAM_DEINTERLACE_PATTERN))) == 0)
		{
			long dpat;
