...
static int control(struct af_instance_s* af, int cmd, void* arg)
{
	...
	case AF_CONTROL_GATE_ON_OFF | AF_CONTROL_SET:
		memcpy(s->enable, arg, AF_NCH * sizeof(int));
		return AF_OK;
	case AF_CONTROL_GATE_ON_OFF | AF_CONTROL_GET:
		memcpy(arg, s->enable, AF_NCH * sizeof(int));
		return AF_OK;
	case AF_CONTROL_GATE_THRESH | AF_CONTROL_SET:
		return af_from_dB(AF_NCH, arg, s->tresh, 20.0, -60.0, -1.0);
	case AF_CONTROL_GATE_THRESH | AF_CONTROL_GET:
		return af_to_dB(AF_NCH, s->tresh, arg, 10.0);
	case AF_CONTROL_GATE_ATTACK | AF_CONTROL_SET:
		return af_from_ms(AF_NCH, arg, s->attack, af->data->rate, 500.0, 0.1);
	case AF_CONTROL_GATE_ATTACK | AF_CONTROL_GET:
		return af_to_ms(AF_NCH, s->attack, arg, af->data->rate);
	case AF_CONTROL_GATE_RELEASE | AF_CONTROL_SET:
		return af_from_ms(AF_NCH, arg, s->release, af->data->rate, 3000.0, 10.0);
	case AF_CONTROL_GATE_RELEASE | AF_CONTROL_GET:
		return af_to_ms(AF_NCH, s->release, arg, af->data->rate);
	case AF_CONTROL_GATE_RANGE | AF_CONTROL_SET:
		return af_from_dB(AF_NCH, arg, s->range, 20.0, 100.0, 0.0);
	case AF_CONTROL_GATE_RANGE | AF_CONTROL_GET:
		return af_to_dB(AF_NCH, s->range, arg, 10.0);
	}
	return AF_UNKNOWN;
}
...
static af_data_t* play(struct af_instance_s* af, af_data_t* data)
{
	...
	af_gate_t* s = af->setup; // Setup for this instance
