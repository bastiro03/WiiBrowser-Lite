...
int smooth_init(AVFilterContext* ctx, const char* args, void* opaque)
{
	...
	if (args)
		sscanf(args, "%127[^:]:%d:%d:%lf:%lf", type_str, &smooth->param1, &smooth->param2, &smooth->param3,
		       &smooth->param4);
...
int dilate_init(AVFilterContext* ctx, const char* args, void* opaque)
{
	OCVContext* ocv = ctx->priv;
...
void dilate_uninit(AVFilterContext* ctx)
{
	OCVContext* ocv = ctx->priv;
	DilateContext* dilate = ocv->priv;
