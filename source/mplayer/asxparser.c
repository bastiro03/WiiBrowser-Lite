...
asx_parse_attribs(ASX_Parser_t* parser, char* buffer, char*** _attribs)
{
	...
	while (1)
	{
		for (; strchr(SPACE, *ptr1) != NULL; ptr1++)
		{
			// Skip space
			if (*ptr1 == '\0') break;
		}
		...
		for (ptr2 = ptr3 - 1; strchr(SPACE, *ptr2) != NULL; ptr2--)
		{
			if (ptr2 == ptr1)
			{
				mp_msg(MSGT_PLAYTREE, MSGL_ERR,
				       "At line %d : this should never append, back to attribute begin while skipping end space",
				       parser->line);
				break;
			}
		}
	...
	*_attribs = NULL;
	...
	if (parser->ret_stack && /*parser->last_body && */buffer != parser->last_body)
	{
		...
		if (i < parser->ret_stack_size)
		{
			i++;
			if (i < parser->ret_stack_size)
				memmove(parser->ret_stack, parser->ret_stack + i,
				        (parser->ret_stack_size - i) * sizeof(ASX_LineSave_t));
	...
	for (ptr1++; strchr(SPACE, ptr1[0]) != NULL; ptr1++)
	{
		// Skip space
		if (ptr1[0] == '\0')
		{
	...
	for (ptr2 = ptr1; strchr(LETTER, *ptr2) != NULL; ptr2++)
	{
		// Go to end of name
		if (*ptr2 == '\0')
		{
	...
	for (; strchr(SPACE, *ptr2) != NULL; ptr2++)
	{
		// Skip space
		if (ptr2[0] == '\0')
		{
	...
	if (ptr3[0] != '/')
	{
		// Not Self closed element
		ptr3++;
		for (; strchr(SPACE, *ptr3) != NULL; ptr3++)
		{
			// Skip space on body begin
			if (*ptr3 == '\0')
			{
				mp_msg(MSGT_PLAYTREE, MSGL_ERR, "At line %d : EOB reached while parsing %s element body", parser->line,
				       element);
				free(element);
				free(attribs);
				return -1;
		...
		while (1)
		{
			...
			if (strncasecmp(element, ptr4 + 2, strlen(element)) == 0)
			{
				// Extract body
				if (in > 0)
				{
	...
	for (; ret[0] != '\0' && strchr(SPACE, ret[0]) != NULL; ret++)
	{
		// Skip space
		if (ret[0] == '\n') parser->line++;
	}
	...
	if (attribs)
	{
		...
		if (n_attrib < 0)
		{
			mp_msg(MSGT_PLAYTREE, MSGL_WARN, "At line %d : error while parsing element %s attributes", parser->line,
			       element);
			free(element);
			free(body);
			return -1;
