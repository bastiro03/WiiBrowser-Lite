...
{
	return (DVDFileSeekForce(dvd_file, offset, force_size) == (int)offset);
}

static inline int DVDFileSeek_(dvd_file_t* dvd_file, uint32_t offset)
{
	return (DVDFileSeek(dvd_file, offset) == (int)offset);
}
...
ifo_handle_t* ifoOpen(dvd_reader_t* dvd, int title)
{
	...
	if (!ifofile->file)
	{
		/* Failed to open IFO, try to open BUP */
...
int ifoRead_VTS_TMAPT(ifo_handle_t* ifofile)
{
	...
	for (i = 0; i < vts_tmapt->nr_of_tmaps; i++)
	{
		...
		if (vts_tmapt->tmap[i].nr_of_entries == 0)
		{
			/* Early out if zero entries */
                                  ...
                                  unsigned int offset)
{
	...
	CHECK_VALUE(vts_attributes->nr_of_vtstt_subp_streams <= 32);
	{
		...
		if (nr_coded > 32)
		{
			/* We haven't read more from disk/file anyway */
			nr_coded = 32;
		}
