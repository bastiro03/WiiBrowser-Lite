                           ...
                           int ref_index, int size, int h, int add_rate)
{
	...
	d = cmp(s, mx >> (qpel + 1), my >> (qpel + 1), mx & mask, my & mask, size, h, ref_index, src_index, cmp_sub,
                              ...
                              int size, int h)
{
		...
		my > ymin && my < ymax)
	{
		...
		else
		{
			...
			if (map[(index - (1 << ME_MAP_SHIFT) - 1) & (ME_MAP_SIZE - 1)] == (my << ME_MAP_MV_BITS) + mx +
				map_generation && 0)
			{
				//FIXME
			...
			for (ny = -3; ny <= 3; ny++)
			{
				for (nx = -3; nx <= 3; nx++)
				{
					...
					score += 32 * (mv_penalty[4 * mx + nx - pred_x] + mv_penalty[4 * my + ny - pred_y]) *
						penalty_factor;
...
int small_diamond_search(MpegEncContext* s, int* best, int dmin,
                              ...
                              int size, int h, int flags)
{
	...
	for (j = i = 0; i < ME_MAP_SIZE && j < MAX_SAB_SIZE; i++)
	{
		...
		if (minima[j].x || minima[j].y)
			minima[j].height += (mv_penalty[((minima[j].x) << shift) - pred_x] + mv_penalty[((minima[j].y) << shift) -
...
int diamond_search(MpegEncContext* s, int* best, int dmin,
                   int src_index, int ref_index, const int penalty_factor,
                   int size, int h, int flags)
{
	MotionEstContext* const c = &s->me;
...
int epzs_motion_search_internal(MpegEncContext* s, int* mx_ptr, int* my_ptr,
