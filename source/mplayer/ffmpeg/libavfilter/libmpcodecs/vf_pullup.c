...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	p = mpi->fields & MP_IMGFIELD_TOP_FIRST ? 0 : (mpi->fields & MP_IMGFIELD_ORDERED ? 1 : 0);
...
static int vf_open(vf_instance_t* vf, char* args)
{
	...
	if (args)
	{
		sscanf(args, "%d:%d:%d:%d:%d:%d", &c->junk_left, &c->junk_right, &c->junk_top, &c->junk_bottom,
		       &c->strict_breaks, &c->metric_plane);
	}
	return 1;
}
