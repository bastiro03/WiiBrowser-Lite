...
static void mxf_write_identification(AVFormatContext* s)
{
	...
	version = s->streams[0]->codec->flags & CODEC_FLAG_BITEXACT ? "0.0.0" : AV_STRINGIFY(LIBAVFORMAT_VERSION);
...
static void mxf_write_timecode_component(AVFormatContext* s, AVStream* st, enum MXFMetadataSetType type)
{
	...
	// UID
	mxf_write_local_tag(pb, 16, 0x3C0A);
	mxf_write_uuid(pb, type == MaterialPackage ? TimecodeComponent : TimecodeComponent + TypeBottom, st->index);
...
static void mxf_write_package(AVFormatContext* s, enum MXFMetadataSetType type)
{
	...
	mxf_write_uuid(pb, type == MaterialPackage ? Track : Track + TypeBottom, -1); // timecode track
	for (i = 0; i < s->nb_streams; i++)
...
static const UID* mxf_get_mpeg2_codec_ul(AVCodecContext* avctx)
{
	...
	if (avctx->profile == 4)
	{
		...
		if (avctx->level == 4) // High
			return &mxf_mpeg2_codec_uls[4 + long_gop];
		if (avctx->level == 6) // High 14
                                 ...
                                 AVPacket* pkt, MXFIndexEntry* e)
{
	...
	for (i = 0; i < pkt->size - 4; i++)
	{
		...
		else if (c == 0x1b3)
		{
			...
			{
			case 2: sc->aspect_ratio = (AVRational)
				{
					4, 3
				}
				break;
			case 3: sc->aspect_ratio = (AVRational)
				{
					16, 9
				}
				break;
			case 4: sc->aspect_ratio = (AVRational)
				{
					221, 100
				}
				break;
...
static int mxf_write_header(AVFormatContext* s)
{
	...
	for (i = 0; i < s->nb_streams; i++)
	{
		...
		if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
		{
			...
			rate = (AVRational)
			{
				mxf->time_base.den, mxf->time_base.num
			}
