...
static int get_ext_stream_properties(char* buf, int buf_len, int stream_num, struct asf_priv* asf, int is_video)
{
	...
	while ((pos = find_asf_guid(buf, asf_ext_stream_header, pos, buf_len)) >= 0)
	{
		...
		buffer += 8 * 4;
		this_stream_num = AV_RL16(buffer);
		buffer += 2;
		...
		if (this_stream_num == stream_num)
		{
			...
			stnamect = AV_RL16(buffer);
			buffer += 2;
			payct = AV_RL16(buffer);
			buffer += 2;
			...
			for (i = 0; i < stnamect; i++)
			{
				...
				stream_name_len = AV_RL16(buffer);
				buffer += 2;
...
static int asf_init_audio_stream(demuxer_t* demuxer, struct asf_priv* asf, sh_audio_t* sh_audio,
                                 ASF_stream_header_t* streamh, int* ppos, uint8_t** buf, char* hdr,
