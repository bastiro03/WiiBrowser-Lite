...
{
	return (unsigned)pix_fmt < PIX_FMT_NB ? format_entries[pix_fmt].is_supported_in : 0;
}

int sws_isSupportedOutput(enum PixelFormat pix_fmt)
{
	return (unsigned)pix_fmt < PIX_FMT_NB ? format_entries[pix_fmt].is_supported_out : 0;
}
		...
		for (i = 0; i < dstW; i++)
		{
			...
			for (j = 0; j < filterSize; j++)
			{
				...
				else if (flags & SWS_LANCZOS)
				{
					double p = param[0] != SWS_PARAM_DEFAULT ? param[0] : 3.0;
					coeff = (d
						         ? sin(floatd * M_PI) * sin(floatd * M_PI / p) /
						         (floatd * floatd * M_PI * M_PI / p)
						         : 1.0) * fone;
					if (floatd > p)
						coeff = 0;
				}
...
static int handle_0alpha(enum PixelFormat* format)
{
	switch (*format)
	{
	case PIX_FMT_0BGR: *format = PIX_FMT_ABGR;
		return 1;
	case PIX_FMT_BGR0: *format = PIX_FMT_BGRA;
		return 4;
	case PIX_FMT_0RGB: *format = PIX_FMT_ARGB;
		return 1;
	case PIX_FMT_RGB0: *format = PIX_FMT_RGBA;
		return 4;
	default: return 0;
	}
}
...
int sws_init_context(SwsContext* c, SwsFilter* srcFilter, SwsFilter* dstFilter)
{
	...
	if (flags & SWS_FAST_BILINEAR)
	{
		...
		else if (HAVE_MMX&& cpu_flags & AV_CPU_FLAG_MMX && c
		->
		dstBpc <= 10
		)
		{
...
#endif /* HAVE_MMX2 */
		{
			const int filterAlign =
				(HAVE_MMX && cpu_flags & AV_CPU_FLAG_MMX)
					? 4
					: (HAVE_ALTIVEC && cpu_flags & AV_CPU_FLAG_ALTIVEC)
					? 8
					: 1;
	...
	{
		const int filterAlign =
			(HAVE_MMX && cpu_flags & AV_CPU_FLAG_MMX) ? 2 : (HAVE_ALTIVEC && cpu_flags & AV_CPU_FLAG_ALTIVEC) ? 8 : 1;
