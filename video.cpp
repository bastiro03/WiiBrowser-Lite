...
unsigned int* xfb[2] = {nullptr, nullptr}; // Double buffered
u8 whichfb = 0; // Switch
bool need_wait = false;

GXRModeObj* vmode; // Menu video mode
u8* videoScreenshot = nullptr;
...
u8* TakeScreenshot(bool global)
{
	...
	if (!global)
		video = static_cast<u8*>(memalign(32, vmode->fbWidth * vmode->efbHeight * 4));
...
InitVideo2()
{
	...
	xfb[0] = static_cast<u32*>(SYS_AllocateFramebuffer(vmode));
	xfb[1] = static_cast<u32*>(SYS_AllocateFramebuffer(vmode));
	DCInvalidateRange(xfb[0], VIDEO_GetFrameBufferSize(vmode));
	DCInvalidateRange(xfb[1], VIDEO_GetFrameBufferSize(vmode));
	xfb[0] = static_cast<u32*>(MEM_K0_TO_K1(xfb[0]));
	xfb[1] = static_cast<u32*>(MEM_K0_TO_K1(xfb[1]));
	...
	gp_fifo = static_cast<unsigned char*>(memalign(32, DEFAULT_FIFO_SIZE));
	...
	videoScreenshot = static_cast<u8*>(memalign(32, vmode->fbWidth * vmode->efbHeight * 4));
}
}
