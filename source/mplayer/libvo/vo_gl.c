...
static void genEOSD(struct mp_eosd_image_list* imgs)
{
	...
	if (!largeeosdtex[0])
	{
		...
		glCreateClearTex(gl_target, GL_ALPHA, GL_ALPHA, GL_UNSIGNED_BYTE, scale_type, LARGE_EOSD_TEX_SIZE,
		                 LARGE_EOSD_TEX_SIZE, 0);
...
static void uninitGl(void)
{
	...
	gl_buffer_uv[0] = gl_buffer_uv[1] = 0;
	gl_buffersize_uv = 0;
