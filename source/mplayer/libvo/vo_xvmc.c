                  ...
                  uint32_t flags, char* title, uint32_t format)
{
	...
	if (number_of_surfaces < 4)
	{
		// +2 I or P and +2 for B (to avoid visible motion drawing)
	...
	if (subpicture.num_palette_entries > 0)
	{
		...
		for (i = 0; i < snum; i++)
		{
			...
			for (j = 0; j < seb; j++)
				...
				{
				case 'U': palette[i * seb + j] = U;
					break;
				case 'V': palette[i * seb + j] = V;
					break;
				case 'Y':
				default:
					palette[i * seb + j] = Y;
...
static void draw_osd_IA44(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca, int stride)
{
	...
	for (ox = 0; ox < w; ox++)
	{
		for (oy = 0; oy < h; oy++)
		{
			xvimage->data[oy * xvimage->width + ox] = (src[oy * stride + ox] >> 4) | ((0 - srca[oy * stride + ox]) &
				0xf0);
		}
	}
...
static void draw_osd_AI44(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca, int stride)
{
	...
	for (ox = 0; ox < w; ox++)
	{
		for (oy = 0; oy < h; oy++)
		{
			xvimage->data[oy * xvimage->width + ox] = (src[oy * stride + ox] & 0xf0) | (((0 - srca[oy * stride + ox]) >>
				4) & 0xf);
		}
	}
	rez = XvMCCompositeSubpicture(mDisplay, &subpicture, xvimage, 0, 0,
...
static void flip_page(void)
{
	...
	if (use_queue)
	{
		...
		if (cfs > 3)
		{
			//well have 3 free surfaces after add queue
