...
{
	unsigned int fmt;
	/* Not exactly sure of the use for this. It came with the example filter I used as a basis for this, and it looks like a lot of stuff will break if I remove it. */
	int max_mask_size;
	/* The largest possible mask size that will be needed with the given filter and corresponding half_size_filter. The half_size_filter can have a larger requirment in some rare (but not degenerate) cases. */
	int*** mask;
	/* Stores our collection of masks. The first * is for an array of masks, the second for the y axis, and the third for the x axis. */
	pgm_structure* filter;
	/* Stores the full-size filter image. This is used to tell what pixels are in the logo or not in the luma plane. */
	pgm_structure* half_size_filter;
	/* Stores a 50% width and 50% height filter image. This is used to tell what pixels are in the logo or not in the chroma planes. */
...
static void convert_mask_to_strength_mask(vf_instance_t* vf, pgm_structure* mask)
{
		...
		for (y = 1; y < mask->height - 1; y++)
		{
			for (x = 1; x < mask->width - 1; x++)
			{
				...
				if (*current_pixel >= current_pass &&
					/* By using >= instead of ==, we allow the algorithm to work in place. */
				...
				{
					(*current_pixel)++;
					/* Increment the value since it still has not been eroded, as evidenced by the if statement
	...
	max_mask_size = current_pass + 1;
	/* As a side-effect, we now know the maximum mask size, which we'll use to generate our masks. */
...
{
	int mask_size;
	/* Mask size tells how large a circle to use. The radius is about (slightly larger than) mask size. */
...
static pgm_structure* load_pgm(const char* file_name)
{
	...
	if ((input = fopen(file_name, "rb")) == NULL) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: Unable to open file. File not found or insufficient permissions.\n");

	/* Parse the PGM header. */
	if (fgetc(input) != 'P') REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: File is not a valid PGM or PPM file.\n");
	pnm_number = fgetc(input) - '0';
	if (pnm_number != 5 && pnm_number != 6) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: Invalid PNM file. Only raw PGM (Portable Gray Map) and raw PPM (Portable Pixel Map) subtypes are allowed.\n");
	load_pgm_skip(input);
	if (fscanf(input, "%i", &(new_pgm->width)) != 1) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: Invalid PGM/PPM header.\n");
	load_pgm_skip(input);
	if (fscanf(input, "%i", &(new_pgm->height)) != 1) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: Invalid PGM/PPM header.\n");
	load_pgm_skip(input);
	if (fscanf(input, "%i", &maximum_greyscale_value) != 1) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove-logo: Invalid PGM/PPM header.\n");
	if (maximum_greyscale_value >= 256) REMOVE_LOGO_LOAD_PGM_ERROR_MESSAGE(
		"[vf]remove_logo: Only 1 byte per pixel (pgm) or 1 byte per color value (ppm) are supported.\n");
	load_pgm_skip(input);
...
static pgm_structure* generate_half_size_image(vf_instance_t* vf, pgm_structure* input_image)
{
	...
	while (has_anything_changed)
	{
		...
		for (y = 1; y < new_pgm->height - 1; y++)
		{
			...
			{
				if (new_pgm->pixel[(y * new_pgm->width) + x] >= current_pass &&
					/* By using >= instead of ==, we allow the algorithm to work in place. */
					...
					new_pgm->pixel[((y - 1) * new_pgm->width) + x] >= current_pass)
				{
					new_pgm->pixel[(y * new_pgm->width) + x]++;
					/* Increment the value since it still has not been eroded,
	...
	for (y = 1; y < new_pgm->height - 1; y++)
	{
		...
		{
			new_pgm->pixel[(y * new_pgm->width) + x] =
				apply_mask_fudge_factor(new_pgm->pixel[(y * new_pgm->width) + x]);
		}
	}
 ...
 */
static int config(struct vf_instance* vf, int width, int height, int d_width, int d_height, unsigned int flags,
                  unsigned int outfmt)
{
...
static void convert_yv12(const vf_instance_t* const vf, const char* const source, const int source_stride,
                         ...
                         char* const destination, const int destination_stride, int is_image_direct,
                         pgm_structure* filter,
                         const int plane, const int logo_start_x, const int logo_start_y, const int logo_end_x,
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (vf->priv->filter->width != mpi->w || vf->priv->filter->height != mpi->h)
	{
		mp_msg(MSGT_VFILTER, MSGL_ERR,
		       "Filter image and video stream are not of the same size. (Filter: %d x %d, Stream: %d x %d)\n",
