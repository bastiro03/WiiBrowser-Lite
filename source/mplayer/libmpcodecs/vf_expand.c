                  ...
                  unsigned int flags, unsigned int outfmt)
{
	...
	if (vf->priv->exp_x < 0 || vf->priv->exp_x + width > vf->priv->exp_w) vf->priv->exp_x = (vf->priv->exp_w - width) /
		2;
	if (vf->priv->exp_y < 0 || vf->priv->exp_y + height > vf->priv->exp_h) vf->priv->exp_y = (vf->priv->exp_h - height)
		/ 2;
		...
		(mpi->flags & (MP_IMGFLAG_ACCEPT_STRIDE | MP_IMGFLAG_ACCEPT_WIDTH)))
	{
		...
		if (mpi->flags & MP_IMGFLAG_PLANAR)
		{
			...
			mpi->planes[1] = vf->dmpi->planes[1] +
				(vf->priv->exp_y >> mpi->chroma_y_shift) * vf->dmpi->stride[1] + (vf->priv->exp_x >> mpi->
					chroma_x_shift);
			mpi->planes[2] = vf->dmpi->planes[2] +
				(vf->priv->exp_y >> mpi->chroma_y_shift) * vf->dmpi->stride[2] + (vf->priv->exp_x >> mpi->
					chroma_x_shift);
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (mpi->flags & MP_IMGFLAG_DIRECT || mpi->flags & MP_IMGFLAG_DRAW_CALLBACK)
	{
		...
		if (!vf->dmpi)
		{
			mp_msg(MSGT_VFILTER, MSGL_WARN, MSGTR_MPCODECS_FunWhydowegetNULL);
			return 0;
		}
		mpi->priv = NULL;
#ifdef OSD_SUPPORT
