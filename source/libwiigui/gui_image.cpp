...
{
	image = static_cast<u8*>(memalign(32, w * h << 2));
	width = w;
	height = h;
	imageangle = 0;
	tile = -1;
...
void GuiImage::Draw()
{
	...
	else if (tileVertical >= 0)
	{
		...
		{
			Menu_DrawImg(currLeft, currTop + height * i, width, height, image, imageangle, currScaleX, currScaleY,
			             alpha, format);
		}
	}
	else
	...
	if (stripe > 0)
	{
		...
		for (int y = 0; y < thisHeight; y += 6)
			Menu_DrawRectangle(currLeft, currTop + y, thisWidth, 3, (GXColor)
		{
			0, 0, 0, static_cast<u8>(stripe)
		}
		,
		1
		)
	...
	if (effects & EFFECT_ROTATE)
	{
		...
		if (diff_usec(prev, now) > (u32)(effectAmount * 1000))
		{
			...
			if (imageangle >= 360.0f)
				imageangle = static_cast<int>(imageangle) % 360;
		}
	}
