...
static void draw_alpha(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca, int stride)
{
	...
	case IMGFMT_I420:
		vo_draw_alpha_yv12(w, h, src, srca, stride,
		                   ((char*)P) + be2me_32(P->componentInfoY.offset) + x0 + y0 * imgRect.right, imgRect.right);
		break;
	case IMGFMT_UYVY:
...
static OSStatus KeyEventHandler(EventHandlerCallRef nextHandler, EventRef event, void* userData)
{
	...
	if (class == kEventClassKeyboard)
	{
		...
		GetEventParameter(event, kEventParamKeyModifiers, typeUInt32, NULL, sizeof(macKeyModifiers), NULL,
		                  &macKeyModifiers);
		...
		else if (macKeyModifiers == 256)
		{
			switch (macCharCodes)
			{
			case '[': SetWindowAlpha(theWindow, winAlpha -= 0.05);
				break;
			case ']': SetWindowAlpha(theWindow, winAlpha += 0.05);
				break;
			}
...
static OSStatus WindowEventHandler(EventHandlerCallRef nextHandler, EventRef event, void* userData)
{
	...
	if (class == kEventClassCommand)
	{
		...
		GetEventParameter(event, kEventParamDirectObject, typeHICommand, NULL, sizeof(theHICommand), NULL,
		                  &theHICommand);
...
static void quartz_CreateWindow(uint32_t d_width, uint32_t d_height, WindowAttributes windowAttrs)
{
	...
	InstallApplicationEventHandler(NewEventHandlerUPP(KeyEventHandler), GetEventTypeCount(key_events), key_events, NULL,
	                               NULL);
	InstallApplicationEventHandler(NewEventHandlerUPP(MouseEventHandler), GetEventTypeCount(mouse_events), mouse_events,
	                               NULL, NULL);
	InstallWindowEventHandler(theWindow, NewEventHandlerUPP(WindowEventHandler), GetEventTypeCount(win_events),
	                          win_events, theWindow, NULL);
}
...
static void update_screen_info(void)
{
	...
	if (kCGErrorSuccess != CGGetActiveDisplayList(displayCount, displays, &displayCount) || displayCount < device_id +
		1)
	{
	...
	case IMGFMT_YUY2:
		{
			...
			{
				ScaleMatrix(&matrix, FixDiv(Long2Fix(d_width), Long2Fix(width)),
				            FixDiv(Long2Fix(d_height), Long2Fix(height)), 0, 0);
			}
			...
			((NCLCColorInfoImageDescriptionExtension*)(*yuv_qt_stuff.extension_colr))->colorParamType =
			...
			((CleanApertureImageDescriptionExtension*)(*yuv_qt_stuff.extension_clap))->cleanApertureWidthN = imgRect.
				right;
			((CleanApertureImageDescriptionExtension*)(*yuv_qt_stuff.extension_clap))->cleanApertureWidthD = 1;
			((CleanApertureImageDescriptionExtension*)(*yuv_qt_stuff.extension_clap))->cleanApertureHeightN = imgRect.
				bottom;
			...
			qterr = AddImageDescriptionExtension(yuv_qt_stuff.desc, yuv_qt_stuff.extension_colr,
			...
			qterr = AddImageDescriptionExtension(yuv_qt_stuff.desc, yuv_qt_stuff.extension_fiel,
			...
			qterr = AddImageDescriptionExtension(yuv_qt_stuff.desc, yuv_qt_stuff.extension_clap,
			...
			qterr = AddImageDescriptionExtension(yuv_qt_stuff.desc, yuv_qt_stuff.extension_pasp,
			                                     kCleanApertureImageDescriptionExtension);
			if (qterr)
			{
...
static void flip_page(void)
{
	...
	if (!vo_quartz_fs)
	{
		...
		CGContextMoveToPoint(context, winRect.right - 1, 1);
		CGContextAddLineToPoint(context, winRect.right - 1, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 5);
		CGContextAddLineToPoint(context, winRect.right - 5, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 9);
		CGContextAddLineToPoint(context, winRect.right - 9, 1);
		CGContextStrokePath(context);

		// line gray
		CGContextSetRGBStrokeColor(context, 0.4, 0.4, 0.4, 0.5);
		CGContextMoveToPoint(context, winRect.right - 1, 2);
		CGContextAddLineToPoint(context, winRect.right - 2, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 6);
		CGContextAddLineToPoint(context, winRect.right - 6, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 10);
		CGContextAddLineToPoint(context, winRect.right - 10, 1);
		CGContextStrokePath(context);

		// line black
		CGContextSetRGBStrokeColor(context, 0.6, 0.6, 0.6, 0.5);
		CGContextMoveToPoint(context, winRect.right - 1, 3);
		CGContextAddLineToPoint(context, winRect.right - 3, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 7);
		CGContextAddLineToPoint(context, winRect.right - 7, 1);
		CGContextMoveToPoint(context, winRect.right - 1, 11);
		CGContextAddLineToPoint(context, winRect.right - 11, 1);
		CGContextStrokePath(context);
...
static int draw_slice(uint8_t* src[], int stride[], int w, int h, int x, int y)
{
	...
	case IMGFMT_YV12:
	case IMGFMT_I420:
		memcpy_pic(((char*)P) + be2me_32(P->componentInfoY.offset) + x + imgRect.right * y, src[0], w, h, imgRect.right,
		           stride[0]);
		x = x / 2;
		y = y / 2;
		w = w / 2;
		...
		memcpy_pic(((char*)P) + be2me_32(P->componentInfoCb.offset) + x + imgRect.right / 2 * y, src[1], w, h,
		           imgRect.right / 2, stride[1]);
		memcpy_pic(((char*)P) + be2me_32(P->componentInfoCr.offset) + x + imgRect.right / 2 * y, src[2], w, h,
		           imgRect.right / 2, stride[2]);
		return 0;
	...
	case IMGFMT_IYUV:
		...
		memcpy_pic(((char*)P) + be2me_32(P->componentInfoCr.offset) + x + imgRect.right / 2 * y, src[1], w, h,
		           imgRect.right / 2, stride[1]);
		memcpy_pic(((char*)P) + be2me_32(P->componentInfoCb.offset) + x + imgRect.right / 2 * y, src[2], w, h,
		           imgRect.right / 2, stride[2]);
		return 0;
	}
	return -1;
...
static int control(uint32_t request, void* data)
{
	...
	case VOCTRL_FULLSCREEN: vo_fs = !vo_fs;
		window_fullscreen();
		return VO_TRUE;
	case VOCTRL_ONTOP: vo_ontop = !vo_ontop;
		window_ontop();
		return VO_TRUE;
	case VOCTRL_QUERY_FORMAT: return query_format(*(uint32_t*)data);
	case VOCTRL_GET_PANSCAN: return VO_TRUE;
	case VOCTRL_SET_PANSCAN: window_panscan();
		return VO_TRUE;
				...
				if (mode != NULL)
				{
					if (!exactMatch)
					{
						...
						mp_msg(MSGT_VO, MSGL_WARN, "Quartz warning: did not get exact mode match (got %dx%d) \n",
						       (int)CFDictionaryGetValue(mode, kCGDisplayWidth),
						       (int)CFDictionaryGetValue(mode, kCGDisplayHeight));
					}
