...
static const AVOption options[] = {
	{
		"no_resync_search", "Don't try to resynchronize by looking for a certain optional start code",
		offsetof(ASFContext, no_resync_search), AV_OPT_TYPE_INT, {.dbl = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM
	},
	{NULL},
};
...
static int asf_read_header(AVFormatContext* s)
{
	...
	for (i = 0; i < 128; i++)
	{
		...
		if (stream_num >= 0)
		{
			...
			else if ((asf->dar[0].num > 0) && (asf->dar[0].den > 0) && (st->codec->codec_type == AVMEDIA_TYPE_VIDEO))
				// Use ASF container value if the stream doesn't AR set.
				av_reduce(&st->sample_aspect_ratio.num,
				          &st->sample_aspect_ratio.den,
