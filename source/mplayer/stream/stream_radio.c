...
static int parse_channels(radio_priv_t* priv, float freq_channel, float* pfreq)
{
	...
	if (priv->radio_param->channels)
	{
		...
		while (*channels)
		{
			...
			if ((priv->radio_channel_current->freq > priv->rangehigh) || (priv->radio_channel_current->freq < priv->
				rangelow))
	...
	else
	{
		if (freq_channel)
		{
			...
			priv->radio_channel_list->prev = NULL;
			priv->radio_channel_list->index = 1;
			snprintf(priv->radio_channel_list->name, sizeof(priv->radio_channel_current->name) - 1, "Freq: %.2f",
			         freq_channel);
