...
int bktr_init(const char* video_device, int width, int height,
              int format, int* video_fd, int* tuner_fd, int idev, double frequency)
{
	...
	*video_fd = open(video_device, O_RDONLY);
	...
	case PAL: h_max = PAL_HEIGHT;
		...
		break;
	case PALN: h_max = PAL_HEIGHT;
		c = BT848_IFORM_F_PALN;
		break;
	case PALM: h_max = PAL_HEIGHT;
...
static int grab_read_header(AVFormatContext* s1)
{
	...
	if (!s->framerate)
		switch (s->standard)
		{
		case PAL: s->framerate = av_strdup("pal");
			break;
		case NTSC: s->framerate = av_strdup("ntsc");
			break;
		case SECAM: s->framerate = av_strdup("25");
			break;
		default:
...
static const AVOption options[] = {
	{
		"standard", "", offsetof(VideoData, standard), AV_OPT_TYPE_INT, {.dbl = VIDEO_FORMAT}, PAL, NTSCJ,
	...
	{"NTSCJ", "", 0, AV_OPT_TYPE_CONST, {.dbl = NTSCJ}, 0, 0, AV_OPT_FLAG_DECODING_PARAM, "standard"},
	{
		...
		AV_OPT_TYPE_IMAGE_SIZE, {.str = "vga"}, 0, 0, DEC
	},
