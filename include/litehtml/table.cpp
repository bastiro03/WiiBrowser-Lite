...
extern int wtoi(const wchar_t* str);

void litehtml::table_grid::add_cell(element* el)
{
	table_cell cell;
	cell.el = el;
	cell.colspan = wtoi(el->get_attr(L"colspan", L"1"));
	cell.rowspan = wtoi(el->get_attr(L"rowspan", L"1"));
...
void litehtml::table_grid::finish()
{
	...
	{
		m_cols_count = max(m_cols_count, static_cast<int>(m_cells[i].size()));
	}
	for (int i = 0; i < static_cast<int>(m_cells.size()); i++)
	{
...
{
	auto t = new table_column_accessor_max_width();
	distribute_width(width, start, end, t);
}

void litehtml::table_grid::distribute_min_width(int width, int start, int end)
{
	auto t = new table_column_accessor_min_width();
	distribute_width(width, start, end, t);
}
...
void litehtml::table_grid::distribute_width(int width, int start, int end)
{
	...
	for (int step = 0; step < 3; step++)
	{
		...
		{
			int cols_width = 0;
			for (std::vector<table_column*>::iterator col = distribute_columns.begin(); col != distribute_columns.end();
			     col++)
			{
			...
			if (cols_width)
			{
				...
				for (std::vector<table_column*>::iterator col = distribute_columns.begin(); col != distribute_columns.
				     end(); col++)
				{
					add = round_f(
