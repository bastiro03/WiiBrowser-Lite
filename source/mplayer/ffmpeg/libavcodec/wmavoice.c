...
enum
{
	...
	///< hardcoded (fixed) codebook with per-block
	   ///< gain values
	FCB_TYPE_AW_PULSES = 2,
	///< Pitch-adaptive window (AW) pulse signals,
	   ///< used in particular for low-bitrate streams
	FCB_TYPE_EXC_PULSES = 3,
	///< Innovation (fixed) codebook pulse sets in
...
int decode_vbmtree(GetBitContext* gb, int8_t vbm_tree[25])
{
...
int wmavoice_decode_init(AVCodecContext* ctx)
{
...
int wmavoice_decode_end(AVCodecContext* ctx)
{
	WMAVoiceContext* s = ctx->priv_data;
...
void wmavoice_flush(AVCodecContext* ctx)
{
	WMAVoiceContext* s = ctx->priv_data;
	int n;
