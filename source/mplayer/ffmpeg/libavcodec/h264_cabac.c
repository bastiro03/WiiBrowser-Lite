...
static int decode_cabac_field_decoding_flag(H264Context* h)
{
	...
	ctx += h->mb_field_decoding_flag & !!s->mb_x;
	//for FMO:(s->current_picture.f.mb_type[mba_xy] >> 7) & (h->slice_table[mba_xy] == h->slice_num);
...
static int decode_cabac_mb_mvd(H264Context* h, int ctxbase, int amvd, int* mvda)
{
	...
	if (!get_cabac(
		&h->cabac, &h->cabac_state[ctxbase + ((amvd - 3) >> (INT_BIT - 1)) + ((amvd - 33) >> (INT_BIT - 1)) + 2]))
	{
...
int get_cabac_cbf_ctx(H264Context* h, int cat, int idx, int max_coeff, int is_dc)
{
	int nza, nzb;
	int ctx = 0;
                               ...
                               int is_dc, int chroma422)
{
	static const int significant_coeff_flag_offset[2][14] = {
		...
		{
			277 + 0, 277 + 15, 277 + 29, 277 + 44, 277 + 47, 436, 776 + 0, 776 + 15, 776 + 29, 675, 820 + 0, 820 + 15,
			820 + 29, 733
		}
	};
	static const int last_coeff_flag_offset[2][14] = {
		{
			166 + 0, 166 + 15, 166 + 29, 166 + 44, 166 + 47, 417, 572 + 0, 572 + 15, 572 + 29, 690, 616 + 0, 616 + 15,
			616 + 29, 748
		},
		{
			338 + 0, 338 + 15, 338 + 29, 338 + 44, 338 + 47, 451, 864 + 0, 864 + 15, 864 + 29, 699, 908 + 0, 908 + 15,
			908 + 29, 757
		}
	};
	static const int coeff_abs_level_m1_offset[14] = {
		227 + 0, 227 + 10, 227 + 20, 227 + 30, 227 + 39, 426, 952 + 0, 952 + 10, 952 + 20, 708, 982 + 0, 982 + 10,
		982 + 20, 766
	};
...
static void decode_cabac_residual_dc_internal(H264Context* h, DCTELEM* block, int cat, int n, const uint8_t* scantable,
                                              int max_coeff)
{
...
static void decode_cabac_residual_nondc_internal(H264Context* h, DCTELEM* block, int cat, int n,
...
void decode_cabac_residual_dc(H264Context* h, DCTELEM* block, int cat, int n, const uint8_t* scantable, int max_coeff)
{
...
void decode_cabac_residual_nondc(H264Context* h, DCTELEM* block, int cat, int n, const uint8_t* scantable,
                                 const uint32_t* qmul, int max_coeff)
{
	/* read coded block flag */
	if ((cat != 5 || CHROMA444) && get_cabac(&h->cabac, &h->cabac_state[get_cabac_cbf_ctx(h, cat, n, max_coeff, 0)]) ==
		0)
	{
		if (max_coeff == 64)
		{
...
void decode_cabac_luma_residual(H264Context* h, const uint8_t* scan, const uint8_t* scan8x8, int pixel_shift,
	...
	if (IS_INTRA16x16(mb_type))
	{
		...
		if (cbp & 15)
		{
			...
			for (i4x4 = 0; i4x4 < 16; i4x4++)
			{
				...
				decode_cabac_residual_nondc(h, h->mb + (16 * index << pixel_shift), ctx_cat[1][p], index, scan + 1,
				                            qmul, 15);
			}
		}
		else
			...
			if (cbp & (1 << i8x8))
			{
				...
				else
				{
					...
					for (i4x4 = 0; i4x4 < 4; i4x4++)
					{
						...
						//START_TIMER
						decode_cabac_residual_nondc(h, h->mb + (16 * index << pixel_shift), ctx_cat[2][p], index, scan,
						                            qmul, 16);
			...
			if (cbp & 0x30)
			{
				...
				for (c = 0; c < 2; c++)
				{
					...
					decode_cabac_residual_dc(h, h->mb + ((256 + 16 * 16 * c) << pixel_shift), 3,
					                         CHROMA_DC_BLOCK_INDEX + c, chroma_dc_scan, 4);
				}
			}
			...
			if (cbp & 0x20)
			{
				...
				for (c = 0; c < 2; c++)
				{
					...
					for (i = 0; i < 4; i++)
					{
						...
						//av_log( s->avctx, AV_LOG_ERROR, "INTRA C%d-AC %d\n",c, index - 16 );
						decode_cabac_residual_nondc(h, h->mb + (16 * index << pixel_shift), 4, index, scan + 1, qmul,
						                            15);
					}
				}
			}
