                                   ...
                                   const AVFrame* pict, int* got_packet)
{
	...
	*p = *pict;
	...
	else
	{
		...
		{
			if (s->pb.buf_end - s->pb.buf - (put_bits_count(&s->pb) >> 3) < mb_width * 4 * 3 * s->mjpeg_hsample[0] * s->
				mjpeg_vsample[0])
			{
				av_log(s->avctx, AV_LOG_ERROR, "encoded frame too large\n");
				return -1;
					...
					for (i = 0; i < 3; i++)
					{
						...
						for (y = 0; y < v; y++)
						{
							for (x = 0; x < h; x++)
							{
								...
								if (i == 0)
									ff_mjpeg_encode_dc(s, *ptr - pred, m->huff_size_dc_luminance,
									                   m->huff_code_dc_luminance); //FIXME ugly
								else
									ff_mjpeg_encode_dc(s, *ptr - pred, m->huff_size_dc_chrominance,
