...
static void ff_h264_idct_add_paired(uint8_t* dst, DCTELEM* block, int stride)
{
	const float half = 0.5;
	vector
	float pair[2], subv[2], addv[2];
	vector
	float result, coeffs[8];
...
static void ff_h264_idct_add16_paired(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
                                      const uint8_t nnzc[48])
{
	for (int i = 0; i < 16; i++)
	{
...
static void ff_h264_idct_add16intra_paired(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
                                           const uint8_t nnzc[48])
{
	for (int i = 0; i < 16; i++)
	{
...
static void ff_h264_idct_add8_paired(uint8_t** dest, const int* block_offset, DCTELEM* block, int stride,
                                     const uint8_t nnzc[48])
{
	for (int i = 16; i < 24; i++)
	{
...
static void ff_h264_idct8_add4_paired(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
                                      const uint8_t nnzc[48])
{
