...
typedef WINAPI AVS_Value(* imp_avs_invoke)(AVS_ScriptEnvironment*, const char* name, AVS_Value args,
...
typedef WINAPI
void (*imp_avs_release_clip)(AVS_Clip*);
...
static demuxer_t* demux_open_avs(demuxer_t* demuxer)
{
	...
	if (avs_has_audio(AVS->video_info))
		...
		{
		case AVS_SAMPLE_INT8: audio_samplesize = 1;
			break;
		case AVS_SAMPLE_INT16: audio_samplesize = 2;
			break;
		case AVS_SAMPLE_INT24: audio_samplesize = 3;
			break;
		case AVS_SAMPLE_INT32:
		case AVS_SAMPLE_FLOAT: audio_samplesize = 4;
			break;
		default:
	...
	if (audio_samplesize)
	{
		...
		found = 1;
		mp_msg(MSGT_DEMUX, MSGL_V, "AVS: Clip has audio -> Channels = %d - Freq = %d\n", AVS->video_info->nchannels,
		       AVS->video_info->audio_samples_per_second);
...
static void demux_seek_avs(demuxer_t* demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	sh_audio_t* sh_audio = demuxer->audio->sh;
	AVS_T* AVS = demuxer->priv;
	double video_pos = sh_video ? (double)AVS->frameno / sh_video->fps : (double)AVS->sampleno / sh_audio->samplerate;
