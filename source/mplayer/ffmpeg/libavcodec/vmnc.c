...
int vmnc_get_pixel(const uint8_t* buf, int bpp, int be)
{
	switch (bpp * 2 + be)
	{
	case 2:
...
void paint_rect(uint8_t* dst, int dx, int dy, int w, int h, int color, int bpp, int stride)
{
	int i, j;
...
void paint_raw(uint8_t* dst, int w, int h, const uint8_t* src, int bpp, int be, int stride)
{
	int i, j, p;
	for (j = 0; j < h; j++)
	{
