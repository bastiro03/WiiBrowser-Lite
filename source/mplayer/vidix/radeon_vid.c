...
static uint32_t radeon_vid_get_dbpp(void)
{
	...
	switch (dbpp)
	{
	case DST_8BPP: retval = 8;
		break;
	case DST_15BPP: retval = 15;
		break;
	case DST_16BPP: retval = 16;
		break;
	case DST_24BPP: retval = 24;
		break;
	default: retval = 32;
		break;
	}
...
static void ComputeBorders(vidix_playback_t* config, int VertUVSubSample)
{
	...
	TopUVLine = ((int)(config->src.y / VertUVSubSample) < 0) ? 0 : (int)(config->src.y / VertUVSubSample);
	/* Round rSrcTop down */
	BottomUVLine = (ceil(((config->src.y + config->src.h) / VertUVSubSample)) > (config->src.h / VertUVSubSample))
		               ? (config->src.h / VertUVSubSample) - 1
		               : (unsigned int)ceil(((config->src.y + config->src.h) / VertUVSubSample)) - 1;
...
static int radeon_vid_init_video(vidix_playback_t* config)
{
	...
	horz_repl_factor = 1 << ((INPLL(VCLK_ECP_CNTL) & 0x300) >> 8);
	H_scale_ratio = ceil(((double)dest_w + 1) / horz_repl_factor) / src_w;
	...
	if (is_420 || is_410 || is_400)
	{
		...
		{
			besr.vid_buf_base_adrs_y[i] = ((radeon_overlay_off + config->offsets[i] + config->offset.y) &
			...
			else
			{
				...
				{
					besr.vid_buf_base_adrs_u[i] = ((radeon_overlay_off + config->offsets[i] + config->offset.v) &
						VIF_BUF1_BASE_ADRS_MASK) | VIF_BUF1_PITCH_SEL;
					besr.vid_buf_base_adrs_v[i] = ((radeon_overlay_off + config->offsets[i] + config->offset.u) &
						VIF_BUF2_BASE_ADRS_MASK) | VIF_BUF2_PITCH_SEL;
				}
				else
				{
					besr.vid_buf_base_adrs_v[i] = ((radeon_overlay_off + config->offsets[i] + config->offset.v) &
						VIF_BUF1_BASE_ADRS_MASK) | VIF_BUF1_PITCH_SEL;
					besr.vid_buf_base_adrs_u[i] = ((radeon_overlay_off + config->offsets[i] + config->offset.u) &
						VIF_BUF2_BASE_ADRS_MASK) | VIF_BUF2_PITCH_SEL;
				}
			}
		}
