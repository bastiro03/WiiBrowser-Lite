...
static inline void RENAME(rgb24tobgr32)(const uint8_t* src, uint8_t* dst, int src_size)
{
	...
	volatile __asm__ (SFENCE

	::
	:
	"memory"
	)
	volatile __asm__ (EMMS
                                       ...
                                       int lumStride, int chromStride, int srcStride)
{
	...
	for (y = 0; y < height - 2; y += 2)
	{
		...
		:
		:
		"r"(src + chromWidth * 6), "r"(src + srcStride + chromWidth * 6), "r"(udst + chromWidth), "r"(vdst + chromWidth)
			, "g"(-chromWidth)
		:
		"%"
		REG_a, "%"
...
static void RENAME(extract_even2avg)(const uint8_t* src0, const uint8_t* src1, uint8_t* dst0, uint8_t* dst1,
                                     x86_reg count)
{
	dst0 += count;
	dst1 += count;
...
static void RENAME(extract_odd2avg)(const uint8_t* src0, const uint8_t* src1, uint8_t* dst0, uint8_t* dst1,
                                    x86_reg count)
{
	dst0 += count;
	dst1 += count;
