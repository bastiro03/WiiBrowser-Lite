                     ...
                     int want_margins, int want_interlace, struct VesaCRTCInfoBlock* result)
{
	...
	if (type == GTF_PF)
	{
		ideal_H_period = ((c.c - 100) + (sqrt(
				((100 - c.c) * (100 - c.c)) + (0.4 * c.m * (total_active_pixels + margin_left + margin_right) / freq))))
			/ 2
			/ c.m * 1000;
	...
	else if (type == GTF_VF)
	{
		...
		estimated_H_period = ((1 / RR) - c.min_Vsync_BP / 1000000) / (Y + (2 * margin_top) + c.min_front_porch +
			interlace) * 1000000;
