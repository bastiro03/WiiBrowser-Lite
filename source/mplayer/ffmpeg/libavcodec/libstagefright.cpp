...
public:
	...
	{
		s = static_cast<StagefrightContext*>(avctx->priv_data);
		source_meta = meta;
...
{
	auto avctx = static_cast<AVCodecContext*>(arg);
	auto s = static_cast<StagefrightContext*>(avctx->priv_data);
	...
	{
		buffer = NULL;
		frame = static_cast<Frame*>(av_mallocz(sizeof(Frame)));
		if (!frame)
		{
			frame = s->end_frame;
		...
		if (frame->status == OK)
		{
			...
			frame->vframe = static_cast<AVFrame*>(av_mallocz(sizeof(AVFrame)));
			if (!frame->vframe)
			{
				frame->status = AVERROR(ENOMEM);
				decode_done = 1;
			...
			src_data[0] = static_cast<uint8_t*>(buffer->data());
...
static av_cold int Stagefright_init(AVCodecContext* avctx)
{
	...
	s->orig_extradata = static_cast<uint8_t*>(av_mallocz(avctx->extradata_size +
	...
	*s->source = new CustomSource(avctx, meta);
	...
	s->end_frame = static_cast<Frame*>(av_mallocz(sizeof(Frame)));
	if (s->source == nullptr || !s->in_queue || !s->out_queue || !s->client ||
                                    ...
                                    int* data_size, AVPacket* avpkt)
{
	...
	if (avpkt && avpkt->data)
	{
		av_bitstream_filter_filter(s->bsfc, avctx, nullptr, &pkt.data, &pkt.size,
	...
	if (!s->source_done)
	{
		if (!s->dummy_buf)
		{
			s->dummy_buf = static_cast<uint8_t*>(av_malloc(avpkt->size));
			if (!s->dummy_buf)
		...
		if (avpkt->data)
		{
			...
			frame->buffer = static_cast<uint8_t*>(av_malloc(avpkt->size));
			if (!frame->buffer)
			{
				av_freep(&frame);
				return AVERROR(ENOMEM);
	...
	*static_cast<AVFrame*>(data) = *ret_frame;
	return orig_size;
}
...
static av_cold int Stagefright_close(AVCodecContext* avctx)
{
	...
	if (s->thread_started)
	{
		if (!s->thread_exited)
		{
			...
			if (s->dummy_buf && (frame = static_cast<Frame*>(av_mallocz(sizeof(Frame)))))
			{
				frame->status = OK;
