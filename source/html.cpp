...
void AddButton(Lista::iterator lista, int type, GuiImage* image, void* arg)
{
	...
	{
	case ANCHOR:
		btn->label = new GuiText(static_cast<char*>(lista->value[*static_cast<int*>(arg)].text.c_str()), 20, (GXColor)
		{
			0, 0, 255, 255
		}
		)
		...
		while (!choice)
		{
			...
			else if (lista != l1.end() && Doc.YPos + Doc.Height < screenheight + 25)
			{
				...
				else
				{
					...
					{
						text = InsText(text);
						text->txt = new GuiText(static_cast<char*>(lista->value[i].text.c_str()), 20, (GXColor)
						{
							0, 0, 0, 255
						}
						)
