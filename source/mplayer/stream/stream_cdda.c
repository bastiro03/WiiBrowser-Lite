...
static int open_cdda(stream_t* st, int m, void* opts, int* file_format)
{
	...
	for (i = 0; i < cdd->tracks; i++)
	{
		...
		cd_info_add_track(cd_info, track_name, i + 1, (unsigned int)(off / (60 * 75)), (unsigned int)((off / 75) % 60),
		                  (unsigned int)(off % 75), sec, off);
		audiolen += off;
	}
	cd_info->min = audiolen / (60 * 75);
	cd_info->sec = audiolen / 75 % 60;
	cd_info->msec = audiolen % 75;
