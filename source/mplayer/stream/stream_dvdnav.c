...
typedef enum
{
	...
	NAV_FLAG_WAIT = 1 << 1,
	/* wait event */
	NAV_FLAG_WAIT_SKIP = 1 << 2,
	/* wait skip disable */
	NAV_FLAG_CELL_CHANGE = 1 << 3,
	/* cell change event */
	NAV_FLAG_WAIT_READ_AUTO = 1 << 4,
	/* wait read auto mode */
	NAV_FLAG_WAIT_READ = 1 << 5,
	/* suspend read from stream */
	NAV_FLAG_VTS_DOMAIN = 1 << 6,
	/* vts domain */
	NAV_FLAG_SPU_SET = 1 << 7,
	/* spu_clut is valid */
	NAV_FLAG_STREAM_CHANGE = 1 << 8,
	/* title, chapter, audio or SPU */
	NAV_FLAG_AUDIO_CHANGE = 1 << 9,
	/* audio stream change event */
	NAV_FLAG_SPU_CHANGE = 1 << 10,
	/* spu stream change event */
} dvdnav_state_t;
	...
	*len = -1;
	...
	else if (event != DVDNAV_BLOCK_OK)
	{
		...
		case DVDNAV_STILL_FRAME:
			{
				dvdnav_still_event_t* still_event = buf;
		...
		case DVDNAV_CELL_CHANGE:
			{
				dvdnav_cell_change_event_t* ev = buf;
				uint32_t nextstill;
...
static int fill_buffer(stream_t* s, char* but, int len)
{
	...
	while (!len) /* grab all event until DVDNAV_BLOCK_OK (len=2048), DVDNAV_STOP or DVDNAV_STILL_FRAME */
	{
		...
		case DVDNAV_VTS_CHANGE:
			{
				int tit = 0, part = 0;
				dvdnav_vts_change_event_t* vts_event = s->buffer;
		...
		case DVDNAV_CELL_CHANGE:
			{
				...
				{
					int tit = 0, part = 0;
					if (dvdnav_current_title_info(priv->dvdnav, &tit, &part) == DVDNAV_STATUS_OK && part >
						dvd_last_chapter)
					{
						priv->state |= NAV_FLAG_EOF;
						return 0;
