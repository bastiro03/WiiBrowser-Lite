...
namespace htmlcxx
{
	namespace CSS
	{
		...
		public:
			...
			public:
				Selector();
				Selector(const std::string& e, const std::string& i, const std::string& c, const PseudoClass& pc,
		...
		public:
			...
			friend std::ostream& operator<<(std::ostream& out, const std::map<std::string, Attribute>& s);
		...
		private:
			using RuleSet = std::map<std::vector<Selector>, std::map<std::string, Attribute>>;
			RuleSet mRuleSets;
		};
