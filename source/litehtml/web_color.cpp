...
litehtml::web_color litehtml::web_color::from_string(const wchar_t* str)
{
	...
	if (str[0] == L'#')
	{
		...
		wchar_t* sss = nullptr;
		web_color clr;
		clr.red = static_cast<byte>(wcstol(red.c_str(), &sss, 16));
		clr.green = static_cast<byte>(wcstol(green.c_str(), &sss, 16));
		clr.blue = static_cast<byte>(wcstol(blue.c_str(), &sss, 16));
		return clr;
	}
	if (!wcsncmp(str, L"rgb", 3))
	{
		std::wstring s = str;
		...
		if (tokens.size() >= 1) clr.red = static_cast<byte>(_wtoi(tokens[0].c_str()));
		if (tokens.size() >= 2) clr.green = static_cast<byte>(_wtoi(tokens[1].c_str()));
		if (tokens.size() >= 3) clr.blue = static_cast<byte>(_wtoi(tokens[2].c_str()));
		if (tokens.size() >= 4) clr.alpha = static_cast<byte>(_wtoi(tokens[3].c_str()));
...
const wchar_t* litehtml::web_color::resolve_name(const wchar_t* name)
{
	...
	{
		if (!wcscasecmp(name, g_def_colors[i].name))
		{
