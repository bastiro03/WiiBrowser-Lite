...
int mp_decode_init(AVCodecContext* avctx)
{
                           ...
                           AVPacket* avpkt)
{
	...
	if (mp->max_codes_bits <= 0)
		goto end;
	if (init_vlc(&mp->vlc, mp->max_codes_bits, mp->codes_count, &mp->codes[0].size, sizeof(HuffCode), 1,
	             &mp->codes[0].code, sizeof(HuffCode), 4, 0))
		goto end;
...
int mp_decode_end(AVCodecContext* avctx)
{
	MotionPixelsContext* mp = avctx->priv_data;
