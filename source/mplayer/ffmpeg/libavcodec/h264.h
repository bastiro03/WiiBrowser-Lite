...
#define PART_NOT_AVAILABLE -2
	...
	,
	mb
	)[16 * 48 * 2]; ///< as a dct coeffecient is int32_t in high depth, we need to reserve twice the space.
	DECLARE_ALIGNED (
	16
	,
	DCTELEM
...
void ff_h264_direct_dist_scale_factor(H264Context* h);
void ff_h264_direct_ref_list_init(H264Context* h);
void ff_h264_pred_direct_motion(H264Context* h, int* mb_type);
...
int get_chroma_qp(H264Context* h, int t, int qscale)
{
...
void write_back_motion_list(H264Context* h,
                            ...
                            int mb_type, int list)
{
	...
	if (CABAC)
	{
		uint8_t (*mvd_dst)[2] = &h->mvd_table[list][FMO
			                                            ? 8 * h->mb_xy
			                                            : h->mb2br_xy[h->mb_xy]];
...
int get_dct8x8_allowed(H264Context* h)
{
