...
static void scale(struct SwsContext* sws1, struct SwsContext* sws2, uint8_t* src[MP_MAX_PLANES],
                  int src_stride[MP_MAX_PLANES],
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (!(mpi->flags & MP_IMGFLAG_DRAW_CALLBACK && dmpi))
	{
		...
		scale(vf->priv->ctx, vf->priv->ctx, mpi->planes, mpi->stride, 0, mpi->h, dmpi->planes, dmpi->stride,
		      vf->priv->interlaced);
	}
...
static int control(struct vf_instance* vf, int request, void* data)
{
	...
	if (vf->priv->ctx)
		...
		case VFCTRL_GET_EQUALIZER:
			r = sws_getColorspaceDetails(vf->priv->ctx, &inv_table, &srcRange, &table, &dstRange, &brightness,
			                             &contrast, &saturation);
			if (r < 0) break;
		...
		//printf("set %f %f %f\n", brightness/(float)(1<<16), contrast/(float)(1<<16), saturation/(float)(1<<16));
			...
			r = sws_setColorspaceDetails(vf->priv->ctx, inv_table, srcRange, table, dstRange, brightness, contrast,
			                             saturation);
			if (r < 0) break;
			if (vf->priv->ctx2)
			{
				r = sws_setColorspaceDetails(vf->priv->ctx2, inv_table, srcRange, table, dstRange, brightness, contrast,
				                             saturation);
				if (r < 0) break;
			}
