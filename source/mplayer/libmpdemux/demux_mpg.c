...
static demuxer_t* demux_mpg_open(demuxer_t* demuxer)
{
	...
	if (mpg_d)
	{
		...
		)
		{
			...
			if (first_pts != -1.0)
			{
				float middle_pts = read_first_mpeg_pts_at_position(
					demuxer, (demuxer->movi_end + demuxer->movi_start) / 2);
				if (middle_pts != -1.0)
				{
					...
					if (final_pts != -1.0)
					{
						...
						float proportion = (middle_pts - first_pts == 0)
							                   ? -1
							                   : (final_pts - middle_pts) / (middle_pts - first_pts);
...
static void new_audio_stream(demuxer_t* demux, int aid)
{
	if (!demux->a_streams[aid])
	{
		...
		case 0x00: sh_a->format = 0x50;
			break; // mpeg
		case 0xA0: sh_a->format = 0x10001;
			break; // dvd pcm
...
static int demux_mpg_read_packet(demuxer_t* demux, int id)
{
	...
	else if ((c >> 6) == 2)
	{
		...
		if (parse_ext2 && hdrlen >= 3)
		{
			...
			if (c != 0x81)
			{
				mp_msg(MSGT_DEMUX, MSGL_V, "demux_mpg: unknown pes_extension2 format, len is > 1  \n");
				return -1;
			}
			...
			if (c < 0x55 || c > 0x5F)
			{
				mp_msg(MSGT_DEMUX, MSGL_V, "demux_mpg: unknown vc1 substream_id: 0x%x  \n", c);
				return -1;
			}
			pes_ext2_subid = c;
		}
		...
		if (id == 0x1FD && pes_ext2_subid != -1)
		{
			...
			if (!demux->v_streams[pes_ext2_subid])
				new_sh_video(demux, pes_ext2_subid);
	...
	if (ds)
	{
		...
		/*
		  ...
		  back and forth
		*/
		if (ds == demux->video && stream_control(demux->stream, STREAM_CTRL_GET_CURRENT_TIME, (void*)&stream_pts) !=
			STREAM_UNSUPPORTED)
