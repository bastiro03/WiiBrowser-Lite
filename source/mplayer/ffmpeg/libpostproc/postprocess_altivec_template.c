...
static inline int vertClassify_altivec(uint8_t src[], int stride, PPContext* c)
{
	...
	*/
	...
	vector
	signed short v_data = vec_ld(0, data);
	vector
...
#undef ITER
	...
	{
		const vector unsigned char mmoP1 = (const vector
unsigned char
		)
		{
		...
		const vector unsigned char mmoP2 = (const vector
unsigned char
		)
		{
		...
		const vector unsigned char mmoP = (const vector
unsigned char
		)
...
static inline void doVertLowPass_altivec(uint8_t* src, int stride, PPContext* c)
{
	...
	*/
	...
	vector
	unsigned char vbA0, av_uninit(vbA1), av_uninit(vbA2), av_uninit(vbA3), av_uninit(vbA4), av_uninit(vbA5),
	              av_uninit(vbA6), av_uninit(vbA7), av_uninit(vbA8), vbA9;
	vector
	unsigned char vbB0, av_uninit(vbB1), av_uninit(vbB2), av_uninit(vbB3), av_uninit(vbB4), av_uninit(vbB5),
	              av_uninit(vbB6), av_uninit(vbB7), av_uninit(vbB8), vbB9;
	vector
...
unsigned short
		...
		vec_cmplt(vec_abs(v_diff01), vqp);
...
unsigned short
		...
		vec_cmplt(vec_abs(v_diff89), vqp);
...
#define COMPUTE_VR(i, j, k)                                             \
		...
		const vector unsigned char permHH = (const vector
unsigned char
		)
		{
...
static inline void doVertDefFilter_altivec(uint8_t src[], int stride, PPContext* c)
{
	...
	*/
	...
	signed short vqp = vec_splat(
		             (vector signed short)vec_ld(0, qp), 
	0
	)
...
static inline void dering_altivec(uint8_t src[], int stride, PPContext* c)
{
	...
	const vector unsigned char permA1 = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char permA2 = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char permA1inc = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char permA2inc = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char magic = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char extractPerm = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char extractPermInc = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char tenRight = (vector
 unsigned char
	)
	{
	...
	const vector unsigned char eightLeft = (vector
 unsigned char
	)
	{
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
#undef LOAD_LINE
	...
	{
		const vector unsigned char trunc_perm = (vector
 unsigned char
		)
		{
...
#undef EXTRACT
	...
	{
		const vector unsigned short mask1 = (vector
 unsigned short
		)
		{
		...
		const vector unsigned short mask2 = (vector
 unsigned short
		)
		{
...
unsigned char
	...
	vector
	signed int v_dp = zero;
	vector
	signed int v_sysdp = zero;
	int d, sysd, i;
...
#undef LOAD_DOUBLE_LINE
	...
	vector
	unsigned char tempA = vec_mergeh(src0, zero);
	vector
	unsigned char tempB = vec_mergel(src0, zero);
	vector
	unsigned char tempC = vec_mergeh(src1, zero);
	vector
	unsigned char tempD = vec_mergel(src1, zero);
	vector
...
#undef LOAD_DOUBLE_LINE
	...
	vector
	unsigned char tempA = vec_mergeh(src0, src8);
	vector
	unsigned char tempB;
	vector
	unsigned char tempC = vec_mergeh(src1, src9);
	vector
	unsigned char tempD;
	vector
