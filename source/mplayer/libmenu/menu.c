...
typedef void (*draw_alpha_f)(int w, int h, unsigned char* src, unsigned char* srca, int srcstride,
...
void menu_draw_text(mp_image_t* mpi, char* txt, int x, int y)
{
	...
	{
		int c = utf8_get_char((const char**)&txt);
		if ((font = vo_font->font[c]) >= 0 && (x + vo_font->width[c] <= mpi->w) && (y + vo_font->pic_a[font]->h <= mpi->
			h))
	...
	while (sy < ymax && *txt)
	{
		...
		if (align & (MENU_TEXT_HCENTER | MENU_TEXT_RIGHT))
		{
			...
			if (ll > xmax - xmin)
			{
				...
				{
					// We are before the start point -> go forward
