                              ...
                              unsigned crop_w, unsigned crop_h)
{
	...
	for (i = 0; i < 4; ++i)
	{
		...
		if (this->custom && (this->cuspal[i] >> 31) != 0)
			alpha = 0;
		color = this->custom ? this->cuspal[i] : this->global_palette[this->palette[i]];
		color = (color >> 16) & 0xff;
...
void spudec_draw(void* this, void (*draw_alpha)(int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca,
                                                int stride))
{
	spudec_handle_t* spu = this;
...
static void scale_table(unsigned int start_src, unsigned int start_tar, unsigned int end_src, unsigned int end_tar,
                        scale_pixel* table)
{
	unsigned int t;
...
static void scale_image(int x, int y, scale_pixel* table_x, scale_pixel* table_y, spudec_handle_t* spu)
{
	...
	spu->scaled_image[scaled] = (color[0] * scale[0] + color[1] * scale[1] + color[2] * scale[2] + color[3] * scale[3])
		>> 24;
							...
							for (y = 0; y < spu->scaled_height; ++y)
							{
								...
								for (x = 0; x < spu->scaled_width; ++x)
								{
									...
									spu->scaled_aimage[base] =
										alpha * (1 + unscaled_bottom - unscaled_top) * (1 + unscaled_right -
											unscaled_left);
									/* spu->scaled_aimage[base] =
									...
									*/
									...
									if (width > 0 && height > 0)
									{
										...
										for (walky = top_low_row; walky < (unsigned int)unscaled_y_bottom; ++walky)
										{
											...
											for (walkx = left_right_column; walkx < (unsigned int)unscaled_x_right; ++
											     walkx)
											{
									...
									{
										base = spu->stride * (unsigned int)unscaled_y_bottom + (unsigned int)
											unscaled_x_right;
...
void* spudec_new_scaled(unsigned int* palette, unsigned int frame_width, unsigned int frame_height, uint8_t* extradata,
                        int extradata_len)
{
