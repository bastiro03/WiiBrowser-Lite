...
int tdfx_map_io = 1;
static __initdata
unsigned long map_start = 0; //0x7300000;
static __initdata
unsigned long map_max = (10 * 1024 * 1024);

MODULE_PARM(tdfx_map_io, "i");
MODULE_PARM_DESC(tdfx_map_io,
...
static int tdfx_vid_set_overlay(unsigned long arg)
{
	...
	screen_w &= 0xFFF;
	disp_w = ov.dst_x + ov.dst_width >= screen_w ? screen_w - ov.dst_x : ov.dst_width;
	disp_h = ov.dst_y + ov.dst_height >= screen_h ? screen_h - ov.dst_y : ov.dst_height;
