...
{
	int ret = VFCAP_CSP_SUPPORTED | VFCAP_CSP_SUPPORTED_BY_HW | VFCAP_OSD;
                  ...
                  uint32_t format)
{
	...
	if (params.scale)
	{
		...
		ret = layer->SetScreenLocation(layer, (1 - (float)out_width / 10000) / 2, (1 - (float)out_height / 10000) / 2,
		                               ((float)out_width / 10000), ((float)out_height / 10000));
		...
		if (buffer->GetEvent(buffer, DFB_EVENT(&event)) == DFB_OK)
		{
			if (event.type == DIET_KEYPRESS)
			{
				...
				case DIKS_ESCAPE:
					...
					break;
				case DIKS_PAGE_UP: mplayer_put_key(KEY_PAGE_UP);
					break;
				case DIKS_PAGE_DOWN: mplayer_put_key(KEY_PAGE_DOWN);
					break;
				case DIKS_CURSOR_UP: mplayer_put_key(KEY_UP);
					break;
				case DIKS_CURSOR_DOWN: mplayer_put_key(KEY_DOWN);
					break;
				case DIKS_CURSOR_LEFT: mplayer_put_key(KEY_LEFT);
					break;
				case DIKS_CURSOR_RIGHT: mplayer_put_key(KEY_RIGHT);
					break;
				case DIKS_INSERT: mplayer_put_key(KEY_INSERT);
					break;
				case DIKS_DELETE: mplayer_put_key(KEY_DELETE);
					break;
				case DIKS_HOME: mplayer_put_key(KEY_HOME);
					break;
				case DIKS_END: mplayer_put_key(KEY_END);
					break;
	...
	else
	{
		...
		*/
		...
		if (frame)
		{
			...
			framelocked = 1;
			mem2agpcpy_pic(dst, mpi->planes[0] + mpi->y * mpi->stride[0] + mpi->x * (mpi->bpp >> 3),
			               mpi->w * (mpi->bpp >> 3), mpi->h, pitch, mpi->stride[0]);
		}
		else
		{
			DFBCHECK(primary->Lock(primary, DSLF_WRITE, (void*)&dst, &pitch));
			...
			mem2agpcpy_pic(dst + yoffset * pitch + xoffset * (mpi->bpp >> 3),
			               mpi->planes[0] + mpi->y * mpi->stride[0] + mpi->x * (mpi->bpp >> 3),
			               mpi->w * (mpi->bpp >> 3), mpi->h, pitch, mpi->stride[0]);
		}
		unlock();
	}
