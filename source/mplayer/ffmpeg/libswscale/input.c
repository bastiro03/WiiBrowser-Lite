...
static void abgrToA_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, int width,
                      uint32_t* unused)
{
	int i;
	for (i = 0; i < width; i++)
...
static void rgbaToA_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, int width,
                      uint32_t* unused)
{
	int i;
	for (i = 0; i < width; i++)
...
static void palToA_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, int width,
                     uint32_t* pal)
{
	int i;
	for (i = 0; i < width; i++)
...
static void palToY_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, long width,
                     uint32_t* pal)
{
	int i;
	for (i = 0; i < width; i++)
...
static void monowhite2Y_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, int width,
                          uint32_t* unused)
{
	int i, j;
	width = (width + 7) >> 3;
...
static void monoblack2Y_c(int16_t* dst, const uint8_t* src, const uint8_t* unused1, const uint8_t* unused2, int width,
                          uint32_t* unused)
{
	int i, j;
	width = (width + 7) >> 3;
                       ...
                       int width, uint32_t* unused)
{
	...
	for (i = 0; i < width; i++)
	{
		...
		dst[i] = ((RY * r + GY * g + BY * b + (32 << (RGB2YUV_SHIFT - 1)) + (1 << (RGB2YUV_SHIFT - 7))) >> (
			RGB2YUV_SHIFT - 6));
	}
}
                        ...
                        const uint8_t* src2, int width, uint32_t* unused)
{
	...
	for (i = 0; i < width; i++)
	{
		...
		dstU[i] = (RU * r + GU * g + BU * b + (256 << (RGB2YUV_SHIFT - 1)) + (1 << (RGB2YUV_SHIFT - 7))) >> (
			RGB2YUV_SHIFT - 6);
		dstV[i] = (RV * r + GV * g + BV * b + (256 << (RGB2YUV_SHIFT - 1)) + (1 << (RGB2YUV_SHIFT - 7))) >> (
			RGB2YUV_SHIFT - 6);
	}
	assert(src1 == src2);
}
                             ...
                             const uint8_t* src2, int width, uint32_t* unused)
{
	...
	for (i = 0; i < width; i++)
	{
		...
		dstU[i] = (RU * r + GU * g + BU * b + (256 << RGB2YUV_SHIFT) + (1 << (RGB2YUV_SHIFT - 6))) >> (RGB2YUV_SHIFT -
			5);
		dstV[i] = (RV * r + GV * g + BV * b + (256 << RGB2YUV_SHIFT) + (1 << (RGB2YUV_SHIFT - 6))) >> (RGB2YUV_SHIFT -
			5);
	}
	assert(src1 == src2);
}
...
void ff_sws_init_input_funcs(SwsContext* c)
{
	...
	if (c->alpPixBuf)
	{
		...
		{
		case PIX_FMT_RGBA64LE:
		case PIX_FMT_RGBA64BE: c->alpToYV12 = rgba64ToA_c;
			break;
		case PIX_FMT_BGRA:
		case PIX_FMT_RGBA:
