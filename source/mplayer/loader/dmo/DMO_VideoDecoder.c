...
DMO_VideoDecoder* DMO_VideoDecoder_Open(char* dllname, GUID* guid, BITMAPINFOHEADER* format, int flip, int maxauto)
{
	...
	/*try*/
	{
		...
		if (!flip)
		{
			...
			result = this->m_pDMO_Filter->m_pMedia->vt->SetOutputType(this->m_pDMO_Filter->m_pMedia, 0,
			                                                          &this->m_sDestType, DMO_SET_TYPEF_TEST_ONLY);
			if (result)
			{
	...
	//	case fccIV50:
		...
		default:
			...
			for (c = check; c->bits; c++)
			{
				...
				//result = this->m_pDMO_Filter->m_pOutputPin->vt->QueryAccept(this->m_pDMO_Filter->m_pOutputPin, &this->m_sDestType);
				result = this->m_pDMO_Filter->m_pMedia->vt->SetOutputType(
					this->m_pDMO_Filter->m_pMedia, 0, &this->m_sDestType, DMO_SET_TYPEF_TEST_ONLY);
				if (!result)
				{
...
int DMO_VideoDecoder_DecodeInternal(DMO_VideoDecoder* this, const void* src, int size, int is_keyframe, char* imdata)
{
	...
	db.pBuffer->vt->Release((IUnknown*)db.pBuffer);
...
*/
	...
	result = this->m_pDMO_Filter->m_pMedia->vt->SetOutputType(this->m_pDMO_Filter->m_pMedia, 0, &this->m_sDestType,
