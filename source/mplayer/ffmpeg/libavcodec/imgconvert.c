                             ...
                             int has_alpha)
{
	...
	if (dst_pix_fmt == PIX_FMT_PAL8 &&
		(src_pix_fmt != PIX_FMT_PAL8 && (ps->color_type != FF_COLOR_GRAY || (pixdesc_has_alpha(src_desc) &&
			has_alpha))))
...
static int avg_bits_per_pixel(enum PixelFormat pix_fmt)
{
	...
	return info->padded_size ? info->padded_size : av_get_bits_per_pixel(desc);
}
                                            ...
                                            enum PixelFormat src_pix_fmt, int has_alpha, int* loss_ptr)
{
	...
	for (i = 0; loss_mask_order[i] != 0 && dst_pix_fmt == PIX_FMT_NONE; i++)
	{
		...
		if (loss_order1 == 0 && loss_order2 == 0)
		{
			/* use format with smallest depth */
			dst_pix_fmt = avg_bits_per_pixel(dst_pix_fmt2) < avg_bits_per_pixel(dst_pix_fmt1)
				              ? dst_pix_fmt2
				              : dst_pix_fmt1;
		}
		else if (loss_order1 == 0 || loss_order2 == 0)
		{
			/* use format with no loss */
