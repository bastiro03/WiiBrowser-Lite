...
static dvb_channels_list* dvb_get_channels(char* filename, int type)
{
	...
	char tmp_lcr[256], tmp_hier[256], inv[256], bw[256], cr[256], mod[256], transm[256], gi[256], vpid_str[256],
	     apid_str[256];
	...
	while (!feof(f))
	{
		...
		{
			cnt = ptr->pids_cnt;
			pcnt = sscanf(apid_str, "%d+%d+%d+%d+%d+%d+%d+%d", &ptr->pids[cnt], &ptr->pids[cnt + 1],
			              &ptr->pids[cnt + 2],
			              &ptr->pids[cnt + 3], &ptr->pids[cnt + 4], &ptr->pids[cnt + 5], &ptr->pids[cnt + 6],
			              &ptr->pids[cnt + 7]);
			if (pcnt > 0)
			{
				ptr->pids_cnt += pcnt;
...
int dvb_set_channel(stream_t* stream, int card, int n)
{
	...
	if (priv->is_on) //the fds are already open and we have to stop the demuxers
	{
		...
		//close all demux_fds with pos > pids required for the new channel or open other demux_fds if we have too few
		{
	...
	if (channel->freq != priv->last_freq)
		if (!dvb_tune(priv, channel->freq, channel->pol, channel->srate, channel->diseqc, channel->tone,
		              channel->inv, channel->mod, channel->gi, channel->trans, channel->bw, channel->cr, channel->cr_lp,
		              channel->hier, priv->timeout))
			return 0;
...
dvb_config_t* dvb_get_config(void)
{
	...
	for (i = 0; i < MAX_CARDS; i++)
	{
		...
		snprintf(name, 20, "DVB-%c card n. %d", type == TUNER_TER ? 'T' : (type == TUNER_CBL ? 'C' : 'S'),
		         conf->count + 1);
