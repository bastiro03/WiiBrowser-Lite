...
static void *DownloadSingleImage (void *arg)
{
    CURL *curl_upd = curl_easy_init();
    Argument Arg = *(Argument *)arg;
    struct block THREAD = downloadfile(curl_upd, Arg.url, NULL);

    if(THREAD.size>0 && strstr(THREAD.type, "image"))
    {
        Arg.lista->imgdata = new GuiImageData ((u8*)THREAD.data, THREAD.size);
        Arg.lista->img->SetImage(Arg.lista->imgdata);
        Arg.lista->img->SetScale(screenwidth-80, atoi(Arg.lista->tag->attribute.c_str()));
        Arg.lista->img->SetEffect(EFFECT_FADE, 50);
    }
        ...
        else
        {
            ...
            for (lista = img; !NoImg(lista); lista = lista->prox)
            {
                ...
                if (!lista->fetched && !lista->img->GetImage() && lista->tag)
                {
                    ...
                    Arg.lista = lista;
                    Arg.url = strdup(tmp.c_str());
                    for(int i = 0; i<THR; i++)
                    {
                        if(LWP_ThreadIsSuspended(threads[i]))
                        {
                            LWP_ResumeThread(threads[i]);
                            LWP_JoinThread(threads[i], NULL);
                            threads[i] = LWP_THREAD_NULL;
                        }
                        if(threads[i] == LWP_THREAD_NULL)
                        {
                            LWP_CreateThread (&threads[i], DownloadSingleImage, (void*)&Arg, NULL, 0, 60);
                            usleep(100);
                            lista->fetched = true;
                            break;
                        }
