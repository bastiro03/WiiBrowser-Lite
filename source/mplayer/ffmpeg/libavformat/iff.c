...
static int iff_probe(AVProbeData* p)
{
	...
	if (AV_RL32(d) == ID_FORM &&
		(AV_RL32(d + 8) == ID_8SVX || AV_RL32(d + 8) == ID_PBM || AV_RL32(d + 8) == ID_ACBM || AV_RL32(d + 8) == ID_DEEP
			|| AV_RL32(d + 8) == ID_ILBM))
...
static int iff_read_header(AVFormatContext* s)
{
	...
	while (!url_feof(pb))
	{
		...
		case ID_ANNO:
		case ID_TEXT: metadata_tag = "comment";
			break;
		case ID_AUTH: metadata_tag = "artist";
			break;
		case ID_COPYRIGHT: metadata_tag = "copyright";
			break;
		case ID_NAME: metadata_tag = "title";
			break;
		}
