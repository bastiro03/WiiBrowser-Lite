...
using mp_cmd_arg_value_t = union mp_cmd_arg_value
{
	int i;
	float f;
	char* s;
...
using mp_cmd_arg_t = struct mp_cmd_arg
{
	int type;
	mp_cmd_arg_value_t v;
};

using mp_cmd_t = struct mp_cmd
{
	int id;
	char* name;
	int nargs;
...
using mp_cmd_bind_t = struct mp_cmd_bind
{
	int input[MP_MAX_KEY_DOWN + 1];
	char* cmd;
};

using mp_key_name_t = struct mp_key_name
{
	int key;
	char* name;
};
