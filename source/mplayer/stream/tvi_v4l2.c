...
static int control(priv_t* priv, int cmd, void* arg)
{
	...
	case TVI_CONTROL_IS_VIDEO:
		return priv->capability.capabilities & V4L2_CAP_VIDEO_CAPTURE ? TVI_CONTROL_TRUE : TVI_CONTROL_FALSE;
	case TVI_CONTROL_IS_AUDIO:
		if (priv->tv_param->force_audio) return TVI_CONTROL_TRUE;
	case TVI_CONTROL_IS_TUNER:
		return priv->capability.capabilities & V4L2_CAP_TUNER ? TVI_CONTROL_TRUE : TVI_CONTROL_FALSE;
...
static int uninit(priv_t* priv)
{
	...
	priv->map = NULL;
	priv->mapcount = 0;
	if (priv->video_fd != -1)close(priv->video_fd);
	priv->video_fd = -1;
	free(priv->video_dev);
	priv->video_dev = NULL;
...
static int init(priv_t* priv)
{
	...
	*/
	...
	mp_msg(MSGT_TV, MSGL_INFO, " Capabilities:%s%s%s%s%s%s%s%s%s%s%s\n",
	       priv->capability.capabilities & V4L2_CAP_VIDEO_CAPTURE ? "  video capture" : "",
	       priv->capability.capabilities & V4L2_CAP_VIDEO_OUTPUT ? "  video output" : "",
	       priv->capability.capabilities & V4L2_CAP_VIDEO_OVERLAY ? "  video overlay" : "",
	       priv->capability.capabilities & V4L2_CAP_VBI_CAPTURE ? "  VBI capture device" : "",
	       priv->capability.capabilities & V4L2_CAP_VBI_OUTPUT ? "  VBI output" : "",
	       priv->capability.capabilities & V4L2_CAP_RDS_CAPTURE ? "  RDS data capture" : "",
	       priv->capability.capabilities & V4L2_CAP_TUNER ? "  tuner" : "",
	       priv->capability.capabilities & V4L2_CAP_AUDIO ? "  audio" : "",
	       priv->capability.capabilities & V4L2_CAP_READWRITE ? "  read/write" : "",
	       priv->capability.capabilities & V4L2_CAP_ASYNCIO ? "  async i/o" : "",
	       priv->capability.capabilities & V4L2_CAP_STREAMING ? "  streaming" : "");
...
static void* video_grabber(void* data)
{
	...
	for (priv->frames = 0; !priv->shutdown;)
	{
		...
		if (!FD_ISSET(priv->video_fd, &rdset))
		{
			continue;
		}
		...
		if (ret < 0)
		{
			...
			for (i = 0; i < priv->mapcount; i++)
			{
				...
				if ((buf.flags & (V4L2_BUF_FLAG_QUEUED | V4L2_BUF_FLAG_MAPPED | V4L2_BUF_FLAG_DONE)) ==
		...
		mp_msg(MSGT_TV, MSGL_DBG3, "\nfps = %f, interval = %f, a_skew = %f, corr_skew = %f\n",
		       delta ? 1e6 / delta : -1,
		       1e-6 * interval, 1e-6 * xskew, 1e-6 * skew);
		...
		if (priv->video_buffer_size_current < priv->video_buffer_size_max)
		{
			if (priv->video_cnt == priv->video_buffer_size_current)
			{
				...
				if (newbuf)
				{
					memmove(priv->video_ringbuffer + priv->video_tail + 1,
					        priv->video_ringbuffer + priv->video_tail,
...
static void* audio_grabber(void* data)
{
	...
	for (; !priv->shutdown;)
	{
...
#else
		if (audio_in_read_chunk(&priv->audio_in,
		                        priv->audio_ringbuffer + priv->audio_tail * priv->audio_in.blocksize) < 0)
			continue;
#endif
		...
		{
			start_time_avg += (long long)1e6 * tv.tv_sec + tv.tv_usec - priv->audio_usecs_per_block * priv->
		...
		{
			priv->audio_skew_factor = (double)(priv->audio_skew - prev_skew) / (current_time - priv->
