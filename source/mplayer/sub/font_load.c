...
font_desc_t* read_font_desc(const char* fname, float factor, int verbose)
{
	...
	if (!f)
	{
		mp_msg(MSGT_OSD, MSGL_V, "font: can't open file: %s\n", fname);
		goto fail_out;
	}
	...
	while (fgets(sor, 1020, f))
	{
		...
		if (first)
		{
			if (!sor[0] || sor[1] == 1 || (sor[0] == 'M' && sor[1] == 'Z') || (sor[0] == 0x1f && sor[1] == 0x8b) || (sor
				[0] == 1 && sor[1] == 0x66))
			{
		...
		if (pdb == 1 && p[0][0] == '[')
		{
			...
			if (len && len < 63 && p[0][len - 1] == ']')
			{
				...
				if (strcmp(section, "[files]") == 0)
				{
					...
					if (fontdb >= 16)
					{
						mp_msg(MSGT_OSD, MSGL_ERR, "font: Too many bitmaps defined.\n");
						goto fail_out;
					}
				}
				continue;
...
#define FONT_PATH_SEP "/"
...
#else
				char* default_dir = MPLAYER_DATADIR
				FONT_PATH_SEP "font";
#endif
	...
	for (i = 0; i <= fontdb; i++)
	{
		...
		//if(factor!=1.0f)
		{
			...
			int j;
			mp_msg(MSGT_OSD, MSGL_DBG2, "font: resampling alpha by factor %5.3f (%d) ", factor, f);
			fflush(stdout);
			for (j = 0; j < size; j++)
			{
