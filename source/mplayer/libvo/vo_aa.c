...
getcolor(char* s)
{
	...
	if (!strcasecmp(s, "dim")) return AA_DIM;
	if (!strcasecmp(s, "bold")) return AA_BOLD;
	if (!strcasecmp(s, "boldfont")) return AA_BOLDFONT;
	if (!strcasecmp(s, "special")) return AA_SPECIAL;
	return -1;
}
