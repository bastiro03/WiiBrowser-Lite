...
static inline void copy_frame(priv_t* priv, unsigned char* dest, unsigned char* source)
{
	...
	if (priv->format == IMGFMT_YV12)
	{
		...
		fast_memcpy(dest + priv->width * priv->height * 5 / 4, source + priv->width * priv->height,
		            priv->width * priv->height / 4);
		fast_memcpy(dest + priv->width * priv->height, source + priv->width * priv->height * 5 / 4,
		            priv->width * priv->height / 4);
		return;
	}
	...
	for (framecount = 0; !priv->shutdown;)
	{
		for (i = 0; i < priv->nbuf && !priv->shutdown; i++, framecount++)
		{
			...
			mp_msg(MSGT_TV, MSGL_DBG3, "\nfps = %f, interval = %f, a_skew = %f, corr_skew = %f\n",
			       (interval != prev_interval) ? 1e6 / (interval - prev_interval) : -1,
			...
			if (priv->video_buffer_size_current < priv->video_buffer_size_max)
			{
				if (priv->video_cnt == priv->video_buffer_size_current)
				{
					...
					if (newbuf)
					{
						...
						memmove(priv->video_timebuffer + priv->video_tail + 1,
...
static void* audio_grabber(void* data)
{
	...
	for (; !priv->shutdown;)
	{
		...
		if (priv->audio_recv_blocks_total > priv->aud_skew_cnt)
		{
			...
			priv->audio_skew += (priv->audio_skew * priv->aud_skew_cnt) / (2 * priv->audio_recv_blocks_total - priv->
				aud_skew_cnt);
		}
		else
		{
			// this smoothes the evolution of audio_skew at startup a bit
			priv->audio_skew = ((priv->aud_skew_cnt + priv->audio_recv_blocks_total) * priv->audio_skew_total) / (priv->
				aud_skew_cnt * priv->audio_recv_blocks_total);
		}
