...
static const AVOption mandelbrot_options[] = {
	...
	{
		"start_x", "set the initial x position", OFFSET(start_x), AV_OPT_TYPE_DOUBLE,
		{.dbl = -0.743643887037158704752191506114774}, -100, 100
	},
	{
		"start_y", "set the initial y position", OFFSET(start_y), AV_OPT_TYPE_DOUBLE,
		{.dbl = -0.131825904205311970493132056385139}, -100, 100
	},
	...
	{
		"outer", "set outer coloring mode", OFFSET(outer), AV_OPT_TYPE_INT, {.dbl = NORMALIZED_ITERATION_COUNT}, 0,
		INT_MAX, 0, "outer"
	},
	{
		"iteration_count", "set iteration count mode", 0, AV_OPT_TYPE_CONST, {.dbl = ITERATION_COUNT}, INT_MIN, INT_MAX,
		0, "outer"
	},
	{
		"normalized_iteration_count", "set normalized iteration count mode", 0, AV_OPT_TYPE_CONST,
		{.dbl = NORMALIZED_ITERATION_COUNT}, INT_MIN, INT_MAX, 0, "outer"
	},
	...
	{
		"convergence", "show time until convergence", 0, AV_OPT_TYPE_CONST, {.dbl = CONVTIME}, INT_MIN, INT_MAX, 0,
		"inner"
	},
	{
		"mincol", "color based on point closest to the origin of the iterations", 0, AV_OPT_TYPE_CONST, {.dbl = MINCOL},
		INT_MIN, INT_MAX, 0, "inner"
	},
...
#define Z_Z2_C_ZYKLUS(outr,outi,inr,ini, Z)\
				...
				if (zr * zr + zi * zi > mb->bailout)
				{
					...
					for (; i < mb->maxiter; i++)
					{
						...
						if (zr * zr + zi * zi > mb->bailout)
						{
							switch (mb->outer)
							{
							case ITERATION_COUNT: zr = i;
								break;
							case NORMALIZED_ITERATION_COUNT: zr = i + log2(log(mb->bailout) / log(zr * zr + zi * zi));
								break;
							}
							c = lrintf((sin(zr) + 1) * 127) + lrintf((sin(zr / 1.234) + 1) * 127) * 256 * 256 + lrintf(
								(sin(zr / 100) + 1) * 127) * 256;
							break;
						}
					}
					break;
			...
			if (!c)
			{
				...
				else if (mb->inner == MINCOL)
				{
					...
					c = lrintf((mb->zyklus[closest_index][0] / closest + 1) * 127 + dv) + lrintf(
						(mb->zyklus[closest_index][1] / closest + 1) * 127 + dv) * 256;
				}
			}
			c |= 0xFF000000;
...
static int request_frame(AVFilterLink* link)
{
	...
	picref->video->sample_aspect_ratio = (AVRational)
	{
		1, 1
	}
	picref->pts = mb->pts++;
