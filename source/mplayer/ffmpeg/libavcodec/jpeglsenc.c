 ...
 */
static inline void ls_encode_line(JLSState* state, PutBitContext* pb, void* last, void* cur, int last2, int w,
                                  int stride, int comp, int bits)
{
	...
	while (x < w)
	{
		...
		else
		{
			...
			context = ff_jpegls_quantize(state, D0) * 81 + ff_jpegls_quantize(state, D1) * 9 +
...
int encode_init_ls(AVCodecContext* ctx)
{
	JpeglsContext* c = ctx->priv_data;

	c->avctx = ctx;
	ctx->coded_frame = &c->picture;

	if (ctx->pix_fmt != PIX_FMT_GRAY8 && ctx->pix_fmt != PIX_FMT_GRAY16 && ctx->pix_fmt != PIX_FMT_RGB24 && ctx->pix_fmt
		!= PIX_FMT_BGR24)
	{
