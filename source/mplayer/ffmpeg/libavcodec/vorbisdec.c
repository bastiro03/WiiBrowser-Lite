			...
			for (j = 0, i = 0; i < entries; ++i)
			{
				...
				if (tmp_vlc_bits[i])
				{
					...
					for (k = 0; k < dim; ++k)
					{
						...
						codebook_setup->codevectors[j * dim + k] = codebook_multiplicands[multiplicand_offset] *
...
static int vorbis_parse_setup_hdr_floors(vorbis_context* vc)
{
	...
	for (i = 0; i < vc->floor_count; ++i)
	{
		...
		if (floor_setup->floor_type == 1)
		{
			...
			for (j = 0; j < floor_setup->data.t1.partitions; ++j)
				floor_setup->data.t1.x_list_dim += floor_setup->data.t1.class_dimensions[floor_setup->data.t1.
					partition_class[j]];
			...
			for (j = 0; j < floor_setup->data.t1.partitions; ++j)
			{
				for (k = 0; k < floor_setup->data.t1.class_dimensions[floor_setup->data.t1.partition_class[j]]; ++k, ++
				     floor1_values)
				{
...
static int vorbis_parse_setup_hdr_residues(vorbis_context* vc)
{
	...
	for (i = 0; i < vc->residue_count; ++i)
	{
		...
		{
			av_log(vc->avccontext, AV_LOG_ERROR,
			       "partition out of bounds: type, begin, end, size, blocksize: %"PRIu16", %"PRIu32", %"PRIu32", %u, %"
			       PRIu32"\n",
...
static int vorbis_parse_id_hdr(vorbis_context* vc)
{
	...
	av_dlog(
		NULL,
		" vorbis version %d \n audio_channels %d \n audio_samplerate %d \n bitrate_max %d \n bitrate_nom %d \n bitrate_min %d \n blk_0 %d blk_1 %d \n ",
		vc->version, vc->audio_channels, vc->audio_samplerate, vc->bitrate_maximum, vc->bitrate_nominal,
		vc->bitrate_minimum, vc->blocksize[0], vc->blocksize[1]);
...
int vorbis_decode_init(AVCodecContext* avccontext)
{
                                ...
                                vorbis_floor_data* vfu, float* vec)
{
	...
	ff_vorbis_floor1_render_list(vf->list, vf->x_list_dim, floor1_Y_final, floor1_flag, vf->multiplier, vec,
	                             vf->list[1].x);
					...
					if (!do_not_decode[j])
					{
						...
						for (i = 0; i < c_p_c; ++i)
						{
							...
							if (partition_count + c_p_c - 1 - i < ptns_to_read)
								classifs[j_times_ptns_to_read + partition_count + c_p_c - 1 - i] = temp - temp2 * vr->
									classifications;
							temp = temp2;
						}
					}
...
int vorbis_decode_close(AVCodecContext* avccontext)
{
	vorbis_context* vc = avccontext->priv_data;
...
void vorbis_decode_flush(AVCodecContext* avccontext)
{
	vorbis_context* vc = avccontext->priv_data;
