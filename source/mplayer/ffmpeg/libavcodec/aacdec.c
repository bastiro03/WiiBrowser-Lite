...
int che_configure(AACContext* ac,
...
static ChannelElement* get_che(AACContext* ac, int type, int elem_id)
{
	...
	case 6:
		...
		if (ac->tags_mapped == tags_per_config[ac->oc[1].m4ac.chan_config] - 1 && (type == TYPE_LFE || type ==
			TYPE_SCE))
		{
			ac->tags_mapped++;
...
void reset_predict_state(PredictorState* ps)
{
	ps->r0 = 0.0f;
	ps->r1 = 0.0f;
	ps->cor0 = 0.0f;
...
static int sample_rate_idx(int rate)
{
	...
	if (75132 <= rate) return 1;
	if (55426 <= rate) return 2;
	if (46009 <= rate) return 3;
	if (37566 <= rate) return 4;
	if (27713 <= rate) return 5;
	if (23004 <= rate) return 6;
	if (18783 <= rate) return 7;
	if (13856 <= rate) return 8;
	if (11502 <= rate) return 9;
	if (9391 <= rate) return 10;
	return 11;
}
...
int aac_decode_init(AVCodecContext* avctx)
{
	AACContext* ac = avctx->priv_data;
...
void predict(PredictorState* ps, float* coef,
             int output_enable)
{
	...
	case EXT_SBR_DATA:
		{
			...
			if (ac->oc[1].m4ac.sbr == -1 && ac->oc[1].status == OC_LOCKED)
			{
				av_log(ac->avctx, AV_LOG_ERROR,
			...
			if (ac->oc[1].m4ac.ps == -1 && ac->oc[1].status < OC_LOCKED && ac->avctx->channels == 1)
			{
				ac->oc[1].m4ac.sbr = 1;
				ac->oc[1].m4ac.ps = 1;
...
static void apply_channel_coupling(AACContext* ac, ChannelElement* cc,
                                   ...
                                   void (*apply_coupling_method)(AACContext* ac, SingleChannelElement* target,
                                                                 ChannelElement* cce, int index))
{
	int i, c;
...
int aac_decode_close(AVCodecContext* avctx)
{
	AACContext* ac = avctx->priv_data;
	int i, type;
...
static int read_payload_length_info(struct LATMContext* ctx, GetBitContext* gb)
{
	...
	if (ctx->frame_length_type == 1)
	{
		return ctx->frame_length;
	}
	if (ctx->frame_length_type == 3 ||
                                  ...
                                  GetBitContext* gb)
{
	...
	if (latmctx->audio_mux_version_A == 0)
	{
		...
		if (mux_slot_length_bytes * 8 + 256 < get_bits_left(gb))
		{
...
int latm_decode_init(AVCodecContext* avctx)
{
	struct LATMContext* latmctx = avctx->priv_data;
