...
private:
	...
	const char* fTag; // used for debugging
};
...
static char* openURL_rtsp(RTSPClient* client, const char* url)
{
	// If we were given a user name (and optional password), then use them:
	if (network_username != nullptr)
	{
		const char* password = network_password == nullptr ? "" : network_password;
		return client->describeWithPassword(url, network_username, password);
	}
	return client->describeURL(url);
}

static char* openURL_sip(SIPClient* client, const char* url)
{
	// If we were given a user name (and optional password), then use them:
	if (network_username != nullptr)
	{
...
extern "C" demuxer_t* demux_open_rtp(demuxer_t* demuxer)
{
	...
	do
	{
		...
		rtpState->audioBufferQueue = rtpState->videoBufferQueue = nullptr;
		rtpState->flags = 0;
		...
		while ((subsession = iter.next()) != NULL)
		{
			...
			else
			{
				fprintf(stderr, "Initiated \"%s/%s\" RTP subsession on port %d\n", subsession->mediumName(),
				        subsession->codecName(), subsession->clientPortNum());
	...
	while (false);
	if (!success) return nullptr; // an error occurred
	...
	if (demux_is_multiplexed_rtp_stream(demuxer))
	{
		...
		demuxer_t* od = demux_open(s, DEMUXER_TYPE_UNKNOWN,
		                           audio_id, video_id, dvdsub_id, nullptr);
...
extern "C" int demux_is_mpeg_rtp_stream(demuxer_t* demuxer)
{
	...
	auto rtpState = static_cast<RTPState*>(demuxer->priv);
...
extern "C" void demux_close_rtp(demuxer_t* demuxer)
{
	...
	// Get the RTP state that was stored in the demuxer's 'priv' field:
	auto rtpState = static_cast<RTPState*>(demuxer->priv);
	if (rtpState == nullptr) return;

	teardownRTSPorSIPSession(rtpState);

	UsageEnvironment* env = nullptr;
	TaskScheduler* scheduler = nullptr;
	if (rtpState->mediaSession != nullptr)
	{
                         ...
                         unsigned /*durationInMicroseconds*/)
{
	...
	auto bufferQueue = static_cast<ReadBufferQueue*>(clientData);
	...
	if (bufferQueue->readSource()->isAMRAudioSource())
		...
		static_cast<sh_video_t*>(demuxer->video->sh)->format == mmioFOURCC('H', '2', '6', '4'))
	{
		dp->buffer[0] = 0x00;
		dp->buffer[1] = 0x00;
		dp->buffer[2] = 0x01;
                                 ...
                                 float& ptsBehind)
{
	...
	auto rtpState = static_cast<RTPState*>(demuxer->priv);
	ReadBufferQueue* bufferQueue = nullptr;
	int headersize = 0;
	int waitboth = 0;
	TaskToken task, task2;

	if (demuxer->stream->eof) return nullptr;
	...
	if (ds == demuxer->video)
	{
		...
		if (static_cast<sh_video_t*>(ds->sh)->format == mmioFOURCC('H', '2', '6', '4'))
			headersize = 3;
	}
	...
	if (headersize == 1) // amr
		dp->buffer[0] =
			static_cast<AMRAudioSource*>(bufferQueue->readSource())->lastFrameHeader();
#ifdef CONFIG_FFMPEG
		}
		else {
	...
	if (bufferQueue->prevPacketPTS != 0.0
		...
		&& *(bufferQueue->otherQueue) != nullptr
...
static void teardownRTSPorSIPSession(RTPState* rtpState)
{
	...
	if (mediaSession == nullptr) return;
	if (rtpState->rtspClient != nullptr)
	{
		rtpState->rtspClient->teardownMediaSession(*mediaSession);
	}
	else if (rtpState->sipClient != nullptr)
	{
...
ReadBufferQueue::ReadBufferQueue(MediaSubsession* subsession,
	...
	: prevPacketWasSynchronized(False), prevPacketPTS(0.0), otherQueue(nullptr),
	  dp(nullptr), nextpacket(nullptr),
	  pendingDPHead(nullptr), pendingDPTail(nullptr),
	  fReadSource(subsession == nullptr ? NULL : subsession->readSource()),
	  fRTPSource(subsession == nullptr ? NULL : subsession->rtpSource()),
...
demux_packet_t* ReadBufferQueue::getPendingBuffer()
{
	...
	if (dp != nullptr)
	{
		...
		if (pendingDPHead == nullptr) pendingDPTail = nullptr;
...
{
	double endpts = static_cast<RTPState*>(demuxer->priv)->mediaSession->playEndTime();
	...
	case DEMUXER_CTRL_GET_TIME_LENGTH:
		...
		*static_cast<double*>(arg) = endpts;
		return DEMUXER_CTRL_OK;

	case DEMUXER_CTRL_GET_PERCENT_POS:
		if (endpts <= 0)
			return DEMUXER_CTRL_DONTKNOW;
		*static_cast<int*>(arg) = static_cast<int>(((RTPState*)demuxer->priv)->videoBufferQueue->prevPacketPTS * 100 /
			endpts);
		return DEMUXER_CTRL_OK;
