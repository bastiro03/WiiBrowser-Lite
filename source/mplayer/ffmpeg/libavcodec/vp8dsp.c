...
void filter_common(uint8_t* p, ptrdiff_t stride, int is4tap)
{
	LOAD_PIXELS
	int a, f1, f2;
...
int simple_limit(uint8_t* p, ptrdiff_t stride, int flim)
{
	LOAD_PIXELS
...
int normal_limit(uint8_t* p, ptrdiff_t stride, int E, int I)
{
	LOAD_PIXELS
...
int hev(uint8_t* p, ptrdiff_t stride, int thresh)
{
	LOAD_PIXELS
...
void filter_mbedge(uint8_t* p, ptrdiff_t stride)
{
	int a0, a1, a2, w;
	uint8_t* cm = ff_cropTbl + MAX_NEG_CROP;
