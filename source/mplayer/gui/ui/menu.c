...
static void uiMenuDraw(void)
{
	...
	if (menuRender || menuItem != oldMenuItem)
	{
		...
		if (menuItem != -1)
		{
			...
			for (y = guiApp.menuItems[menuItem].y; y < guiApp.menuItems[menuItem].y + guiApp.menuItems[menuItem].height;
			     y++)
				for (x = guiApp.menuItems[menuItem].x; x < guiApp.menuItems[menuItem].x + guiApp.menuItems[menuItem].
				     width; x++)
				{
...
void uiMenuInit(void)
{
	...
	wsCreateWindow(&guiApp.menuWindow,
	               ...
	               wsNoBorder, wsShowMouseCursor | wsHandleMouseButton | wsHandleMouseMove,
	               wsOverredirect | wsHideFrame | wsMaxSize | wsMinSize | wsHideWindow, "MPlayer menu");
