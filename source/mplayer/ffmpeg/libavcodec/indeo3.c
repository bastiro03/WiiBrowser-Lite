...
int allocate_frame_buffers(Indeo3DecodeContext* ctx,
...
void free_frame_buffers(Indeo3DecodeContext* ctx)
{
	int p;
                       ...
                       const uint8_t* last_ptr)
{
	...
	case 11: /*----------------- MODE 11 (4x8 INTER block processing) ------------------*/
		...
		{
			/* mode 10 and 11 INTER processing */
	...
	while (get_bits_left(&ctx->gb) >= 2)
	{
		...
		case INTRA_NULL:
			...
			else
			{
				...
				{
					av_log(avctx, AV_LOG_ERROR, "motion vector %d %d outside reference\n", mv_x + 4 * curr_cell.xpos,
					       mv_y + 4 * curr_cell.ypos);
					return AVERROR_INVALIDDATA;
				}
...
int decode_close(AVCodecContext* avctx)
{
	Indeo3DecodeContext* ctx = avctx->priv_data;
