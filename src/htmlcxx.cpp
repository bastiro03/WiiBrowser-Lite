...
{
	string array[] = {
		"h1", "h2", "h3", "h4", "h5", "h6", "p", "div", "br", "center", "hr", "li", "title", "form", "td" /*,"img"*/
	};
...
{
	auto link = (char *)attr.c_str();
	string dest;

	int i = strcspn(link, "'\"");
	if (i >= static_cast<int>(strlen(link)))
		return dest;
...
string stripEntities(string input)
{
	...
	while ((start = text.find('&', begin)) != string::npos)
	{
		...
		if (end++ != static_cast<int>(string::npos))
			text.erase(start, end - start);

		if (end - 1 == static_cast<int>(string::npos))
			begin = start + 1;
		else
			begin = end;
	}
...
string findText(tree<HTML::Node>::iterator it)
{
	...
	for (; begin != ends; ++begin)
	{
		...
		{
			auto decode = static_cast<char *>(malloc(begin->text().length() + 1));
			decode_html_entities_utf8(strcpy(decode, begin->text().c_str()), nullptr);
			text.append(decode);
			free(decode);
		}
	}
...
Lista getTag(char *buffer, char *url)
{
	...
	try
	{
		...
		if (it == end)
		{
			decode = static_cast<char *>(malloc(html.length() + 1));
			decode_html_entities_utf8(strcpy(decode, html.c_str()), nullptr);
			string item(decode);
			free(decode);
		...
		for (; it != end; ++it)
		{
			...
			if (!it->isTag() && !it->isClosing() && !it->isComment())
			{
				...
				{
					decode = static_cast<char *>(malloc(it->text().length() + 1));
					decode_html_entities_utf8(strcpy(decode, it->text().c_str()), nullptr);
					string item(decode);
					free(decode);
			...
			else if (it->isTag())
			{
				...
				else if (equal(it->tagName(), "input"))
				{
					if (open[form].open)
					{
						...
						open[form].p->form.input = InsInFondo(open[form].p->form.input, it->attribute("name").second,
															  it->attribute("type").second,
															  stripEntities(it->attribute("value").second), label);
