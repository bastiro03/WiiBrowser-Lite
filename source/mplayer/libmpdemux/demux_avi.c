...
static int demux_avi_read_packet(demuxer_t* demux, demux_stream_t* ds, unsigned int id, unsigned int len, int idxpos,
                                 int flags)
{
	avi_priv_t* priv = demux->priv;
	int skip;
...
static int demux_avi_fill_buffer(demuxer_t* demux, demux_stream_t* dsds)
{
	...
	do
	{
		...
		if (priv->idx_size > 0 && priv->idx_pos < priv->idx_size)
		{
			...
			{
				mp_dbg(MSGT_DEMUX, MSGL_DBG3, "Skip chunk %.4s (0x%X)  \n", (char*)&idx->ckid, idx->ckid);
				continue; // skip this chunk
			}

			pos = priv->idx_offset + AVI_IDX_OFFSET(idx);
			if ((pos < demux->movi_start || pos >= demux->movi_end) && (demux->movi_end > demux->movi_start) && (demux->
		...
		{
			demux->filepos = stream_tell(demux->stream);
			if (demux->filepos >= demux->movi_end && demux->movi_end > demux->movi_start && (demux->stream->flags &
				MP_STREAM_SEEK))
			{
				demux->stream->eof = 1;
				return 0;
...
static int demux_avi_fill_buffer_nini(demuxer_t* demux, demux_stream_t* ds)
{
	...
	if (ds == demux->video) fpos = &priv->idx_pos_v;
	else if (ds == demux->audio) fpos = &priv->idx_pos_a;
	else
		return 0;
...
static demuxer_t* demux_open_avi(demuxer_t* demuxer)
{
	...
	if (priv->idx_size > 0)
	{
		...
		sh_video->i_bps = ((float)vsize / (float)vsamples) * (float)sh_video->video.dwRate / (float)sh_video->video.
			dwScale;
		if (sh_audio) sh_audio->i_bps = ((float)asize / (float)asamples) * (float)sh_audio->audio.dwRate / (float)
	...
	else
	{
		...
		if ((priv->numberofframes = sh_video->video.dwLength) <= 1)
			...
			if (sh_audio && sh_audio->wf->nAvgBytesPerSec) priv->numberofframes = sh_video->fps * sh_audio->audio.
				dwLength / sh_audio->audio.dwRate * sh_audio->audio.dwScale;
		...
		if (sh_audio)
		{
			...
			{
				asize = (float)sh_audio->wf->nAvgBytesPerSec * sh_audio->audio.dwLength * sh_audio->audio.dwScale /
					sh_audio->audio.dwRate;
			}
			else
			{
                           ...
                           float audio_delay, int flags)
{
	...
	if (sh_audio)
	{
		...
		if (sh_audio->audio.dwSampleSize)
		{
			...
			curr_audio_pos = (priv->avi_video_pts + audio_delay) * (float)sh_audio->audio.dwRate / (float)sh_audio->
				audio.dwScale;
		...
		else
		{
			// VBR audio
			/* immediate seeking to audio position, including when streams are delayed */
			int chunks = (priv->avi_video_pts + audio_delay) * (float)sh_audio->audio.dwRate / (float)sh_audio->audio.
				dwScale;
			audio_chunk_pos = 0;
		...
		if (demuxer->type == DEMUXER_TYPE_AVI)
		{
			...
			if (audio_chunk_pos < video_chunk_pos)
			{
				...
				priv->avi_video_pts -= priv->skip_video_frames * (float)sh_video->video.dwScale / (float)sh_video->video
					.dwRate;
