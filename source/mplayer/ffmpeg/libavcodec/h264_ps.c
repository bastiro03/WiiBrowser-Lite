...
int ff_h264_decode_seq_parameter_set(H264Context* h)
{
	...
	if (sps->crop)
	{
		...
		if (sps->crop_left || sps->crop_top)
		{
			av_log(h->s.avctx, AV_LOG_ERROR,
			       "insane cropping not completely supported, this could look slightly wrong ... (left: %d, top: %d)\n",
			       sps->crop_left, sps->crop_top);
		}
		if (sps->crop_right >= crop_horizontal_limit || sps->crop_bottom >= crop_vertical_limit)
		{
			av_log(h->s.avctx, AV_LOG_ERROR,
			       "brainfart cropping not supported, cropping disabled (right: %d, bottom: %d)\n", sps->crop_right,
			       sps->crop_bottom);
	...
	if (s->avctx->debug & FF_DEBUG_PICT_INFO)
	{
		av_log(h->s.avctx, AV_LOG_DEBUG,
		       "sps:%u profile:%d/%d poc:%d ref:%d %dx%d %s %s crop:%d/%d/%d/%d %s %s %d/%d b%d\n",
...
int ff_h264_decode_picture_parameter_set(H264Context* h, int bit_length)
{
	...
	if (pps->sps_id >= MAX_SPS_COUNT || h->sps_buffers[pps->sps_id] == NULL)
	{
