...
static GuiSwitch *Right = nullptr;
static GuiSwitch *Left = nullptr;
GuiToolbar *App = nullptr;
...
static GuiButton *actionButton = nullptr;
...
bool performDownload(file *hfile, char *url, struct block *html)
{
	...
	if (choice)
	{
		...
		if (!select)
#endif
			select = GuiBrowser(nullptr, mainWindow, path, "Download!");
	}
...
static GuiImageData *throbber = nullptr;
static GuiText *msgTxt = nullptr;
 ...
 ***************************************************************************/
int WindowPrompt(const char *title, const char *msg, const char *btn1Label, const char *btn2Label, const char *longText)
{
	int choice = -1;
...
static void *UpdateThread(void *arg)
{
	...
	if (strlen(upd.changelog) > 0)
	{
		...
		sprintf(caption, "An update is available!");
...
void ToggleButtons(GuiToolbar *toolbar, bool checkState)
{
	...
	if (toolbar->btnBack->GetState() != STATE_SELECTED || checkState)
	{
	...
	if (toolbar->btnForward->GetState() != STATE_SELECTED || checkState)
	{
...
{
	auto b = static_cast<GuiButton *>(ptr);
...
static int MenuAdvanced()
{
	...
	TextboxImg.SetScaleX(static_cast<float>(552) / TextboxImg.GetWidth());
	...
	while (menu == MENU_NONE)
	{
		...
		if (sendBtn.GetState() == STATE_CLICKED)
		{
			if (strlen(dataToSend) > 0)
			{
				...
				WindowPrompt("Send report", "Thanks, your comment was posted at: http://wiibrowser.altervista.org/",
							 "Ok", nullptr);
			}
...
static int MenuSettings()
{
	...
	while (menu == MENU_NONE)
	{
		...
		if (ret >= 0 || firstRun)
		{
			...
			if (Settings.ShowTooltip == 0)
				sprintf(options.value[2], "Hide");
			else if (Settings.ShowTooltip == 1)
				sprintf(options.value[2], "Show");
			if (Settings.ShowThumbs == 0)
				sprintf(options.value[3], "Hide");
			else if (Settings.ShowThumbs == 1)
				sprintf(options.value[3], "Show");

			if (Settings.Autoupdate == 0)
				sprintf(options.value[4], "Disabled");
			else if (Settings.Autoupdate == 1)
				sprintf(options.value[4], "Stable");
			else if (Settings.Autoupdate == 2)
				sprintf(options.value[4], "Nightly");

			if (Settings.Language == LANG_JAPANESE)
				sprintf(options.value[5], "Japanese");
			else if (Settings.Language == LANG_ENGLISH)
				sprintf(options.value[5], "English");
			else if (Settings.Language == LANG_GERMAN)
				sprintf(options.value[5], "German");

			if (Settings.Restore == 0)
				sprintf(options.value[6], "Start new");
			else if (Settings.Restore == 1)
				sprintf(options.value[6], "Restore");
			if (Settings.MuteSound == 0)
				sprintf(options.value[8], "On");
			else if (Settings.MuteSound == 1)
				sprintf(options.value[8], "Off");

			if (Settings.ZipFile == 0)
				sprintf(options.value[7], "Never");
			else if (Settings.ZipFile == 1)
				sprintf(options.value[7], "Always");
			else if (Settings.ZipFile == 2)
				sprintf(options.value[7], "Ask each time");
	...
	if (menu == MENU_HOME)
		...
		w.SetEffect(EFFECT_SLIDE_TOP | EFFECT_SLIDE_OUT, 50);
...
static int MenuSplash()
{
	...
	if (conn == NET_ERR)
		WindowPrompt("Connection failure", "Please check network settings, exiting...", "Ok", nullptr);
	if (conn == IP_ERR)
		WindowPrompt("Error reading IP address", "Application exiting. Please check connection settings", "Ok",
					 nullptr);
...
static int MenuBrowse()
{
	...
	url = static_cast<char *>(malloc(sizeof(new_page) + 10));
...
jump:
	decode_html_entities_utf8(url, nullptr);
	save_mem(url);
	promptWindow.SetEffect(EFFECT_SLIDE_TOP | EFFECT_SLIDE_IN, 50);
	...
	if (pFile)
	{
		...
		HTML.data = static_cast<char *>(malloc(sizeof(char) * size));
	...
	{
		url = static_cast<char *>(realloc(url, strlen(nurl) + 1));
		strcpy(url, nurl);
	}
	...
	if (link.length() > 0)
	{
		...
		url = static_cast<char *>(realloc(url, strlen(link.c_str()) + 1));
	...
	while (!choice)
	{
		...
		for (i = 0; i < N; i++)
		{
			if (Block[i].Block->GetState() == STATE_SELECTED)
				...
				Block[i].Label->SetScroll(SCROLL_NONE);
...
{
	struct page str;
	struct favorite *elem, *ret = nullptr;
	...
	for (i = 0, j = 0; i < N; j++)
	{
		...
		ret = static_cast<struct favorite *>(realloc(ret, (i + 1) * sizeof(struct favorite)));
		ret[i++] = temp;
	}
...
static int MenuFavorites()
{
	...
	for (i = 0, xpos = 40, ypos = 100; i < Page.n_url; i++)
	{
		if (strlen(Page.ret[i].name))
			...
			Block[i].Label->SetText(Page.ret[i].url);
	...
	while (!choice)
	{
		...
		for (i = 0; i < n_rec; i++)
		{
			if (Recent[i].Block->GetState() == STATE_SELECTED)
				...
				Recent[i].Label->SetScroll(SCROLL_NONE);
