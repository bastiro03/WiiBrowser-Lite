...
void exponent_init(AC3EncodeContext* s)
{
...
static void compute_exp_strategy(AC3EncodeContext* s)
{
	...
	for (ch = !s->cpl_on; ch <= s->fbw_channels; ch++)
	{
		...
		for (blk = 1; blk < s->num_blocks; blk++, exp += AC3_MAX_COEFS)
		{
			...
			exp_strategy[blk] = EXP_REUSE;
			if (ch == CPL_CH && exp_diff > (EXP_DIFF_THRESHOLD * (s->blocks[blk].end_freq[ch] - s->start_freq[ch]) /
				AC3_MAX_COEFS))
				exp_strategy[blk] = EXP_NEW;
...
static void output_audio_block(AC3EncodeContext* s, int blk)
{
	...
	for (ch = 1; ch <= s->channels; ch++)
	{
		...
		for (i = s->start_freq[ch]; i < block->end_freq[ch]; i++)
		{
			...
			{
			case 0: break;
			case 1: if (q != 128) put_bits(&s->pb, 5, q);
				break;
			case 2: if (q != 128) put_bits(&s->pb, 7, q);
				break;
			case 3: put_sbits(&s->pb, 3, q);
				break;
			case 4: if (q != 128) put_bits(&s->pb, 7, q);
				break;
			case 14: put_sbits(&s->pb, 14, q);
				break;
			case 15: put_sbits(&s->pb, 16, q);
				break;
			default: put_sbits(&s->pb, b - 1, q);
				break;
			}
		}
		if (ch == CPL_CH)
...
int set_channel_info(AC3EncodeContext* s, int channels,
                     uint64_t* channel_layout)
{
	...
	case AV_CH_LAYOUT_SURROUND: s->channel_mode = AC3_CHMODE_3F;
		break;
	case AV_CH_LAYOUT_2_1: s->channel_mode = AC3_CHMODE_2F1R;
		break;
	case AV_CH_LAYOUT_4POINT0: s->channel_mode = AC3_CHMODE_3F1R;
		break;
...
int validate_options(AC3EncodeContext* s)
{
	AVCodecContext* avctx = s->avctx;
	int i, ret, max_sr;
	...
	if (!avctx->bit_rate)
	{
		...
		{
		case 1: avctx->bit_rate = 96000;
			break;
		case 2: avctx->bit_rate = 192000;
			break;
		case 3: avctx->bit_rate = 320000;
			break;
		case 4: avctx->bit_rate = 384000;
			break;
		case 5: avctx->bit_rate = 448000;
			break;
		}
	}
...
void set_bandwidth(AC3EncodeContext* s)
{
...
int allocate_buffers(AC3EncodeContext* s)
{
	AVCodecContext* avctx = s->avctx;
	int blk, ch;
	...
	else
	{
		for (blk = 0; blk < s->num_blocks; blk++)
		{
			...
			FF_ALLOCZ_OR_GOTO(avctx, block->fixed_coef, channels *
			                  sizeof(*block->fixed_coef), alloc_fail);
			for (ch = 0; ch < channels; ch++)
				block->fixed_coef[ch] = block->mdct_coef[ch];
		}
	}
...
int ff_ac3_encode_init(AVCodecContext* avctx)
{
	...
	if (CONFIG_AC3_ENCODER || CONFIG_EAC3_ENCODER)
	{
