...
retry:
	...
	if (CONFIG_H263I_DECODER&& s
	->
	codec_id == CODEC_ID_H263I
	)
	{
		ret = ff_intel_h263_decode_picture_header(s);
	}
	else
	if (CONFIG_FLV_DECODER&& s
	->
	h263_flv
	)
	{
...
#define SET_QPEL_FUNC(postfix1, postfix2) \
	...
	if (s->last_picture_ptr == NULL && (s->pict_type == AV_PICTURE_TYPE_B || s->dropable)) return
		get_consumed_bytes(s, buf_size);
