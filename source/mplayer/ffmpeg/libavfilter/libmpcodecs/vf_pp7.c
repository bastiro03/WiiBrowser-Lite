...
static int mediumthresh_c(DCTELEM* src, int qp)
{
	...
	for (i = 1; i < 16; i++)
	{
		...
		if (level + threshold1 > threshold2)
		{
			if (level + 2 * threshold1 > 2 * threshold2)
			{
				a += level * factor[i];
			}
			else
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	{
		filter(vf->priv, dmpi->planes[0], mpi->planes[0], dmpi->stride[0], mpi->stride[0], mpi->w, mpi->h, mpi->qscale,
		       mpi->qstride, 1);
		filter(vf->priv, dmpi->planes[1], mpi->planes[1], dmpi->stride[1], mpi->stride[1],
		       mpi->w >> mpi->chroma_x_shift, mpi->h >> mpi->chroma_y_shift, mpi->qscale, mpi->qstride, 0);
...
static int vf_open(vf_instance_t* vf, char* args)
{
	...
	case 1: requantize = softthresh_c;
		break;
	default:
	case 2: requantize = mediumthresh_c;
		break;
	}
