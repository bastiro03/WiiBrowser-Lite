...
typedef struct
{
	...
	,
	samples
	)[MPA_MAX_CHANNELS * MPA_FRAME_SIZE];
...
static void fill_tone_level_array(QDM2Context* q, int flag)
{
	...
	for (ch = 0; ch < q->nb_channels; ch++)
		for (sb = 0; sb < 30; sb++)
			for (i = 0; i < 8; i++)
			{
				if ((tab = coeff_per_sb_for_dequant[q->coeff_per_sb_select][sb]) < (last_coeff[q->coeff_per_sb_select] -
					1))
...
static int process_subpacket_9(QDM2Context* q, QDM2SubPNode* node)
{
	...
	n = coeff_per_sb_for_avg[q->coeff_per_sb_select][QDM2_SB_USED(q->sub_sampling) - 1] + 1;
	// same as averagesomething function
...
static void qdm2_decode_super_block(QDM2Context* q)
{
	...
	for (i = 0; packet_bytes > 0; i++)
	{
		...
		if (packet->type >= 9 && packet->type <= 12)
		{
...
static void qdm2_fft_tone_synthesizer(QDM2Context* q, int sub_packet)
{
	...
	if (q->fft_coefs_min_index[4] >= 0)
		for (i = q->fft_coefs_min_index[4]; i < q->fft_coefs_max_index[4]; i++)
		{
			...
			level = (q->fft_coefs[i].exp < 0)
				        ? 0.0
				        : fft_tone_level_table[q->superblocktype_2_3 ? 0 : 1][q->fft_coefs[i].exp & 63];
		...
		if (q->fft_coefs_min_index[i] >= 0)
		{
			for (j = q->fft_coefs_min_index[i]; j < q->fft_coefs_max_index[i]; j++)
			{
				...
				if (offset < q->frequency_range)
				{
					...
					tone.level = (q->fft_coefs[j].exp < 0)
						             ? 0.0
						             : fft_tone_level_table[q->superblocktype_2_3 ? 0 : 1][q->fft_coefs[j].exp & 63];
...
int qdm2_decode_init(AVCodecContext* avctx)
{
...
int qdm2_decode_close(AVCodecContext* avctx)
{
	QDM2Context* s = avctx->priv_data;
