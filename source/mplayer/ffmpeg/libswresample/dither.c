...
#define TMP_EXTRA 2
	...
	for (i = 0; i < len + TMP_EXTRA; i++)
	{
		...
		{
		case SWR_DITHER_RECTANGULAR: v = ((double)seed) / UINT_MAX - 0.5;
			break;
	...
	for (i = 0; i < len; i++)
	{
		...
		switch (in_fmt)
		{
		case AV_SAMPLE_FMT_S16: ((int16_t*)dst)[i] = v;
			break;
		case AV_SAMPLE_FMT_S32: ((int32_t*)dst)[i] = v;
			break;
		case AV_SAMPLE_FMT_FLT: ((float*)dst)[i] = v;
			break;
		case AV_SAMPLE_FMT_DBL: ((double*)dst)[i] = v;
			break;
		default: av_assert0(0);
		}
	}
