...
TextEditor::TextEditor(const wchar_t* intext, int LinesToDraw, const char* path)
{
	...
	filesize = static_cast<u32>(GetFileSize(path));
	...
	filenameTxt = new GuiText(filename, 22, (GXColor)
	{
		0, 0, 0, 255
	}
	)
	...
	MainFileTxt = new GuiLongText(intext, FONTSIZE, (GXColor)
	{
		0, 0, 0, 255
	}
	)
...
wstring* toWString(const wchar_t* s)
{
	...
	for (std::string::size_type i = 0; i < size; ++i)
		ws->at(i) = static_cast<unsigned char>(s[i]);
...
TextEditor* TextEditor::LoadFileEd(const char* filepath)
{
	...
	char* string = static_cast<char*>(LoadFile((char*)filepath, GetFileSize(filepath)));
