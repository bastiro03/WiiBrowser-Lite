...
struct directx_fourcc_caps
{
	...
	{"I420 ", IMGFMT_I420, {sizeof(DDPIXELFORMAT), DDPF_FOURCC, MAKEFOURCC('I', '4', '2', '0'), 0, 0, 0, 0, 0}},
	//yv12 with swapped uv
	{"IYUV ", IMGFMT_IYUV, {sizeof(DDPIXELFORMAT), DDPF_FOURCC, MAKEFOURCC('I', 'Y', 'U', 'V'), 0, 0, 0, 0, 0}},
	//same as i420
	...
	{"RGB15", IMGFMT_RGB15, {sizeof(DDPIXELFORMAT), DDPF_RGB, 0, 16, 0x0000001F, 0x000003E0, 0x00007C00, 0}},
	//RGB 5:5:5
	{"BGR15", IMGFMT_BGR15, {sizeof(DDPIXELFORMAT), DDPF_RGB, 0, 16, 0x00007C00, 0x000003E0, 0x0000001F, 0}},
	{"RGB16", IMGFMT_RGB16, {sizeof(DDPIXELFORMAT), DDPF_RGB, 0, 16, 0x0000001F, 0x000007E0, 0x0000F800, 0}},
	//RGB 5:6:5
                       ...
                       unsigned char* srca, int stride)
{
	...
	case IMGFMT_IYUV:
	case IMGFMT_YVU9:
		vo_draw_alpha_yv12(w, h, src, srca, stride, image + dstride * y0 + x0, dstride);
		break;
	case IMGFMT_YUY2:
		vo_draw_alpha_yuy2(w, h, src, srca, stride, image + dstride * y0 + 2 * x0, dstride);
		break;
	case IMGFMT_UYVY:
		vo_draw_alpha_yuy2(w, h, src, srca, stride, image + dstride * y0 + 2 * x0 + 1, dstride);
		break;
	case IMGFMT_RGB15:
	case IMGFMT_BGR15:
		vo_draw_alpha_rgb15(w, h, src, srca, stride, image + dstride * y0 + 2 * x0, dstride);
		break;
	case IMGFMT_RGB16:
	case IMGFMT_BGR16:
		vo_draw_alpha_rgb16(w, h, src, srca, stride, image + dstride * y0 + 2 * x0, dstride);
		break;
	case IMGFMT_RGB24:
	case IMGFMT_BGR24:
		vo_draw_alpha_rgb24(w, h, src, srca, stride, image + dstride * y0 + 4 * x0, dstride);
		break;
	case IMGFMT_RGB32:
	case IMGFMT_BGR32:
		vo_draw_alpha_rgb32(w, h, src, srca, stride, image + dstride * y0 + 4 * x0, dstride);
		break;
	}
}
...
static BOOL WINAPI EnumCallbackEx(GUID FAR* lpGUID, LPSTR lpDriverDescription, LPSTR lpDriverName, LPVOID lpContext,
                                  HMONITOR hm)
{
	if (!lpGUID)
...
static uint32_t Directx_InitDirectDraw(void)
{
	...
	if (vidmode)
	{
		...
		mp_msg(MSGT_VO, MSGL_V, "<vo_directx><INFO>Initialized adapter %i for %i x %i @ %i \n", vo_adapter_num,
		       vm_width, vm_height, vm_bpp);
		return 0;
	}
	if (g_lpdd->lpVtbl->SetCooperativeLevel(g_lpdd, vo_w32_window, DDSCL_NORMAL) != DD_OK)
	{
...
static uint32_t Directx_ManageDisplay(void)
{
	...
	if (!nooverlay)
	{
		...
		if ((capsDrv.dwCaps & DDCAPS_ALIGNSIZESRC) && capsDrv.dwAlignSizeSrc)
			rs.right = rs.left + ((rs.right - rs.left + capsDrv.dwAlignSizeSrc / 2) & -(signed)(capsDrv.
				dwAlignSizeSrc));
	...
	if (FAILED(ddrval))
	{
		...
		mp_msg(MSGT_VO, MSGL_ERR, "<vo_directx><ERROR>Overlay:x1:%i,y1:%i,x2:%i,y2:%i,w:%i,h:%i\n", rd.left, rd.top,
		       rd.right, rd.bottom, rd.right - rd.left, rd.bottom - rd.top);
		...
		if (ddrval == DDERR_SURFACELOST)
		{
			...
			ddrval = g_lpddsOverlay->lpVtbl->UpdateOverlay(g_lpddsOverlay, &rs, g_lpddsPrimary, &rd, dwUpdateFlags,
			                                               &ovfx);
			if (ddrval != DD_OK)
		...
		if (ddrval != DD_OK)
		{
			ddrval = g_lpddsOverlay->lpVtbl->UpdateOverlay(g_lpddsOverlay, NULL, g_lpddsPrimary, NULL, DDOVER_HIDE,
			                                               NULL);
			return 1;
		}
	}
...
static int preinit(const char* arg)
{
	...
	if (Directx_CheckPrimaryPixelformat() != 0)
		return 1;
	if (!nooverlay && Directx_CheckOverlayPixelformats() == 0)
	{
	...
	{
		//if we can't have overlay we create a backpuffer with the same imageformat as the primary surface
...
static int config(uint32_t width, uint32_t height, uint32_t d_width, uint32_t d_height, uint32_t options, char* title,
                  uint32_t format)
{
	image_format = format;
	image_width = width;
	...
	{
		if (DD_OK != g_lpddsOverlay->lpVtbl->
		                             QueryInterface(g_lpddsOverlay, &IID_IDirectDrawColorControl, (void**)&g_cc))
