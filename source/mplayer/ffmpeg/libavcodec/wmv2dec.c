...
static void parse_mb_skip(Wmv2Context* w)
{
	...
	case SKIP_TYPE_MPEG:
		for (mb_y = 0; mb_y < s->mb_height; mb_y++)
		{
			for (mb_x = 0; mb_x < s->mb_width; mb_x++)
			{
				mb_type[mb_y * s->mb_stride + mb_x] = (get_bits1(&s->gb) ? MB_TYPE_SKIP : 0) | MB_TYPE_16x16 |
					MB_TYPE_L0;
			}
		}
		break;
...
static int decode_ext_header(Wmv2Context* w)
{
	...
	if (s->avctx->debug & FF_DEBUG_PICT_INFO)
	{
		av_log(s->avctx, AV_LOG_DEBUG,
		       ...
		       fps, s->bit_rate, w->mspel_bit, w->abt_flag, w->j_type_bit, w->top_left_mv_flag, w->per_mb_rl_bit, code,
		       s->loop_filter,
		       code);
	}
	return 0;
...
int wmv2_decode_init(AVCodecContext* avctx)
{
...
int wmv2_decode_end(AVCodecContext* avctx)
{
