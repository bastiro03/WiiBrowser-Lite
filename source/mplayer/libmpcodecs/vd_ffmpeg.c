...
static int init(sh_video_t* sh)
{
	...
	if (lavc_codec->capabilities & CODEC_CAP_DR1 && !do_vis_debug && lavc_codec->id != CODEC_ID_INTERPLAY_VIDEO &&
		lavc_codec->id != CODEC_ID_VP8 && lavc_codec->id != CODEC_ID_LAGARITH)
		ctx->do_dr1 = 1;
...
static int get_buffer(AVCodecContext* avctx, AVFrame* pic)
{
	...
	else if (type == MP_IMGTYPE_IP || type == MP_IMGTYPE_IPB)
	{
