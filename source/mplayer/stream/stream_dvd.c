...
static void list_chapters(ifo_handle_t* vts_file, tt_srpt_t* tt_srpt, int title_no)
{
	...
	for (i = 0; i < vts_file->vts_ptt_srpt->title[title_no].nr_of_ptts; i++)
	{
		...
		if (ptt[i].pgn < pgc->nr_of_programs)
			...
			last_cell = 0;
		mp_msg(MSGT_IDENTIFY, MSGL_INFO, "%02d:%02d:%02d.%03d,", t / 3600000, (t / 60000) % 60, (t / 1000) % 60,
		       t % 1000);
		do
		{
...
static int dvd_seek_to_time(stream_t* stream, ifo_handle_t* vts_file, double sec)
{
	...
	{
		if (tmap_sector >= d->cur_pgc->cell_playback[i].first_sector && tmap_sector <= d->cur_pgc->cell_playback[i].
			last_sector)
		{
...
static int open_s(stream_t* stream, int mode, void* opts, int* file_format)
{
	...
	if (1)
	{
		...
		if (mp_msg_test(MSGT_IDENTIFY, MSGL_INFO))
		{
			...
			for (title_no = 0; title_no < tt_srpt->nr_of_srpts; title_no++)
			{
				mp_msg(MSGT_IDENTIFY, MSGL_INFO, "ID_DVD_TITLE_%d_CHAPTERS=%d\n", title_no + 1,
				       tt_srpt->title[title_no].nr_of_ptts);
				mp_msg(MSGT_IDENTIFY, MSGL_INFO, "ID_DVD_TITLE_%d_ANGLES=%d\n", title_no + 1,
				       tt_srpt->title[title_no].nr_of_angles);
			}
		}
		if (mp_msg_test(MSGT_IDENTIFY, MSGL_V))
		{
			...
			if (DVDUDFVolumeInfo(dvd, volid, sizeof(volid), NULL, 0) >= 0 || DVDISOVolumeInfo(
				dvd, volid, sizeof(volid), NULL, 0) >= 0)
		...
		*file_format = DEMUXER_TYPE_MPEG_PS;
		mp_msg(MSGT_DVD, MSGL_V, "DVD start=%d end=%d  \n", d->cur_pack,
		       d->cur_pgc->cell_playback[d->last_cell - 1].last_sector);
		stream->priv = (void*)d;
		return STREAM_OK;
