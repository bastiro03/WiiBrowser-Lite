...
int init_mdct_win(TwinContext* tctx)
{
	int i, j, ret;
	const ModeTab* mtab = tctx->mtab;
	int size_s = mtab->size / mtab->fmode[FT_SHORT].sub;
...
void construct_perm_table(TwinContext* tctx, enum FrameType ftype)
{
	int block_size;
	const ModeTab* mtab = tctx->mtab;
	int size;
...
void init_bitstream_params(TwinContext* tctx)
{
	const ModeTab* mtab = tctx->mtab;
...
int twin_decode_close(AVCodecContext* avctx)
{
	TwinContext* tctx = avctx->priv_data;
	int i;
...
int twin_decode_init(AVCodecContext* avctx)
{
	int ret;
	TwinContext* tctx = avctx->priv_data;
	int isampf, ibps;
	...
	switch (isampf)
	{
	case 44: avctx->sample_rate = 44100;
		break;
	case 22: avctx->sample_rate = 22050;
		break;
	case 11: avctx->sample_rate = 11025;
		break;
	default: avctx->sample_rate = isampf * 1000;
		break;
	}
	...
	{
	case (8 << 8) + 8: tctx->mtab = &mode_08_08;
		break;
	case (11 << 8) + 8: tctx->mtab = &mode_11_08;
		break;
	case (11 << 8) + 10: tctx->mtab = &mode_11_10;
		break;
	case (16 << 8) + 16: tctx->mtab = &mode_16_16;
		break;
	case (22 << 8) + 20: tctx->mtab = &mode_22_20;
		break;
	case (22 << 8) + 24: tctx->mtab = &mode_22_24;
		break;
	case (22 << 8) + 32: tctx->mtab = &mode_22_32;
		break;
	case (44 << 8) + 40: tctx->mtab = &mode_44_40;
		break;
	case (44 << 8) + 48: tctx->mtab = &mode_44_48;
		break;
	default:
