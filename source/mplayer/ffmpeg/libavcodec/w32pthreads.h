                          ...
                          void* (*start_routine)(void*), void* arg)
{
	...
	thread->arg = arg;
	thread->handle = (void*)_beginthreadex(nullptr, 0, win32thread_worker, thread,
	                                       0, nullptr);
...
{
	win32_cond_t* win32_cond = nullptr;
	if (cond_init)
	{
		cond_init(cond);
		return;
	...
	if (!win32_cond)
		return;
	cond->ptr = win32_cond;
	win32_cond->semaphore = CreateSemaphore(nullptr, 0, 0x7fffffff, nullptr);
	if (!win32_cond->semaphore)
		return;
	win32_cond->waiters_done = CreateEvent(nullptr, TRUE, FALSE, nullptr);
	if (!win32_cond->waiters_done)
		return;

	pthread_mutex_init(&win32_cond->mtx_waiter_count, nullptr);
	pthread_mutex_init(&win32_cond->mtx_broadcast, nullptr);
}
...
static void pthread_cond_broadcast(pthread_cond_t* cond)
{
	...
	if (have_waiter)
	{
		ReleaseSemaphore(win32_cond->semaphore, win32_cond->waiter_count, nullptr);
...
static void pthread_cond_signal(pthread_cond_t* cond)
{
	...
	if (have_waiter)
	{
		ReleaseSemaphore(win32_cond->semaphore, 1, nullptr);
		WaitForSingleObject(win32_cond->waiters_done, INFINITE);
		ResetEvent(win32_cond->waiters_done);
	}
