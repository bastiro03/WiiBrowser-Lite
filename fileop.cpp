...
bool GuiBrowser(GuiWindow* mainWindow, GuiWindow* parentWindow, char* path, const char* label)
{
	...
	GuiText URL(strchr(temp, '/'), 20, (GXColor)
	{
		0, 0, 0, 255
	}
	)
	GuiText DEV("SD", 20, (GXColor)
	{
		0, 0, 0, 255
	}
	)
	...
	GuiText okBtnTxt(label, 24, (GXColor)
	{
		0, 0, 0, 255
	}
	)
	...
	GuiText cancelBtnTxt("Cancel", 24, (GXColor)
	{
		0, 0, 0, 255
	}
	)
			...
			if (fileBrowser.fileList[i]->GetState() == STATE_CLICKED)
			{
				...
				else
				{
					...
					if (strlen(browser.dir) > 1)
						...
						// print current path
					else sprintf(fullpath, "%s%s", rootdir, browserList[browser.selIndex].filename);
					// print current path
