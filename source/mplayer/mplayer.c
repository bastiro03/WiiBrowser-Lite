	...
	else if (mpctx->sh_audio)
	{
		if (mpctx->delay > 0.5)
			saddf(line, &pos, width, "%4.1f%% ",
			      100.0 * audio_time_usage / mpctx->delay);
		else
			saddf(line, &pos, width, "??,?%% ");
	}
	...
	if (mpctx->sh_video)
	{
		...
		else
		{
			mp_msg(MSGT_CPLAYER, MSGL_V, MSGTR_FilefmtFourccSizeFpsFtime,
			       mpctx->demuxer->file_format, mpctx->sh_video->format, mpctx->sh_video->disp_w,
			       mpctx->sh_video->disp_h,
...
void wiiDVDNav(int command)
{
	...
	case MP_CMD_DVDNAV_UP:
		cmd->args[0].v.s = strdup("up");
		break;
	case MP_CMD_DVDNAV_DOWN:
		cmd->args[0].v.s = strdup("down");
		break;
	case MP_CMD_DVDNAV_LEFT:
		cmd->args[0].v.s = strdup("left");
		break;
	case MP_CMD_DVDNAV_RIGHT:
		cmd->args[0].v.s = strdup("right");
		break;
	case MP_CMD_DVDNAV_MENU:
		cmd->args[0].v.s = strdup("menu");
		break;
	case MP_CMD_DVDNAV_SELECT:
		cmd->args[0].v.s = strdup("select");
		break;
	case MP_CMD_DVDNAV_PREVMENU:
		cmd->args[0].v.s = strdup("prev");
		break;
	case MP_CMD_DVDNAV_MOUSECLICK:
		cmd->args[0].v.s = strdup("mouse");
		break;
	}
	mp_input_queue_cmd(cmd);
}
...
void wiiSetProperty(int command, float value)
{
	...
	case MP_CMD_SWITCH_RATIO:
		cmd->name = strdup("switch_ratio");
		ass_force_reload = 1; /*force_load_font = 1;*/
		break;
	case MP_CMD_SWITCH_AUDIO:
		cmd->name = strdup("switch_audio");
		break;
	case MP_CMD_AUDIO_DELAY:
		cmd->name = strdup("audio_delay");
		break;
	case MP_CMD_SUB_VISIBILITY:
		cmd->name = strdup("sub_visibility");
		break;
	case MP_CMD_SUB_DELAY:
		cmd->name = strdup("sub_delay");
		break;
	case MP_CMD_SUB_SELECT:
		cmd->name = strdup("sub");
		break;
	case MP_CMD_SUB_STEP:
		cmd->name = strdup("sub_step");
		break;
	}
