...
unsigned encode_slice_data(AVCodecContext* avctx,
                               ...
                               const AVFrame* pict, int* got_packet)
{
	...
	uint8_t* buf;
	int pic_size, ret;
	int frame_size = FFALIGN(avctx->width, 16) * FFALIGN(avctx->height, 16) * 16 + 500 + FF_MIN_BUFFER_SIZE;
...
int prores_encode_init(AVCodecContext* avctx)
{
	int i;
	ProresContext* ctx = avctx->priv_data;
...
int prores_encode_close(AVCodecContext* avctx)
{
	ProresContext* ctx = avctx->priv_data;
