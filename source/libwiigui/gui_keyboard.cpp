...
GuiKeyboard::GuiKeyboard(char* t, u32 max, int autofill)
{
	...
	kbText = new GuiLongText(GetDisplayText(kbtextstr), 20, (GXColor)
	{
		0, 0, 0, 0xff
	}
	)
	...
	autoCompleteText = new GuiText("autocmpltstr", 20, (GXColor)
	{
		0, 0, 0, 150
	}
	)
	...
	keyEnterText = new GuiText("Enter", 20, (GXColor)
	{
		0, 0, 0, 0xff
	}
	)
...
{
	if (pos < 0 || pos >= static_cast<int>(strlen(kbtextstr)))
		return;
