                                               ...
                                               const char* section)
{
	if (wctx->is_packets_and_frames)
		wctx->nb_section_packet_frame = !strcmp(section, "packet")
...
static const AVOption default_options[] = {
	{
		"noprint_wrappers", "do not print headers and footers", OFFSET(noprint_wrappers), AV_OPT_TYPE_INT, {.dbl = 0},
		0, 1
	},
...
typedef struct CompactContext
{
	...
	const char* (*escape_str)(AVBPrint* dst, const char* src, char sep, void* log_ctx);
} CompactContext;
...
static const AVOption flat_options[] = {
	...
	{
		"hierachical", "specify if the section specification should be hierarchical", OFFSET(hierarchical),
		AV_OPT_TYPE_INT, {.dbl = 1}, 0, 1
	},
	{
		"h", "specify if the section specification should be hierarchical", OFFSET(hierarchical), AV_OPT_TYPE_INT,
		{.dbl = 1}, 0, 1
	},
	{NULL},
};
...
{
	FlatContext* flat = wctx->priv;
	int n = wctx->is_packets_and_frames
...
static const AVOption xml_options[] = {
	{
		"fully_qualified", "specify if the output should be fully qualified", OFFSET(fully_qualified), AV_OPT_TYPE_INT,
		{.dbl = 0}, 0, 1
	},
	{
		"q", "specify if the output should be fully qualified", OFFSET(fully_qualified), AV_OPT_TYPE_INT, {.dbl = 0}, 0,
		1
	},
...
static void show_stream(WriterContext* w, AVFormatContext* fmt_ctx, int stream_idx)
{
	...
	if (dec_ctx->bit_rate > 0)
		print_val("bit_rate", dec_ctx->bit_rate, unit_bit_per_second_str);
	else
