...
static volatile int entangled_thread_counter = 0;
	...
	if (!buf->base[0])
	{
		...
		for (i = 0; i < 4 && size[i]; i++)
		{
			...
			if ((s->flags & CODEC_FLAG_EMU_EDGE) || !size[2])
				...
				buf->data[i] = buf->base[i] + FFALIGN(
					(buf->linesize[i] * EDGE_WIDTH >> v_shift) + (pixel_size * EDGE_WIDTH >> h_shift), stride_align[i]);
		}
...
int avcodec_default_reget_buffer(AVCodecContext* s, AVFrame* pic)
{
	...
	if (pic->data[0] && (pic->width != s->width || pic->height != s->height || pic->format != s->pix_fmt))
	{
		av_log(s, AV_LOG_WARNING, "Picture changed from size:%dx%d fmt:%s to size:%dx%d fmt:%s in reget buffer()\n",
		       pic->width, pic->height, av_get_pix_fmt_name(pic->format), s->width, s->height,
...
int avcodec_default_execute2(AVCodecContext* c, int (*func)(AVCodecContext* c2, void* arg2, int jobnr, int threadnr),
...
void avcodec_get_frame_defaults(AVFrame* pic)
{
	...
	pic->pkt_pos = -1;
	pic->key_frame = 1;
	pic->sample_aspect_ratio = (AVRational)
	{
		0, 1
	}
                                              ...
                                              const AVPacket* avpkt)
{
	...
	*got_picture_ptr = 0;
	if ((avctx->coded_width || avctx->coded_height) && av_image_check_size(
		avctx->coded_width, avctx->coded_height, 0, avctx))
		return -1;
...
enum AVMediaType avcodec_get_type(enum CodecID codec_id)
{
	...
	if (codec_id < CODEC_ID_FIRST_SUBTITLE)
		return AVMEDIA_TYPE_AUDIO;
	if (codec_id < CODEC_ID_FIRST_UNKNOWN)
