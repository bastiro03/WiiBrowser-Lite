...
static void plRowSelect(GtkCList* clist, gint row, gint column, GdkEvent* event, gpointer user_data)
{
	...
	{
	case 0: CLFileSelected[row] = 1;
		break;
	case 1: CLListSelected[row] = 1;
		break;
	}
}
...
static void plUnRowSelect(GtkCList* clist, gint row, gint column, GdkEvent* event, gpointer user_data)
{
	switch ((int)user_data)
	{
	case 0: CLFileSelected[row] = 0;
		break;
	case 1: CLListSelected[row] = 0;
		break;
	}
}
			...
			for (i = 0; i < NrOfEntrys; i++)
			{
				if (CLFileSelected[i])
				{
					...
					else
					{
						...
						text[0][0] = itext[0][0];
						text[0][1] = cpath ? cpath : current_path;
						gtk_clist_append(GTK_CLIST(CLSelected), text[0]);
						g_free(cpath);
					}
				}
		...
		if ((dir = opendir(DirNode->path)))
		{
			while ((dirent = readdir(dir)))
			{
				...
				if (stat(path, &statbuf) != -1 && S_ISDIR(statbuf.st_mode) && dirent->d_name[0] != '.')
				{
					...
					DirNode->scaned = 0;
					DirNode->path = strdup(path);
					subdir = check_for_subdir(path);
					node = gtk_ctree_insert_node(ctree, parent_node, NULL, (name ? &name : &text), 4, pxOpenedBook,
					                             msOpenedBook, pxClosedBook, msClosedBook, !subdir, FALSE);
					gtk_ctree_node_set_row_data_full(ctree, node, DirNode, NULL);
					if (subdir) node = gtk_ctree_insert_node(ctree, node, NULL, &dummy, 4, NULL, NULL, NULL, NULL,
					                                         FALSE, FALSE);
				}
				free(path);
				path = NULL;
...
GtkWidget* create_PlayList(void)
{
	...
	if (!pxOpenedBook) pxOpenedBook = gdk_pixmap_create_from_xpm_d(PlayList->window, &msOpenedBook, &transparent,
	                                                               (gchar**)dir2_xpm);
	if (!pxClosedBook) pxClosedBook = gdk_pixmap_create_from_xpm_d(PlayList->window, &msClosedBook, &transparent,
	                                                               (gchar**)open2_xpm);

	parent = gtk_ctree_insert_node(GTK_CTREE(CTDirTree), NULL, NULL, &root, 4, pxOpenedBook, msOpenedBook, pxClosedBook,
	                               msClosedBook, FALSE, FALSE);
	DirNode = malloc(sizeof(DirNodeType));
	DirNode->scaned = 0;
	DirNode->path = strdup(root);
