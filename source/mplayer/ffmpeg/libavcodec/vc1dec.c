...
int get_chroma_mv(int* mvx, int* mvy, int* a, int flag, int* tx, int* ty)
{
	int idx, i;
...
static void vc1_mc_4mv_chroma4(VC1Context* v)
{
	...
	for (i = 0; i < 4; i++)
	{
		...
		if (!v->rnd)
		{
			dsp->put_h264_chroma_pixels_tab[1](s->dest[1] + off, srcU, s->uvlinesize << fieldmv, 4, uvmx_field[i],
			                                   uvmy_field[i]);
			dsp->put_h264_chroma_pixels_tab[1](s->dest[2] + off, srcV, s->uvlinesize << fieldmv, 4, uvmx_field[i],
			                                   uvmy_field[i]);
		}
		else
		{
			v->vc1dsp.put_no_rnd_vc1_chroma_pixels_tab[1](s->dest[1] + off, srcU, s->uvlinesize << fieldmv, 4,
			                                              uvmx_field[i], uvmy_field[i]);
			v->vc1dsp.put_no_rnd_vc1_chroma_pixels_tab[1](s->dest[2] + off, srcV, s->uvlinesize << fieldmv, 4,
			                                              uvmx_field[i], uvmy_field[i]);
		}
	}
}
...
void get_mvdata_interlaced(VC1Context* v, int* dmv_x,
...
int scaleforsame_x(VC1Context* v, int n /* MV */, int dir)
{
...
int scaleforsame_y(VC1Context* v, int i, int n /* MV */, int dir)
{
	int scaledvalue, refdist;
	int scalesame1, scalesame2;
...
int scaleforopp_x(VC1Context* v, int n /* MV */)
{
	int scalezone1_x, zone1offset_x;
...
int scaleforopp_y(VC1Context* v, int n /* MV */, int dir)
{
	int scalezone1_y, zone1offset_y;
	int scaleopp1, scaleopp2, brfd;
	int scaledvalue;
...
int scaleforsame(VC1Context* v, int i, int n /* MV */,
                 int dim, int dir)
{
	int brfd, scalesame;
...
int scaleforopp(VC1Context* v, int n /* MV */,
                int dim, int dir)
{
                               ...
                               int pred_flag, int dir)
{
	...
	s->mv[dir][n][0] = s->current_picture.f.motion_val[dir][xy + v->blocks_off][0] = ((px + dmv_x + r_x) & ((r_x << 1) -
		1)) - r_x;
	s->mv[dir][n][1] = s->current_picture.f.motion_val[dir][xy + v->blocks_off][1] = ((py + dmv_y + r_y - y_bias) & ((
		r_y << 1) - 1)) - r_y + y_bias;
	if (mv1)
	{
		/* duplicate motion data for 1-MV block */
		s->current_picture.f.motion_val[dir][xy + 1 + v->blocks_off][0] = s->current_picture.f.motion_val[dir][xy + v->
			blocks_off][0];
		s->current_picture.f.motion_val[dir][xy + 1 + v->blocks_off][1] = s->current_picture.f.motion_val[dir][xy + v->
			blocks_off][1];
		s->current_picture.f.motion_val[dir][xy + wrap + v->blocks_off][0] = s->current_picture.f.motion_val[dir][xy + v
			->blocks_off][0];
		s->current_picture.f.motion_val[dir][xy + wrap + v->blocks_off][1] = s->current_picture.f.motion_val[dir][xy + v
			->blocks_off][1];
		s->current_picture.f.motion_val[dir][xy + wrap + 1 + v->blocks_off][0] = s->current_picture.f.motion_val[dir][xy
			+ v->blocks_off][0];
		s->current_picture.f.motion_val[dir][xy + wrap + 1 + v->blocks_off][1] = s->current_picture.f.motion_val[dir][xy
			+ v->blocks_off][1];
		v->mv_f[dir][xy + 1 + v->blocks_off] = v->mv_f[dir][xy + v->blocks_off];
		v->mv_f[dir][xy + wrap + v->blocks_off] = v->mv_f[dir][xy + wrap + 1 + v->blocks_off] = v->mv_f[dir][xy + v->
			blocks_off];
	}
}
	...
	if (n == 0 || n == 1 || v->blk_mv_type[xy])
	{
		if (!s->first_slice_line)
		{
			if (!v->is_intra[s->mb_x - s->mb_stride])
			{
				...
				if (v->blk_mv_type[pos_b] && !v->blk_mv_type[xy])
				{
					B[0] = (B[0] + s->current_picture.f.motion_val[0][s->block_index[n_adj ^ 2] - 2 * wrap][0] + 1) >>
						1;
					B[1] = (B[1] + s->current_picture.f.motion_val[0][s->block_index[n_adj ^ 2] - 2 * wrap][1] + 1) >>
						1;
				}
			}
			if (s->mb_width > 1)
			{
				if (!v->is_intra[s->mb_x - s->mb_stride + 1])
				{
					...
					if (v->blk_mv_type[pos_c] && !v->blk_mv_type[xy])
					{
						C[0] = (1 + C[0] + (s->current_picture.f.motion_val[0][s->block_index[n_adj ^ 2] - 2 * wrap + 2]
							[0])) >> 1;
						C[1] = (1 + C[1] + (s->current_picture.f.motion_val[0][s->block_index[n_adj ^ 2] - 2 * wrap + 2]
							[1])) >> 1;
					}
					if (s->mb_x == s->mb_width - 1)
					{
						if (!v->is_intra[s->mb_x - s->mb_stride - 1])
						{
							...
							if (v->blk_mv_type[pos_c] && !v->blk_mv_type[xy])
							{
								C[0] = (1 + C[0] + s->current_picture.f.motion_val[0][s->block_index[1] - 2 * wrap - 2][
									0]) >> 1;
								C[1] = (1 + C[1] + s->current_picture.f.motion_val[0][s->block_index[1] - 2 * wrap - 2][
									1]) >> 1;
							}
						}
						else
...
int scale_mv(int value, int bfrac, int inv, int qs)
{
	int n = bfrac;
...
int scale_mv_intfi(int value, int bfrac, int inv,
                   int qs, int qs_last)
{
	int n = bfrac;
                              ...
                              int* ttmb_out)
{
	...
	if (ttmb == -1)
	{
		ttblk = ff_vc1_ttblk_to_tt[v->tt_index][
			get_vlc2(gb, ff_vc1_ttblk_vlc[v->tt_index].table, VC1_TTBLK_VLC_BITS, 1)];
	}
	if (ttblk == TT_4X4)
	{
...
void vc1_apply_p_v_loop_filter(VC1Context* v, int block_num)
{
	MpegEncContext* s = &v->s;
	...
	if (s->mb_y != s->end_mb_y || block_num < 2)
	{
		...
		{
			bottom_cbp = (block_num < 2)
				             ? (mb_cbp >> ((block_num + 2) * 4))
				             : (v->cbp[s->mb_x] >> ((block_num - 2) * 4));
			bottom_is_intra = (block_num < 2)
				                  ? (mb_is_intra >> ((block_num + 2) * 4))
...
void vc1_apply_p_h_loop_filter(VC1Context* v, int block_num)
{
	MpegEncContext* s = &v->s;
	...
	if (s->mb_x != s->mb_width || !(block_num & 5))
	{
		...
		{
			right_cbp = (block_num & 1)
				            ? (v->cbp[s->mb_x - s->mb_stride] >> ((block_num - 1) * 4))
				            : (mb_cbp >> ((block_num + 1) * 4));
			right_is_intra = (block_num & 1)
				                 ? (v->is_intra[s->mb_x - s->mb_stride] >> ((block_num - 1) * 4))
		...
		if (!skipped /* unskipped MB */)
		{
			...
			/* test if block is intra and has pred */
			{
				...
				for (i = 0; i < 6; i++)
					if (is_intra[i])
					{
						if (((!s->first_slice_line || (i == 2 || i == 3)) && v->mb_type[0][s->block_index[i] - s->
								block_wrap[i]])
		...
		if (ff_vc1_mbmode_intfrp[v->fourmvswitch][idx_mbmode][0] == MV_PMODE_INTFR_INTRA)
		{
			...
			for (i = 0; i < 6; i++)
			{
				...
				if (i < 4)
				{
					stride_y = s->linesize << fieldtx;
					off = (fieldtx)
						      ? ((i & 1) * 8) + ((i & 2) >> 1) * s->linesize
						      : (i & 1) * 8 + 4 * (i & 2) * s->linesize;
				}
				else
				{
...
static int vc1_decode_p_mb_intfi(VC1Context* v)
{
	...
	if (idx_mbmode <= 1)
	{
		...
		for (i = 0; i < 6; i++)
		{
			...
			off = (i & 4) ? 0 : ((i & 1) * 8 + (i & 2) * 4 * s->linesize);
			off += v->second_field
				       ? ((i & 4) ? s->current_picture_ptr->f.linesize[1] : s->current_picture_ptr->f.linesize[0])
				       : 0;
			s->dsp.put_signed_pixels_clamped(s->block[i], s->dest[dst_idx] + off,
			                                 (i & 4) ? s->uvlinesize : s->linesize);
			// TODO: loop filter
...
static void vc1_decode_b_mb_intfi(VC1Context* v)
{
	...
	else
	{
		...
		if (idx_mbmode <= 5)
		{
			...
			{
				get_mvdata_interlaced(v, &dmv_x[bmvtype == BMV_TYPE_BACKWARD], &dmv_y[bmvtype == BMV_TYPE_BACKWARD],
...
int vc1_decode_init_alloc_tables(VC1Context* v)
{
	MpegEncContext* s = &v->s;
	int i;
...
int vc1_decode_init(AVCodecContext* avctx)
{
	VC1Context* v = avctx->priv_data;
	MpegEncContext* s = &v->s;
	GetBitContext gb;
...
int vc1_decode_end(AVCodecContext* avctx)
{
	VC1Context* v = avctx->priv_data;
	int i;
