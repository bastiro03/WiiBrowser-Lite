...
int frei0r_init(AVFilterContext* ctx,
                const char* dl_name, int type)
{
	...
	if (pi->plugin_type != type)
	{
		av_log(ctx, AV_LOG_ERROR,
		       ...
		       pi->plugin_type == F0R_PLUGIN_TYPE_FILTER
			       ? "filter"
			       : pi->plugin_type == F0R_PLUGIN_TYPE_SOURCE
			       ? "source"
			       : pi->plugin_type == F0R_PLUGIN_TYPE_MIXER2
			       ? "mixer2"
			       : pi->plugin_type == F0R_PLUGIN_TYPE_MIXER3
			       ? "mixer3"
			       : "unknown");
		return AVERROR(EINVAL);
	}
...
int filter_init(AVFilterContext* ctx, const char* args, void* opaque)
{
	Frei0rContext* frei0r = ctx->priv;
	char dl_name[1024], c;
	*frei0r->params = 0;
...
int source_init(AVFilterContext* ctx, const char* args, void* opaque)
{
	Frei0rContext* frei0r = ctx->priv;
	char dl_name[1024], c;
	char frame_size[128] = "";
...
static int source_request_frame(AVFilterLink* outlink)
{
	...
	frei0r->update(frei0r->instance, av_rescale_q(picref->pts, frei0r->time_base, (AVRational)
	{
		1, 1000
	}
	)
