...
static void uiPlaybarDraw(void)
{
	...
	{
	case -1: x = (guiApp.videoWindow.Width - guiApp.playbar.width) / 2;
		break;
	case -2: x = (guiApp.videoWindow.Width - guiApp.playbar.width);
		break;
...
static void uiPlaybarMouseHandle(int Button, int X, int Y, int RX, int RY)
{
	...
	for (i = 0; i <= guiApp.IndexOfPlaybarItems; i++)
		if ((guiApp.playbarItems[i].pressed != btnDisabled) &&
			(wgIsRect(X, Y, guiApp.playbarItems[i].x, guiApp.playbarItems[i].y,
			          guiApp.playbarItems[i].x + guiApp.playbarItems[i].width,
			          guiApp.playbarItems[i].y + guiApp.playbarItems[i].height)))
		{
			currentselected = i;
			break;
		}
...
void uiPlaybarInit(void)
{
	...
	wsCreateWindow(&guiApp.playbarWindow,
	               ...
	               wsNoBorder, wsShowMouseCursor | wsHandleMouseButton | wsHandleMouseMove, wsHideFrame | wsHideWindow,
	               "PlayBar");
