...
// Copied from vlc
static unsigned char* parseH264ConfigStr(const char* configStr,
                                         unsigned int& configSize)
{
	...
	auto cfg = new unsigned char[5 * strlen(dup)];
	psz = dup;
	for (i = 0; i < i_records; i++)
	{
		...
		cfg[configSize++] = 0x00;
		cfg[configSize++] = 0x01;
		configSize += av_base64_decode(&cfg[configSize],
		                               psz,
		                               5 * strlen(dup) - 3);
...
static Boolean
parseQTState_video(const QuickTimeGenericRTPSource::QTState& qtState,
                   unsigned& fourcc); // forward
static Boolean
parseQTState_audio(const QuickTimeGenericRTPSource::QTState& qtState,
                                              ...
                                              unsigned size)
{
	...
	if (!size || size > INT_MAX - sizeof(BITMAPINFOHEADER))
		return bih;
	bih = static_cast<BITMAPINFOHEADER*>(realloc(bih, sizeof(BITMAPINFOHEADER) + size));
	if (!bih)
		return original;
                              ...
                              unsigned& flags)
{
	...
	BITMAPINFOHEADER* bih
		= static_cast<BITMAPINFOHEADER*>(calloc(1, sizeof(BITMAPINFOHEADER)));
		...
		strcmp(subsession->codecName(), "X-QUICKTIME") == 0)
	{
		...
		QuickTimeGenericRTPSource* qtRTPSource
			= static_cast<QuickTimeGenericRTPSource*>(subsession->rtpSource());
		unsigned fourcc;
		do
		{
		...
		uint8_t* pos = static_cast<uint8_t*>(qtRTPSource->qtState.sdAtom) + 86;
		uint8_t* endpos = static_cast<uint8_t*>(qtRTPSource->qtState.sdAtom)
                              ...
                              unsigned& flags)
{
	...
	WAVEFORMATEX* wf = static_cast<WAVEFORMATEX*>(calloc(1, sizeof(WAVEFORMATEX)));
	sh_audio->wf = wf;
	...
	else if (strcmp(subsession->codecName(), "MP4A-LATM") == 0)
	{
		...
		static_cast<MPEG4LATMAudioRTPSource*>(subsession->rtpSource())->omitLATMDataLengthField();
	}
		...
		strcmp(subsession->codecName(), "X-QUICKTIME") == 0)
	{
		...
		uint8_t* pos = static_cast<uint8_t*>(qtRTPSource->qtState.sdAtom) + 52;
		uint8_t* endpos = static_cast<uint8_t*>(qtRTPSource->qtState.sdAtom)
			+ qtRTPSource->qtState.sdAtomSize;
		while (pos + 8 < endpos)
		{
			...
			if (!memcmp(pos + 4, "wave", 4) && fourcc == mmioFOURCC('Q', 'D', 'M', '2') &&
				...
				atomLength <= INT_MAX)
			{
				sh_audio->codecdata = static_cast<unsigned char*>(malloc(atomLength - 8));
                               ...
                               MediaSubsession* subsession)
{
	...
	demux_stream_t* d_video = demuxer->video;
	auto sh_video = static_cast<sh_video_t*>(d_video->sh);

	// If we already know the subsession's video frame rate, use it:
	int fps = static_cast<int>(subsession->videoFPS());
	if (fps != 0)
	{
		sh_video->fps = fps;
	...
	const unsigned maxNumFramesToWaitFor = 300;
	int lastfps = 0;
	for (unsigned i = 0; i < maxNumFramesToWaitFor; ++i)
	{
		...
		if (curPTS != lastPTS && lastPTS != 0.0)
		{
			...
			fps = static_cast<int>(1 / fabs(curPTS - lastPTS) + 0.5); // rounding
			if (fps == lastfps)
			{
				fprintf(stderr,
                   ...
                   unsigned& fourcc)
{
	...
	const char* sdAtom = qtState.sdAtom;
	if (sdAtom == nullptr || qtState.sdAtomSize < 2 * 4) return False;

	fourcc = *static_cast<unsigned*>(&sdAtom[4]); // put in host order
	return True;
}
                   ...
                   unsigned& fourcc, unsigned& numChannels)
{
	...
	const char* sdAtom = qtState.sdAtom;
	if (sdAtom == nullptr || qtState.sdAtomSize < 7 * 4) return False;

	fourcc = *static_cast<unsigned*>(&sdAtom[4]); // put in host order

	const char* word7Ptr = &sdAtom[6 * 4];
