			...
			for (ctx = 0; ctx < 3; ctx++)
				for (node = 0; node < 5; node++)
					model->coeff_dcct[pt][ctx][node] = av_clip(
						((model->coeff_dccv[pt][node] * vp6_dccv_lc[ctx][node][0] + 128) >> 8) + vp6_dccv_lc[ctx][node][
							1], 1, 255);
	}
	return 0;
}
...
int vp6_decode_init(AVCodecContext* avctx)
{
...
int vp6_decode_free(AVCodecContext* avctx)
{
	VP56Context* s = avctx->priv_data;
	int pt, ct, cg;
