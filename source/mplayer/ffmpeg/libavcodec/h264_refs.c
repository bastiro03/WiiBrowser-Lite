			...
			for (index = 0; ; index++)
			{
				...
				if (reordering_of_pic_nums_idc < 3)
				{
					...
					else
					{
						...
						{
							if (ref->long_ref == h->ref_list[list][i].long_ref && ref->pic_id == h->ref_list[list][i].
								pic_id)
								break;
						}
						for (; i > index; i--)
...
void ff_h264_fill_mbaff_ref_list(H264Context* h)
{
	...
	for (list = 0; list < h->list_count; list++)
	{
		for (i = 0; i < h->ref_count[list]; i++)
		{
			...
			for (j = 0; j < 2; j++)
			{
				h->chroma_weight[16 + 2 * i][list][j][0] = h->chroma_weight[16 + 2 * i + 1][list][j][0] = h->
					chroma_weight[i][list][j][0];
				h->chroma_weight[16 + 2 * i][list][j][1] = h->chroma_weight[16 + 2 * i + 1][list][j][1] = h->
					chroma_weight[i][list][j][1];
			}
		}
	}
}
...
int ff_h264_execute_ref_pic_marking(H264Context* h, MMCO* mmco, int mmco_count)
{
	...
	if (err >= 0 && h->long_ref_count == 0 && h->short_ref_count <= 2 && h->pps.ref_count[0] <= 1 + (s->
		picture_structure != PICT_FRAME) && s->current_picture_ptr->f.pict_type == AV_PICTURE_TYPE_I)
	{
	...
	else
	{
		if (get_bits1(gb))
		{
			...
			for (i = 0; i < MAX_MMCO_COUNT; i++)
			{
				...
				if (opcode == MMCO_SHORT2LONG || opcode == MMCO_LONG2UNUSED || opcode == MMCO_LONG || opcode ==
					MMCO_SET_MAX_LONG)
				{
					unsigned int long_arg = get_ue_golomb_31(gb);
					if (long_arg >= 32 || (long_arg >= 16 && !(opcode == MMCO_SET_MAX_LONG && long_arg == 16) && !(
