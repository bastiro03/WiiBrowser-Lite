...
static int add_frame(muxer_headers_t* spriv, uint64_t idur, uint8_t* ptr, int len, uint8_t pt, uint64_t dts,
                     uint64_t pts);
...
static muxer_stream_t* mpegfile_new_stream(muxer_t* muxer, int type)
{
	...
	if (type == MUXER_TYPE_VIDEO)
	{
		...
		spriv->framebuf = init_frames(spriv->framebuf_cnt, 5000);
	...
	else
	{
		...
		spriv->framebuf_cnt = 30;
		spriv->framebuf_used = 0;
		spriv->framebuf = init_frames(spriv->framebuf_cnt, 2048);
		if (spriv->framebuf == NULL)
		{
			mp_msg(MSGT_MUXER, MSGL_FATAL, "Couldn't allocate initial frames structure, abort!\n");
			goto init_fail;
...
static int write_mpeg_pes_header(muxer_headers_t* h, uint8_t* pes_id, uint8_t* buff, uint16_t plen, int stuffing_len,
                                 int mux_type)
{
	int len;
	...
	else //MPEG2
	{
		...
		len++;
		buff[len] = ((h->buffer_size > 0) ? 1 : 0) | (h->pts ? (h->dts ? 0xC0 : 0x80) : 0);
		//pes extension + pts/dts flags
		len++;
		buff[len] = (h->pts ? (h->dts ? 10 : 5) : 0) + ((h->buffer_size > 0) ? 3 : 0) + stuffing_len;
		//pts + std + stuffing
		len++;
	}
...
static int find_packet_timestamps(muxer_priv_t* priv, muxer_stream_t* s, unsigned int start, uint64_t* dts,
                                  uint64_t* pts)
{
	muxer_headers_t* spriv = s->priv;
...
static int fill_packet(muxer_t* muxer, muxer_stream_t* s, int finalize)
{
	...
	if (spriv->dts || spriv->pts)
	{
		if ((spriv->dts && priv->scr >= spriv->dts) || priv->scr >= spriv->pts)
			mp_msg(MSGT_MUXER, MSGL_ERR, "\r\nERROR: scr %.3f, dts %.3f, pts %.3f\r\n", (double)priv->scr / 27000000.0,
			       (double)spriv->dts / 27000000.0, (double)spriv->pts / 27000000.0);
			...
			mp_msg(MSGT_MUXER, MSGL_INFO, "\r\nWARNING>: scr %.3f, dts %.3f, pts %.3f, diff %.3f, piff %.3f\r\n",
			       ...
			       (double)(spriv->dts - priv->scr) / 27000000.0, (double)(spriv->pts - priv->scr) / 27000000.0);
	}
	...
	if (spriv->track_bufsize > spriv->max_buffer_size)
		mp_msg(MSGT_MUXER, MSGL_ERR, "\r\nBUFFER OVERFLOW: %d > %d, pts: %"PRIu64"\r\n", spriv->track_bufsize,
		       spriv->max_buffer_size, spriv->pts);
	...
	for (i = 0; i < muxer->avih.dwStreams; i++)
	{
		...
		if (spriv->track_bufsize < 0)
		{
			...
			mp_msg(MSGT_MUXER, MSGL_INFO,
			       ...
			       muxer->sysrate / 125, priv->delta_scr);
...
static int soft_telecine(muxer_priv_t* priv, muxer_headers_t* vpriv, uint8_t* fps_ptr, uint8_t* se_ptr,
                         uint8_t* pce_ptr, int n)
{
	if (!pce_ptr)
		return 0;
	if (fps_ptr != NULL)
...
static size_t parse_mpeg12_video(muxer_stream_t* s, muxer_priv_t* priv, muxer_headers_t* spriv, float fps, size_t len)
{
	...
	while (i + 4 < len)
	{
		...
		if ((s->buffer[i] == 0) && (s->buffer[i + 1] == 0) && (s->buffer[i + 2] == 1))
		{
			...
			case 0xb3: //sequence
				{
					...
					if (!spriv->delta_pts)
						spriv->delta_pts = spriv->nom_delta_pts = (uint64_t)(27000000.0 * spriv->delta_clock);
					mp_msg(MSGT_MUXER, MSGL_DBG2, "\nFPS: %.3f, FRAMETIME: %.3f\n", fps, (double)1 / fps);
					if (priv->patch_seq)
			...
			if (s->buffer[i + 3] >= 0x01 && s->buffer[i + 3] <= 0xAF) break;
			//slice, we have already analized what we need
		}
		i++;
	}
	if (err)
	...
	if (diff > 0)
	{
		for (i = 0; i < 3; i++)
		{
			...
			mp_msg(MSGT_MUXER, MSGL_DBG2, "FIXED_PTS: %.3f, FIXED_DTS: %.3f\n",
			       vpriv->framebuf[i].pts / 27000000.0, vpriv->framebuf[i].dts / 27000000.0);
		}
		return diff;
	}
	return 0;
	...
	while (ptr < len - 5)
	{
		...
		else if (s->buffer[ptr + 3] == 0xb6) //vop
		{
			...
			mp_msg(MSGT_MUXER, MSGL_DBG2,
			       ...
			       FTYPE(pt), vpriv->picture.timeinc_resolution, vpriv->picture.timeinc_unit, delta, delta_pts,
			       delta_pts / 27000000.0,
	...
	if (!vpriv->frame_duration && vpriv->framebuf_used == 3)
	{
		...
		if (vpriv->frame_duration)
		{
			...
			mp_msg(MSGT_MUXER, MSGL_INFO, "FRAME DURATION: %"PRIu64"   %.3f\n",
			       vpriv->frame_duration, vpriv->frame_duration / 27000000.0);
			vpriv->is_ready = 1;
		}
	}

	mp_msg(MSGT_MUXER, MSGL_DBG2, "LAST_PTS: %.3f, LAST_DTS: %.3f\n",
	       vpriv->last_pts / 27000000.0, vpriv->last_dts / 27000000.0);
...
static void fix_parameters(muxer_stream_t* stream)
{
	...
	if (stream->type == MUXER_TYPE_AUDIO)
	{
		...
		if (stream->wf->wFormatTag == AUDIO_A52)
		{
			...
			if (priv->is_genmpeg1 || priv->is_genmpeg2)
				fix_audio_sys_header(priv, spriv->id, 0xbd, FFMAX(conf_abuf_size, 58) * 1024);
			//only one audio at the moment
			spriv->id = 0xbd;
			if (!conf_abuf_size)
	...
	if (s->type == MUXER_TYPE_VIDEO)
	{
		...
		if (!spriv->vframes)
		{
			...
			mp_msg(MSGT_MUXER, MSGL_INFO, "INITV: %.3f, %.3f, fps: %.3f\r\n", (double)spriv->last_pts / 27000000.0f,
			       (double)spriv->last_dts / 27000000.0f, fps);
		}
...
int muxer_init_muxer_mpeg(muxer_t* muxer)
{
	...
	if (priv->vaspect || priv->vframerate || priv->vwidth || priv->vheight || priv->vbitrate || priv->panscan_width ||
		priv->panscan_height)
	{
	...
	if (conf_init_adelay)
		priv->init_adelay = -(double)conf_init_adelay / 1000.0;
