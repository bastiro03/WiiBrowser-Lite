...
char* fsVideoFilterNames[][2] =
{
	...
	{
		MSGTR_Filter_AllAudioFiles,
		"*.aac,*.ac3,*.aif,*.aifc,*.aiff,*.amr,*.ape,*.au,*.awb,*.cdg,*.f4a,*.f4b,*.flac,*.m4a,*.m4b,*.mka,*.mp+,*.mp2,*.mp3,*.mpc,*.mpga,*.mpp,*.nsa,*.oga,*.ogg,*.pcm,*.qcp,*.ra,*.snd,*.spx,*.voc,*.vqf,*.w64,*.wav,*.wma,*.wv,*.wvp"
	},
	{
		MSGTR_Filter_AllVideoFiles,
...
char* fsAudioFileNames[][2] =
{
	...
	{
		MSGTR_Filter_AllAudioFiles,
		"*.aac,*.ac3,*.f4a,*.m4a,*.mka,*.mp2,*.mp3,*.mpga,*.oga,*.ogg,*.pcm,*.spx,*.wav,*.wma"
	},
	{MSGTR_Filter_AllFiles, "*"},
	{NULL, NULL}
};
...
void ShowFileSelect(int type, int modal)
{
	...
	if (fsTopList_items) g_list_free(fsTopList_items);
	fsTopList_items = NULL;
	{
		...
		if (fsMedium)
		{
			for (i = 0; i < FF_ARRAY_ELEMS(fsHistory); i++)
				if (fsHistory[i])
				{
					fsTopList_items = g_list_append(fsTopList_items, fsHistory[i]);
					if (c) c = gstrcmp(dir, fsHistory[i]);
				}
		}
		if (c && dir)
		{
...
static void fs_PersistantHistory(char* subject)
{
	...
	for (i = 0; i < FF_ARRAY_ELEMS(fsHistory); i++)
		...
		{
			char* tmp = fsHistory[i];
			fsHistory[i] = fsHistory[0];
			fsHistory[0] = tmp;
			return;
		}
                                     ...
                                     gpointer user_data)
{
	...
	case fsVideoSelector:
		for (i = 0; fsVideoFilterNames[i][0]; i++)
			...
			{
				fsFilter = fsVideoFilterNames[i][1];
				fsLastVideoFilterSelected = i;
				break;
			}
		break;
	case fsSubtitleSelector:
...
default: return;
	...
	static void fs_Ok_released(GtkButton* button, gpointer user_data)
	{
		...
		case fsVideoSelector:
			...
			uiSetFileName(fsSelectedDirectory, fsSelectedFile,
			              fsLastVideoFilterSelected == l ? STREAMTYPE_PLAYLIST : STREAMTYPE_FILE);
	...
	GtkWidget* create_FileSelect(void)
	{
		...
		gtk_widget_set_events(fsFileSelect,
		                      GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK | GDK_KEY_PRESS_MASK |
		...
		dpixmap = gdk_pixmap_colormap_create_from_xpm_d(fsFileSelect->window, fsColorMap, &dmask,
		                                                &style->bg[GTK_STATE_NORMAL], (gchar**)dir_xpm);
		fpixmap = gdk_pixmap_colormap_create_from_xpm_d(fsFileSelect->window, fsColorMap, &fmask,
		                                                &style->bg[GTK_STATE_NORMAL], (gchar**)file_xpm);
		...
		uppixmap = gdk_pixmap_colormap_create_from_xpm_d(fsFileSelect->window, fsColorMap, &upmask,
		                                                 &upstyle->bg[GTK_STATE_NORMAL], (gchar**)up_xpm);
		...
		gtk_signal_connect(GTK_OBJECT(fsFileSelect), "key_release_event",
		                   GTK_SIGNAL_FUNC(on_FileSelect_key_release_event), NULL);

		gtk_signal_connect(GTK_OBJECT(fsFilterCombo), "changed", GTK_SIGNAL_FUNC(fs_fsFilterCombo_changed),
		                   fsFilterCombo);
		gtk_signal_connect(GTK_OBJECT(fsFilterCombo), "activate", GTK_SIGNAL_FUNC(fs_fsFilterCombo_activate),
		                   fsFilterCombo);
		...
		gtk_signal_connect(GTK_OBJECT(fsUp), "key_release_event", GTK_SIGNAL_FUNC(on_FileSelect_key_release_event),
		                   NULL);
		gtk_signal_connect(GTK_OBJECT(fsOk), "released", GTK_SIGNAL_FUNC(fs_Ok_released), fsCombo4);
		gtk_signal_connect(GTK_OBJECT(fsOk), "key_release_event", GTK_SIGNAL_FUNC(on_FileSelect_key_release_event),
		                   NULL);
		gtk_signal_connect(GTK_OBJECT(fsCancel), "released", GTK_SIGNAL_FUNC(fs_Cancel_released), NULL);
		gtk_signal_connect(GTK_OBJECT(fsCancel), "key_release_event", GTK_SIGNAL_FUNC(on_FileSelect_key_release_event),
		                   NULL);
