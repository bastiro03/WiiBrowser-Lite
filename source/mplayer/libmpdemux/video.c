...
int video_read_properties(sh_video_t* sh_video)
{
	...
	case VIDEO_OTHER:
		{
			if ((d_video->demuxer->file_format == DEMUXER_TYPE_ASF) || (d_video->demuxer->file_format ==
				DEMUXER_TYPE_AVI))
			{
				// display info:
			...
			if (!picture.fps)
			{
				...
				if (diff > 0)
				{
					...
					mp_msg(MSGT_DECVIDEO, MSGL_V, "FPS seems to be: %f, resolution: %d, delta_units: %d\n", picture.fps,
					       picture.timeinc_resolution, diff);
				}
			}
			if (picture.fps)
			{
	...
	*start = NULL;
	...
	if (video_codec == VIDEO_MPEG12)
	{
		...
		while (videobuf_len < VIDEOBUFFER_SIZE - MAX_VIDEO_PACKET_SIZE)
		{
			...
			switch (i)
			{
			case 0x1B3: mp_header_process_sequence_header(&picture, &videobuffer[start]);
				break;
			case 0x1B5: mp_header_process_extension(&picture, &videobuffer[start]);
				break;
			case 0x1B2: process_userdata(&videobuffer[start], videobuf_len - start);
				break;
			case 0x100: picture_coding_type = (videobuffer[start + 1] >> 3) & 7;
				break;
			}
		}
		...
		if (sh_video->fps != fps)
			if (!force_fps && !telecine)
			{
				mp_msg(MSGT_CPLAYER, MSGL_WARN, "Warning! FPS changed %5.3f -> %5.3f  (%f) [%d]  \n", sh_video->fps,
				       fps, sh_video->fps - fps, picture.frame_rate_code);
				sh_video->fps = fps;
