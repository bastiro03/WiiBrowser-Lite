	...
	fail:
		...
		static inline void put_timestamp(AVIOContext* pb, int id, int64_t timestamp)
		{
			avio_w8(pb,
			        ...
			        (((timestamp >> 30) & 0x07) << 1) |
			        1);
			avio_wb16(pb, (timestamp >> 15 & 0x7fff) << 1 | 1);
			avio_wb16(pb, (timestamp & 0x7fff) << 1 | 1);
		}
					                        ...
					                        int64_t pts, int64_t dts, int64_t scr, int trailer_size)
					{
						...
						if (s->is_vcd && (id & 0xe0) == AUDIO_ID)
							...
							if (packet_size > 0)
							{
								...
								av_fifo_generic_read(stream->fifo, ctx->pb, payload_size - stuffing_size,
								                     (void*)avio_write);
	...
	return scr;
							...
							retry:
								...
								if (s->is_vcd)
								{
									...
									while ((vcd_pad_bytes = get_vcd_padding_size(ctx, stream->premux_packet->pts))
										>= s->packet_size)
									{
							...
							static int mpeg_mux_write_packet(AVFormatContext* ctx, AVPacket* pkt)
							{
								...
								PacketDesc* pkt_desc;
								int preload;
								const int is_iframe = st->codec->codec_type == AVMEDIA_TYPE_VIDEO && (pkt->flags &
									AV_PKT_FLAG_KEY);
								...
								*stream->next_packet =
								...
								if (s->is_dvd)
								{
									if (is_iframe && (s->packet_number == 0 || (pts - stream->vobu_start_pts >=
										36000)))
									{
...
#define E AV_OPT_FLAG_ENCODING_PARAM
							static const AVOption options[] = {
								...
								{
									"preload", "Initial demux-decode delay in microseconds.", OFFSET(preload),
									AV_OPT_TYPE_INT, {500000}, 0, INT_MAX, E
								},
								{NULL},
							};
