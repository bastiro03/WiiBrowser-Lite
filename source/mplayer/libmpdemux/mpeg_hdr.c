...
static int h264_parse_vui(mp_mpeg_header_t* picture, unsigned char* buf, unsigned int n)
{
	...
	{
		picture->timeinc_unit = (getbits(buf, n, 8) << 24) | (getbits(buf, n + 8, 8) << 16) | (getbits(buf, n + 16, 8)
			<< 8) | getbits(buf, n + 24, 8);
		n += 32;

		picture->timeinc_resolution = (getbits(buf, n, 8) << 24) | (getbits(buf, n + 8, 8) << 16) | (
			getbits(buf, n + 16, 8) << 8) | getbits(buf, n + 24, 8);
		n += 32;
