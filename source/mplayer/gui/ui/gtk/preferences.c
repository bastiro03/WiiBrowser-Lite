...
void ShowPreferences(void)
{
	...
	gtk_adjustment_set_value(HSExtraStereoMuladj, gtkAOExtraStereoMul);
	{
		...
		while (audio_out_drivers[i])
		{
			...
			tmp[0] = (char*)info->short_name;
			tmp[1] = (char*)info->name;
			gtk_clist_append(GTK_CLIST(CLADrivers), tmp);
		}
...

	{
		...
		while (video_out_drivers[i])
			...
			{
				if (video_driver_list && !gstrcmp(video_driver_list[0],
				                                  (char*)video_out_drivers[i - 1]->info->short_name)) old_video_driver =
					c;
				c++;
				tmp[0] = (char*)video_out_drivers[i - 1]->info->short_name;
				tmp[1] = (char*)video_out_drivers[i - 1]->info->name;
	...
	switch (osd_level)
	{
	case 0: gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(RBOSDNone), TRUE);
		break;
	case 1: gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(RBOSDIndicator), TRUE);
		break;
	case 2: gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(RBOSDTandP), TRUE);
		break;
	case 3: gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(RBOSDTPTT), TRUE);
		break;
	}
#if 0
	...
	if (index_mode == 1) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(CBIndex), 1);
	{
		...
		for (i = 0; mpcodecs_vd_drivers[i]; i++)
		{
			...
			if (video_fm_list && !gstrcmp(video_fm_list[0], (char*)mpcodecs_vd_drivers[i]->info->short_name)) name = (
				char*)mpcodecs_vd_drivers[i]->info->name;
		}
...

	{
		...
		for (i = 0; mpcodecs_ad_drivers[i]; i++)
		{
			...
			if (audio_fm_list && !gstrcmp(audio_fm_list[0], (char*)mpcodecs_ad_drivers[i]->info->short_name)) name = (
				char*)mpcodecs_ad_drivers[i]->info->name;
		}
...
static void prButton(GtkButton* button, gpointer user_data)
{
	...
	/* -- 5th page */
...

		{
			...
			for (i = 0; mpcodecs_vd_drivers[i]; i++)
				...
				{
					listSet(&video_fm_list, (char*)mpcodecs_vd_drivers[i]->info->short_name);
					break;
				}
		}

		{
			...
			for (i = 0; mpcodecs_ad_drivers[i]; i++)
				...
				{
					listSet(&audio_fm_list, (char*)mpcodecs_ad_drivers[i]->info->short_name);
					break;
				}
		}
	...
	/* 6th page */
		...
		if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(CBCache)))
		{
			gtkCacheSize = (int)SBCacheadj->value;
			gtkCacheOn = 1;
		}
		else gtkCacheOn = 0;

		if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(CBAutoSync)))
		{
			gtkAutoSync = (int)SBAutoSyncadj->value;
			gtkAutoSyncOn = 1;
		}
...
GtkWidget* create_Preferences(void)
{
	...
	gtk_table_attach(GTK_TABLE(table1), HSPanscan, 1, 2, 0, 1, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	...
	gtk_table_attach(GTK_TABLE(table1), HSFPS, 1, 2, 1, 2, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), HSSubDelay, 1, 2, 0, 1, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);

	HSSubPositionadj = GTK_ADJUSTMENT(gtk_adjustment_new(100, 0, 100, 1, 0, 0));
	HSSubPosition = AddHScaler(HSSubPositionadj, NULL, 0);
	gtk_table_attach(GTK_TABLE(table1), HSSubPosition, 1, 2, 1, 2, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), HSSubFPS, 1, 2, 2, 3, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	...
	if (guiInfo.sh_video && guiInfo.Playing) HSPPQualityadj = GTK_ADJUSTMENT(
		gtk_adjustment_new(0, 0, get_video_quality_max(guiInfo.sh_video), 0, 0, 0));
	...
	gtk_table_attach(GTK_TABLE(table1), prEDVDDevice, 1, 2, 0, 1, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table1), prECDRomDevice, 1, 2, 1, 2, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
...
GtkWidget* create_AudioConfig(void)
{
	...
	gtk_table_attach(GTK_TABLE(table), CBAudioDevice, 1, 2, 0, 1, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	...
	gtk_table_attach(GTK_TABLE(table), CBAudioMixer, 1, 2, 1, 2, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	items = g_list_append(items, (gpointer)MSGTR_PREFERENCES_DriverDefault);
#ifdef CONFIG_OSS_AUDIO
	if (strncmp(ao_driver[0], "oss", 3) == 0)
	...
	gtk_table_attach(GTK_TABLE(table), CBAudioMixerChannel, 1, 2, 2, 3, (GtkAttachOptions)(GTK_EXPAND | GTK_FILL),
	                 (GtkAttachOptions)(0), 0, 0);
	items = g_list_append(items, (gpointer)MSGTR_PREFERENCES_DriverDefault);
#ifdef CONFIG_OSS_AUDIO
	if (strncmp(ao_driver[0], "oss", 3) == 0)
...
static void dxr3Button(GtkButton* button, gpointer user_data)
{
	...
	case 0: // Ok
		nfree(gtkDXR3Device);
		gtkDXR3Device = strdup(gtk_entry_get_text(GTK_ENTRY(CEDXR3Device)));
