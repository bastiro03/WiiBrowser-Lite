...
static void dump_exports(HMODULE hModule)
{
	...
	IMAGE_EXPORT_DIRECTORY* pe_exports = RVA(rva_start);
...
static void do_relocations(unsigned int load_addr, IMAGE_BASE_RELOCATION* r)
{
	...
	{
		char* page = RVA(r->VirtualAddress);
...
HMODULE PE_LoadImage(int handle, LPCSTR filename, WORD* version)
{
	...
	if (nt->FileHeader.Machine != IMAGE_FILE_MACHINE_I386)
	{
		...
		{
		case IMAGE_FILE_MACHINE_UNKNOWN: dbg_printf("Unknown");
			break;
		case IMAGE_FILE_MACHINE_I860: dbg_printf("I860");
			break;
		case IMAGE_FILE_MACHINE_R3000: dbg_printf("R3000");
			break;
		case IMAGE_FILE_MACHINE_R4000: dbg_printf("R4000");
			break;
		case IMAGE_FILE_MACHINE_R10000: dbg_printf("R10000");
			break;
		case IMAGE_FILE_MACHINE_ALPHA: dbg_printf("Alpha");
			break;
		case IMAGE_FILE_MACHINE_POWERPC: dbg_printf("PowerPC");
			break;
		default: dbg_printf("Unknown-%04x", nt->FileHeader.Machine);
			break;
		}
		dbg_printf(")\n");
		goto error;
	...
	*PE_HEADER(load_addr) = *nt;
	...
	if (FILE_dommap(handle, (void*)load_addr, 0, nt->OptionalHeader.SizeOfHeaders,
	...
	for (i = 0; i < nt->FileHeader.NumberOfSections; i++, pe_sec++)
	{
		...
		if (FILE_dommap(unix_handle, RVA(pe_sec->VirtualAddress),
		                0, pe_sec->SizeOfRawData, 0, pe_sec->PointerToRawData,
		                PROT_EXEC | PROT_WRITE | PROT_READ,
		                MAP_PRIVATE | MAP_FIXED) != RVA(pe_sec->VirtualAddress))
		{
	...
	if (reloc)
		do_relocations(load_addr, RVA(reloc));
