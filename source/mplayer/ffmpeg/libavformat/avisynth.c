		...
		if (AVIFileGetStream(avs->file, &stream->handle, 0, id) == S_OK)
		{
			if (AVIStreamInfo(stream->handle, &stream->info, sizeof(stream->info)) == S_OK)
			{
				...
				else if (stream->info.fccType == streamtypeVIDEO)
				{
					...
					st->codec->bit_rate = (uint64_t)stream->info.dwSampleSize * (uint64_t)stream->info.dwRate * 8 / (
...
static int avisynth_read_packet(AVFormatContext* s, AVPacket* pkt)
{
	...
	res = AVIStreamRead(stream->handle, stream->read, stream->chunck_samples, pkt->data, stream->chunck_size,
	                    &read_size, NULL);
