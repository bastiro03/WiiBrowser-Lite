...
static void pred_temp_direct_motion(H264Context* const h, int* mb_type)
{
...

	{
		...
		//if(h->ref_list[1][0].mbaff && IS_INTERLACED(mb_type_col[0])) ref_offset=16 else 0
		...
		if (IS_16X16(*mb_type))
		{
			...
			else
			{
				const int ref0 = l1ref0[0] >= 0
					                 ? map_col_to_list0[0][l1ref0[0] + ref_offset]
		...
		else
		{
			for (i8 = 0; i8 < 4; i8++)
			{
				...
				if (IS_SUB_8X8(sub_mb_type))
				{
					...
					fill_rectangle(&h->mv_cache[0][scan8[i8 * 4]], 2, 2, 8, pack16to32(mx, my), 4);
					fill_rectangle(&h->mv_cache[1][scan8[i8 * 4]], 2, 2, 8, pack16to32(mx - mv_col[0], my - mv_col[1]),
					               4);
				}
				else
					for (i4 = 0; i4 < 4; i4++)
