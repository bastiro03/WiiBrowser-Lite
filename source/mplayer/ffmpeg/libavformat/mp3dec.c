...
static int mp3_read_probe(AVProbeData* p)
{
	...
	if (max_frames > 200)return AVPROBE_SCORE_MAX / 2;
	if (max_frames >= 4) return AVPROBE_SCORE_MAX / 4;
	if (ff_id3v2_match(buf0, ID3v2_DEFAULT_MAGIC) && 2 * ff_id3v2_tag_len(buf0) >= p->buf_size)
