...
void update_subtitles(sh_video_t* sh_video, double refpts, demux_stream_t* d_dvdsub, int reset)
{
		...
		(vobsub_id >= 0 || type == 'v'))
	{
		...
		while (1)
		{
			...
			{
				// DVD sub
				len = ds_get_packet_sub(d_dvdsub, &packet, NULL, NULL);
				if (len > 0)
				{
	...
	else if (is_text_sub(type) || is_av_sub(type) || type == 'd' || type == 'c')
	{
		...
		while (1)
		{
			...
			if (subpts != MP_NOPTS_VALUE)
			{
				...
				if (type == 'a')
				{
					// ssa/ass subs without libass => convert to plaintext
					int i;
...
int select_audio(demuxer_t* demuxer, int audio_id, char* audio_lang)
{
	...
	if (audio_id == -2)
	{
		// some demuxers don't yet know how to switch to no sound
		demuxer->audio->id = -2;
