...
//// constants
static const unsigned int colors = 256;
static const unsigned int maxcolor = 255;
static const unsigned base = 256;
static const unsigned first_char = 33;
...
void render_one_glyph(font_desc_t* desc, int c)
{
	...
	{
		int newsize = desc->pic_b[font]->charwidth * desc->pic_b[font]->charheight * (desc->pic_b[font]->current_alloc +
			ALLOC_INCR);
...
static int generate_tables(font_desc_t* desc, double thickness, double radius)
{
	...
	if (desc->tables.g_r)
	{
		...
		for (volume_diff = 10000000; volume_diff > 0.0000001; volume_diff *= 0.5)
		{
			...
			{
				desc->tables.g[i] = (unsigned)(exp(A * (i - desc->tables.g_r) * (i - desc->tables.g_r)) * volume_factor
					+ .5);
	...
	for (my = 0; my < desc->tables.o_w; ++my)
	{
		for (mx = 0; mx < desc->tables.o_w; ++mx)
		{
			...
			double d = thickness + 1 - sqrt(
				(mx - desc->tables.o_r) * (mx - desc->tables.o_r) + (my - desc->tables.o_r) * (my - desc->tables.o_r));
...
font_desc_t* read_font_desc_ft(const char* fname, int face_index, int movie_width, int movie_height,
