...
static int get_packet_size(const uint8_t* buf, int size)
{
	...
	if (dvhs_score > score && dvhs_score > fec_score) return TS_DVHS_PACKET_SIZE;
	if (score < fec_score && dvhs_score < fec_score) return TS_FEC_PACKET_SIZE;
	return -1;
}
...
static void m4sl_cb(MpegTSFilter* filter, const uint8_t* section, int section_len)
{
	...
	mp4_read_od(s, p, p_end - p, mp4_descr, &mp4_descr_count, MAX_MP4_DESCR_COUNT);
...
#define CHECK_COUNT 10
	...
	dvhs_score = analyze(p->buf, TS_DVHS_PACKET_SIZE * check_count, TS_DVHS_PACKET_SIZE, NULL) * CHECK_COUNT /
		check_count;
	fec_score = analyze(p->buf, TS_FEC_PACKET_SIZE * check_count, TS_FEC_PACKET_SIZE, NULL) * CHECK_COUNT /
		check_count;
	//    av_log(NULL, AV_LOG_DEBUG, "score: %d, dvhs_score: %d, fec_score: %d \n", score, dvhs_score, fec_score);

	// we need a clear definition for the returned score otherwise things will become messy sooner or later
	if (score > fec_score && score > dvhs_score && score > 6) return AVPROBE_SCORE_MAX + score - CHECK_COUNT;
	if (dvhs_score > score && dvhs_score > fec_score && dvhs_score > 6) return AVPROBE_SCORE_MAX + dvhs_score -
		CHECK_COUNT;
	if (fec_score > 6) return AVPROBE_SCORE_MAX + fec_score - CHECK_COUNT;
	return -1;
#else
                                        ...
                                        int64_t* ppos, int64_t pos_limit)
{
	...
	pos = ((*ppos + ts->raw_packet_size - 1 - ts->pos47) / ts->raw_packet_size) * ts->raw_packet_size + ts->
		pos47;
	while (pos < pos_limit)
	{
