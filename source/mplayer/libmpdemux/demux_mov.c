...
static void mov_build_index(mov_track_t* trak, int timescale)
{
	...
	if (trak->editlist_size > 0)
	{
		...
		for (i = 0; i < trak->editlist_size; i++)
		{
			...
			el->pts_offset = ((long long)e_pts * (long long)trak->timescale) / (long long)timescale - trak->samples[
				sample].pts;
		...
		case MOV_FOURCC('f', 't', 'y', 'p'):
			{
				...
				case MOV_FOURCC('m', 'm', 'p', '4'):
					mp_msg(MSGT_DEMUX, MSGL_INFO,
					       "ISO: File Type Major Brand: Mobile ISO/IEC 14496-1 (MPEG-4 system)\n");
					break;
				default:
					tmp = be2me_32(tmp);
...
static int gen_sh_audio(sh_audio_t* sh, mov_track_t* trak, int timescale)
{
	...
	if (trak->stdata_len >= 36 + adjust)
	{
		...
		case MOV_FOURCC('e', 's', 'd', 's'):
			{
				...
				if (atom_len > 8)
				{
					...
					if (!mp4_parse_esds(&trak->stdata[36 + adjust], atom_len - 8, &esds))
					{
						...
						if (esds.objectTypeId == 0xdd && esds.streamType == 0x15 && sh->format == 0x6134706D && esds.
							decoderConfigLen > 8)
						{
							//vorbis audio
							unsigned char* buf[3];
							...
							for (k = 0; k < 3; k++)
							{
								...
								if (offset + sizes[k] > esds.decoderConfigLen)
								{
									mp_msg(MSGT_DEMUX, MSGL_FATAL,
									       "MOV: ERROR!, not enough vorbis extradata to read: offset = %d, k=%d, size=%d, len: %d\n",
						...
						if (esds.objectTypeId == MP4OTI_13kVoice)
						{
							// 13K Voice, defined by 3GPP2
		...
		case MOV_FOURCC('d', 'a', 'm', 'r'):
			mp_msg(MSGT_DEMUX, MSGL_V, "MOV: Found AMR audio atom %c%c%c%c (%d)!\n", trak->stdata[32 + adjust],
			       trak->stdata[33 + adjust], trak->stdata[34 + adjust], trak->stdata[35 + adjust], atom_len);
			if (atom_len > 14)
			{
				mp_msg(MSGT_DEMUX, MSGL_V, "mov: vendor: %c%c%c%c Version: %d\n", trak->stdata[36 + adjust],
				       trak->stdata[37 + adjust], trak->stdata[38 + adjust], trak->stdata[39 + adjust],
				       trak->stdata[40 + adjust]);
				mp_msg(MSGT_DEMUX, MSGL_V, "MOV: Modes set: %02x%02x\n", trak->stdata[41 + adjust],
				       trak->stdata[42 + adjust]);
			...
			(pos + (atom_len = char2int(trak->stdata, pos))) <= trak->stdata_len)
		{
			...
			// on to the decoder ::atmos
				if (atom_len > 8)
				{
					...
					{
						if (esds.objectTypeId == MP4OTI_MPEG2VisualSimple || esds.objectTypeId ==
							MP4OTI_MPEG2VisualMain ||
							esds.objectTypeId == MP4OTI_MPEG2VisualSNR || esds.objectTypeId ==
							MP4OTI_MPEG2VisualSpatial ||
							esds.objectTypeId == MP4OTI_MPEG2VisualHigh || esds.objectTypeId ==
							MP4OTI_MPEG2Visual422)
			...
			case MOV_FOURCC('d', '2', '6', '3'):
				mp_msg(MSGT_DEMUX, MSGL_V, "MOV: Found H.263 decoder atom %c%c%c%c (%d)!\n", trak->stdata[pos + 4],
				       trak->stdata[pos + 5], trak->stdata[pos + 6], trak->stdata[pos + 7], atom_len);
				if (atom_len > 10)
					mp_msg(MSGT_DEMUX, MSGL_V, "MOV: Vendor: %c%c%c%c H.263 level: %d H.263 profile: %d \n",
					       trak->stdata[pos + 8], trak->stdata[pos + 9], trak->stdata[pos + 10],
					       trak->stdata[pos + 11], trak->stdata[pos + 12], trak->stdata[pos + 13]);
				break;
			case 0:
				break;
			default:
						...
						static demuxer_t* mov_read_header(demuxer_t* demuxer)
						{
							...
							if (!stream_seek(demuxer->stream, priv->moov_start))
							{
								mp_msg(MSGT_DEMUX, MSGL_ERR,
								       "MOV: Cannot seek to the beginning of the Movie header (0x%"PRIx64")\n",
							...
							mp_msg(MSGT_DEMUX, MSGL_V,
							       "MOV: longest streams: A: #%d (%d samples)  V: #%d (%d samples)\n",
							...
							if (demuxer->sub->id >= 0)
							{
								...
								{
									mp_msg(MSGT_DEMUX, MSGL_ERR,
									       "MOV: selected subtitle stream (%d) does not exist\n", demuxer->sub->id);
									demuxer->sub->id = -2;
								}
							}
		...
		demuxer->stream->eof = 0;
						...
						static int demux_mov_fill_buffer(demuxer_t* demuxer, demux_stream_t* ds)
						{
							...
							if (trak->samplesize)
							{
								...
								pts = (float)(trak->chunks[trak->pos].sample * trak->duration) / (float)trak->
									timescale;
								if (trak->samplesize != 1)
								{
									mp_msg(MSGT_DEMUX, MSGL_DBG2, "WARNING! Samplesize(%d) != 1\n",
									       trak->samplesize);
									if ((trak->fourcc != MOV_FOURCC('t', 'w', 'o', 's')) && (trak->fourcc !=
										MOV_FOURCC('s', 'o', 'w', 't')))
								...
								{
									if (trak->stdata_len >= 44 && trak->stdata[9] >= 1 && char2int(trak->stdata, 28)
										> 0)
									{
						...
						static float mov_seek_track(mov_track_t* trak, float pts, int flags)
						{
							...
							if (flags & SEEK_FACTOR) pts *= trak->length;
							else pts *= (float)trak->timescale;
							...
							else
							{
								...
								if (trak->keyframes_size)
								{
									...
									if (i > 0 && (trak->keyframes[i] - trak->pos) > (trak->pos - trak->keyframes[i -
										1]))
										--i;
