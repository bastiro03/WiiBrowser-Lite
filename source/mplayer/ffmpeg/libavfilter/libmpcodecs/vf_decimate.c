...
static int diff_to_drop_plane(int hi, int lo, float frac, unsigned char* old, unsigned char* new, int w, int h, int os,
                              int ns)
{
	int x, y;
	int d, c = 0;
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (diff_to_drop(vf->priv->hi, vf->priv->lo, vf->priv->frac, dmpi, mpi))
	{
		if (vf->priv->max == 0)
			return 0;
		if ((vf->priv->max > 0) && (vf->priv->cnt++ < vf->priv->max))
			return 0;
		if ((vf->priv->max < 0) && (vf->priv->last + 1 >= -vf->priv->max))
