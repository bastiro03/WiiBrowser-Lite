...
int dnxhd_decode_init(AVCodecContext* avctx)
{
...
static int dnxhd_init_vlc(DNXHDContext* ctx, int cid)
{
	if (cid != ctx->cid)
	{
		...
		{
			av_log(ctx->avctx, AV_LOG_ERROR, "bit depth mismatches %d %d\n", ff_dnxhd_cid_table[index].bit_depth,
			       ctx->bit_depth);
...
void dnxhd_decode_dct_block(DNXHDContext* ctx,
                            DCTELEM* block, int n,
                            int qscale,
                            int index_bits,
                            int level_bias,
...
int dnxhd_decode_close(AVCodecContext* avctx)
{
	DNXHDContext* ctx = avctx->priv_data;
