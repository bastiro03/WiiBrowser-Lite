                                 ...
                                 int* got_frame_ptr, AVPacket* avpkt)
{
	...
	if (!esc->samples && avpkt)
	{
		...
		esc->samples_size = avctx->codec->id == CODEC_ID_8SVX_RAW || avctx->codec->id == CODEC_ID_PCM_S8_PLANAR
			                    ? avpkt->size
			                    : avctx->channels + (avpkt->size - avctx->channels) * 2;
	...
	/* get output buffer */
	esc->frame.nb_samples = (FFMIN(MAX_FRAME_SIZE, esc->samples_size - esc->samples_idx) + avctx->channels - 1) / avctx
		->channels;
...
int eightsvx_decode_init(AVCodecContext* avctx)
{
	EightSvxContext* esc = avctx->priv_data;
	...
	{
	case CODEC_ID_8SVX_FIB: esc->table = fibonacci;
		break;
	case CODEC_ID_8SVX_EXP: esc->table = exponential;
		break;
	case CODEC_ID_PCM_S8_PLANAR:
	case CODEC_ID_8SVX_RAW: esc->table = NULL;
		break;
	default:
...
int eightsvx_decode_close(AVCodecContext* avctx)
{
	EightSvxContext* esc = avctx->priv_data;
