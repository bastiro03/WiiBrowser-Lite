...
extern int _wtoi(const wchar_t* str);
...
namespace litehtml
{
	...
	scanner::token_type scanner::scan_head()
	{
		...
		if (c == '>')
		{
			if (tag_name_length == 6 && !wcsncasecmp(tag_name, L"script", tag_name_length))
			{
	...
	} g_HTMLCodes[] =
	{
		...
		{L"&euro;", L'€'},
		{L"&trade;", L'™'},
		{{static_cast<char>(0x97), static_cast<char>(0x00)}, L'-'},
		{{static_cast<char>(0xA0), static_cast<char>(0x00)}, L' '},
		{L"", 0}
	};

	wchar scanner::resolve_entity(const wchar* buf, int buf_size)
	{
		if (buf[0] == '#')
		{
			if (buf[1] == 'x' || buf[1] == 'X')
			{
				wchar* end = nullptr;
				return static_cast<wchar>(wcstol(buf + 2, &end, 16));
			}
			return static_cast<wchar>(_wtoi(buf + 1));
		}
