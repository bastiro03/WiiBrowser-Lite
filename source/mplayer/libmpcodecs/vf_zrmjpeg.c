...
void zr_mjpeg_encode_mb(jpeg_enc_t* j)
{
...
void fill_block(jpeg_enc_t* j, int x, int y,
		...
		for (mb_x = 0; mb_x < j->s->mb_width; mb_x++)
		{
			...
			j->s->block_last_index[0] =
				j->s->dct_quantize(j->s, j->s->block[0],
				                   0, 8, &overflow);
			if (overflow)
				clip_coeffs(j->s, j->s->block[0],
				            j->s->block_last_index[0]);
			j->s->block_last_index[1] =
				j->s->dct_quantize(j->s, j->s->block[1],
				                   1, 8, &overflow);
			if (overflow)
				clip_coeffs(j->s, j->s->block[1],
				            j->s->block_last_index[1]);
                  ...
                  int d_height, unsigned int flags, unsigned int outfmt)
{
	...
	if (aspect_decision > 1.8 && aspect_decision < 2.2)
	{
		...
		{
			WARNING(
				"unable to correct aspect by stretching, because resulting X will be too large, aspect correction by decimating y not yet implemented\n");
			d_width = width;
			d_height = height;
		}
		/* prestretch movie */
