       ...
       uint32_t format)
{
	...
	if (use_bes)
	{
		...
		mp_msg(MSGT_VO, MSGL_INFO, "vo_dfbmga: BES using %s buffering\n",
		       dlc.buffermode == DLBM_TRIPLE ? "triple" : dlc.buffermode == DLBM_BACKVIDEO ? "double" : "single");
		mp_msg(MSGT_VO, MSGL_INFO, "vo_dfbmga: BES surface %dx%d %s\n", dlc.width, dlc.height,
		       pixelformat_name(dlc.pixelformat));
	}
	...
	mp_msg(MSGT_VO, MSGL_INFO, "vo_dfbmga: Sub-picture surface %dx%d %s (%s)\n",
	       ...
	       use_crtc2 ? (use_spic ? "Sub-picture layer" : "CRTC2") : use_crtc1 ? "CRTC1" : "BES");
