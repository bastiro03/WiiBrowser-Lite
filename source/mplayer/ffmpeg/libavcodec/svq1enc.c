...
static void svq1_write_header(SVQ1Context* s, int frame_type)
{
	...
	if (frame_type == AV_PICTURE_TYPE_I)
	{
		...
		i = ff_match_2uint16(ff_svq1_frame_size_table, FF_ARRAY_ELEMS(ff_svq1_frame_size_table), s->frame_width,
		                     s->frame_height);
		put_bits(&s->pb, 3, i);
...
static int encode_block(SVQ1Context* s, uint8_t* src, uint8_t* ref, uint8_t* decoded, int stride, int level,
                        int threshold, int lambda, int intra)
{
	...
	if (best_score > threshold && level)
	{
		...
		score += encode_block(s, src + offset, ref + offset, decoded + offset, stride, level - 1, threshold >> 1,
		                      lambda, intra);
		score += lambda;
...
static int svq1_encode_plane(SVQ1Context* s, int plane, unsigned char* src_plane, unsigned char* ref_plane,
                             ...
                             int width, int height, int src_stride, int stride)
{
	...
	for (y = 0; y < block_height; y++)
	{
		...
		for (x = 0; x < block_width; x++)
		{
			...
			if (s->picture.pict_type == AV_PICTURE_TYPE_I || (s->m.mb_type[x + y * s->m.mb_stride] &
...
int svq1_encode_init(AVCodecContext* avctx)
{
                             ...
                             const AVFrame* pict, int* got_packet)
{
	...
	if ((ret = ff_alloc_packet2(
		avctx, pkt, s->y_block_width * s->y_block_height * MAX_MB_BYTES * 3 + FF_MIN_BUFFER_SIZE) < 0))
		return ret;
...
int svq1_encode_end(AVCodecContext* avctx)
{
	SVQ1Context* const s = avctx->priv_data;
	int i;
