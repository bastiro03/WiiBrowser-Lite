...
void read_avi_header(demuxer_t* demuxer, int index_mode)
{
	...
	while (1)
	{
		...
		// "Seattle from Above."
		case mmioFOURCC('I', 'N', 'A', 'M'): hdr = "Title";
			break;
		...
		case ckidSTREAMHEADER:
			{
				...
				if (h.fccType == streamtypeVIDEO)
				{
					...
					sh_video->stream_delay = (float)sh_video->video.dwStart * sh_video->video.dwScale / sh_video->video.
						dwRate;
				}
				else if (h.fccType == streamtypeAUDIO)
				{
					...
					sh_audio->stream_delay = (float)sh_audio->audio.dwStart * sh_audio->audio.dwScale / sh_audio->audio.
						dwRate;
				...
				*(uint32_t*)s->dwChunkId = stream_read_dword_le(demuxer->stream);
				...
				for (i = 0; i < s->nEntriesInUse; i++)
				{
					...
					mp_msg(MSGT_HEADER, MSGL_V, "ODML (%.4s): [%d] 0x%016"PRIx64" 0x%04x %u\n",
					       s->dwChunkId, i,
					       s->aIndex[i].qwOffset, s->aIndex[i].dwSize, s->aIndex[i].dwDuration);
				}
		...
		mp_msg(MSGT_HEADER, MSGL_DBG2,
		       "list_end=0x%"PRIX64"  pos=0x%"PRIX64"  chunksize=0x%"PRIX64"  next=0x%"PRIX64"\n",
		...
		if (list_end > 0 &&
			chunksize + stream_tell(demuxer->stream) == list_end)
			list_end = 0;
		...
		else if (chunksize > 0) stream_skip(demuxer->stream, chunksize);
		else
...
gen_index:
	if (index_mode >= 2 || (priv->idx_size == 0 && index_mode == 1))
	{
		...
		while (1)
		{
			...
			if (idxfix_divx)
				if (avi_stream_id(id) == idxfix_videostream)
				{
					...
					case 1: if (c & 0x40000000) idx->dwFlags &= ~AVIIF_KEYFRAME;
						break; // divx 3
					case 2: if (c == 0x1B6) idx->dwFlags &= ~AVIIF_KEYFRAME;
						break; // divx 4
					}
				}
		...
		if (index_file_save)
		{
			...
			{
				mp_msg(MSGT_HEADER, MSGL_ERR, MSGTR_MPDEMUX_AVIHDR_Failed2WriteIdxFile, index_file_save,
				       strerror(errno));
				return;
			}
