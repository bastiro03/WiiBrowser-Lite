...
public:
	...
	{
	public:
		bool operator()(const iterator_base& one,
...
iter tree<T, tree_node_allocator>::move_ontop(iter target, iter source)
{
	...
	// connect it to the new point
	if (b_prev_sibling != nullptr) b_prev_sibling->next_sibling = src;
	else b_parent->first_child = src;
	if (b_next_sibling != nullptr) b_next_sibling->prev_sibling = src;
...
typename tree<T, tree_node_allocator>::sibling_iterator tree<T, tree_node_allocator>::child(
	const iterator_base& it, unsigned int num) const
{
	...
	T, tree_node_allocator>::fixed_depth_iterator::operator--()
{
	...
	else
	{
		...
		do
		{
			...
			if (par == nullptr)
			{
				// FIXME: need to keep track of this!
				this->node = 0;
				return *this;
			}
		}
...
template <class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::fixed_depth_iterator& tree<
	T, tree_node_allocator>::fixed_depth_iterator::operator-=(unsigned int num)
{
	while (num > 0)
	{
		--(*this);
