...
static int control(struct af_instance_s* af, int cmd, void* arg)
{
	...
	case AF_CONTROL_VOLUME_LEVEL | AF_CONTROL_SET:
		return af_from_dB(AF_NCH, arg, s->level, 20.0, -200.0, 60.0);
	case AF_CONTROL_VOLUME_LEVEL | AF_CONTROL_GET:
		return af_to_dB(AF_NCH, s->level, arg, 20.0);
	case AF_CONTROL_VOLUME_PROBE | AF_CONTROL_GET:
		return af_to_dB(AF_NCH, s->pow, arg, 10.0);
	case AF_CONTROL_VOLUME_PROBE_MAX | AF_CONTROL_GET:
		return af_to_dB(AF_NCH, s->max, arg, 10.0);
