...
DECLARE_ALIGNED(8, uint8_t, dither_8x8_128)[8][8] =
 {
...
DECLARE_ALIGNED(8, uint8_t, ff_sws_pb_64)[8] =
 {
...
void fillPlane(uint8_t* plane, int stride, int width,
                           ...
                           const int32_t* filterPos, int filterSize)
{
	...
	if ((isAnyRGB(c->srcFormat) || c->srcFormat == PIX_FMT_PAL8) && av_pix_fmt_descriptors[c->srcFormat].comp[0].
		depth_minus1 < 15)
		sh = 9;
                           ...
                           const int32_t* filterPos, int filterSize)
{
	...
	if (sh < 15)
		sh = isAnyRGB(c->srcFormat) || c->srcFormat == PIX_FMT_PAL8
			     ? 13
...
void hyscale(SwsContext* c, int16_t* dst, int dstWidth,
...
void hcscale(SwsContext* c, int16_t* dst1,
                   ...
                   int srcSliceH, uint8_t* dst[], int dstStride[])
{
	...
	if ((int)dst[0] % 16 || (int)dst[1] % 16 || (int)dst[2] % 16 || (int)src[0] % 16 || (int)src[1] % 16 || (int)src[2]
		% 16
	...
	for (; dstY < dstH; dstY++)
	{
...

		{
			...
			const int16_t** chrUSrcPtr = (const int16_t**)(void*)chrUPixBuf + chrBufIndex + firstChrSrcY - lastInChrBuf
				+ vChrBufSize;
			const int16_t** chrVSrcPtr = (const int16_t**)(void*)chrVPixBuf + chrBufIndex + firstChrSrcY - lastInChrBuf
				+ vChrBufSize;
			const int16_t** alpSrcPtr = (CONFIG_SWSCALE_ALPHA && alpPixBuf)
				                            ? (const int16_t**)(void*)alpPixBuf + lumBufIndex + firstLumSrcY -
				                            lastInLumBuf + vLumBufSize
				                            : NULL;
			int16_t* vLumFilter = c->vLumFilter;
			int16_t* vChrFilter = c->vChrFilter;
...
void sws_init_swScale_c(SwsContext* c)
{
	...
	{
		c->hyScale = c->hcScale = c->dstBpc > 10
			                          ? hScale16To19_c
			                          : hScale16To15_c;
	}
