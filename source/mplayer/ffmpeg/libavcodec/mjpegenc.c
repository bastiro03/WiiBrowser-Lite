...
void ff_mjpeg_encode_picture_header(MpegEncContext* s)
{
	...
	{
	case CODEC_ID_MJPEG: put_marker(&s->pb, SOF0);
		break;
	case CODEC_ID_LJPEG: put_marker(&s->pb, SOF3);
		break;
	default: assert(0);
	}
	...
	switch (s->avctx->codec_id)
	{
	case CODEC_ID_MJPEG: put_bits(&s->pb, 8, 63);
		break; /* Se (not used) */
	case CODEC_ID_LJPEG: put_bits(&s->pb, 8, 0);
		break; /* not used */
	default: assert(0);
	}
