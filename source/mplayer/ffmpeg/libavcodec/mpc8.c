...
int mpc8_decode_init(AVCodecContext* avctx)
{
	int i;
	...
	for (i = 0, off = 0; i < maxband; i++, off += SAMPLES_PER_BAND)
	{
		for (ch = 0; ch < 2; ch++)
		{
			...
			case 8:
				...
				for (j = 0; j < SAMPLES_PER_BAND; j++)
				{
					t = get_vlc2(gb, quant_vlc[res - 5][cnt > mpc8_thres[res]].table,
					             quant_vlc[res - 5][cnt > mpc8_thres[res]].bits, 2) + quant_offsets[res - 5];
					c->Q[ch][off + j] = t;
