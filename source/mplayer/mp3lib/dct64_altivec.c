...
void dct64_altivec(real* a, real* b, real* c)
{
	...
	const vector float vczero = (const vector
float
	)
	FOUROF(0.);
	const vector unsigned char reverse = (const vector
unsigned char
	)
	vcprm(3, 2, 1, 0);
	...
	// have been removed, all elements are stored inside b1vX (b2vX)
	{
...

		{
...

			{
				...
				vector
				float costabv1r, costabv1, costabv2;
				vector
				unsigned char costab_perm = vec_lvsl(0, costab);
	...
	{
		register const real cos0 = mp3lib_pnts[3][0];
		register const real cos1 = mp3lib_pnts[3][1];
	...
	{
		register const real cos0 = mp3lib_pnts[4][0];
