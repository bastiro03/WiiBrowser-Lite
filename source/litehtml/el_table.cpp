...
litehtml::el_table::el_table(document* doc) : element(doc)
{
}
...
int litehtml::el_table::render(int x, int y, int max_width)
{
	...
	if (!m_css_width.is_predefined())
	{
		...
		if (table_width > min_table_width)
		{
			...
			m_grid.distribute_width(table_width - min_table_width, 0,
			                        m_grid.cols_count() - 1/*, &table_column_accessor_width()*/);
		}
		else
		{
			for (int col2 = 0; col2 < m_grid.cols_count(); col2++)
	...
	for (int row = 0; row < m_grid.rows_count(); row++)
	{
		...
		for (int col = 0; col < m_grid.cols_count(); col++)
		{
			...
			{
				cell->el->m_pos.height = max_height - cell->el->content_margins_top() - cell->el->
			...
			for (int col = 0; col < m_grid.cols_count(); col++)
			{
				...
				if (cell->el)
				{
					if (cell->rowspan > 1)
					{
						...
						{
							cell->el->m_pos.height = cell_height - cell->el->content_margins_top() - cell->el->
								content_margins_bottom();
						}
						else
						{
							m_grid.row(std::min(row + cell->rowspan - 1, m_grid.rows_count() - 1)).height += cell->el->
								height() - cell_height;
...
bool litehtml::el_table::appendChild(element* el)
{
	if (!el) return false;
...
void litehtml::el_table::parse_styles(bool is_reparse)
{
	...
	if (str)
	{
		...
		{
		case 1:
			m_style.add_property(L"margin-left", L"auto", nullptr);
			m_style.add_property(L"margin-right", L"auto", nullptr);
			break;
		case 2:
			m_style.add_property(L"margin-left", L"auto", nullptr);
			m_style.add_property(L"margin-right", L"0", nullptr);
			break;
		}
		m_style.add_property(L"width", str, nullptr);
	}
	...
	if (str)
	{
		...
		val += L" ";
		val += str;
		m_style.add_property(L"border-spacing", val.c_str(), nullptr);
	}
...
void litehtml::el_table::init()
{
	...
	auto t = new go_inside_table();
	auto r = new table_rows_selector();

	elements_iterator row_iter(this, t, r);
	...
	while (row)
	{
		...
		auto c = new table_cells_selector();

		elements_iterator cell_iter(row, t, c);
