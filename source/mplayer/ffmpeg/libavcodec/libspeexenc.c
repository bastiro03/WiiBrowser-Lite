...
void print_enc_params(AVCodecContext* avctx,
                      LibSpeexEncContext* s)
{
	...
	{
	case SPEEX_MODEID_NB: mode_str = "narrowband";
		break;
	case SPEEX_MODEID_WB: mode_str = "wideband";
		break;
	case SPEEX_MODEID_UWB: mode_str = "ultra-wideband";
		break;
	}
...
int encode_init(AVCodecContext* avctx)
{
	...
	{
	case 8000: mode = &speex_nb_mode;
		break;
	case 16000: mode = &speex_wb_mode;
		break;
	case 32000: mode = &speex_uwb_mode;
		break;
	default:
...
int encode_close(AVCodecContext* avctx)
{
	LibSpeexEncContext* s = avctx->priv_data;
...
static const AVOption options[] = {
	...
	{
		"frames_per_packet", "Number of frames to encode in each packet", OFFSET(frames_per_packet), AV_OPT_TYPE_INT,
		{1}, 1, 8, AE
	},
	{NULL},
};
