...
static inline void decompose2D(float* dstL, float* dstH, float* src, int xstride, int ystride, int step, int w, int h)
{
	...
	for (y = 0; y < h; y++)
		for (x = 0; x < step; x++)
			decompose(dstL + ystride * y + xstride * x, dstH + ystride * y + xstride * x,
			          src + ystride * y + xstride * x, step * xstride, (w - x + step - 1) / step);
}
...
static inline void compose2D(float* dst, float* srcL, float* srcH, int xstride, int ystride, int step, int w, int h)
{
	...
	for (y = 0; y < h; y++)
		for (x = 0; x < step; x++)
			compose(dst + ystride * y + xstride * x, srcL + ystride * y + xstride * x, srcH + ystride * y + xstride * x,
			        step * xstride, (w - x + step - 1) / step);
}
                   ...
                   int height, int is_luma)
{
	...
	for (y = 0; y < height; y++)
		for (x = 0; x < width; x++)
		{
			i = p->plane[0][0][x + y * p->stride] + dither[x & 7][y & 7] * (1.0 / 64) + 1.0 / 128;
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	filter(vf->priv, dmpi->planes[1], mpi->planes[1], dmpi->stride[1], mpi->stride[1], mpi->w >> mpi->chroma_x_shift,
	       mpi->h >> mpi->chroma_y_shift, 0);
	filter(vf->priv, dmpi->planes[2], mpi->planes[2], dmpi->stride[2], mpi->stride[2], mpi->w >> mpi->chroma_x_shift,
	       mpi->h >> mpi->chroma_y_shift, 0);
...
static int vf_open(vf_instance_t* vf, char* args)
{
	...
	if (args)
		sscanf(args, "%d:%f:%f:%d:%f", &vf->priv->depth,
		       &vf->priv->strength[0],
		       &vf->priv->strength[1],
		       &vf->priv->mode,
		       &vf->priv->delta);
