	...
	for (reslevelno = 0; reslevelno < codsty->nreslevels; reslevelno++)
	{
		...
		for (bandno = 0; bandno < reslevel->nbands; bandno++, gbandno++)
		{
			...
			else
			{
				...
				for (i = 0; i < 2; i++)
					for (j = 0; j < 2; j++)
						band->coord[i][j] = ff_j2k_ceildivpow2(
							comp->coord[i][j] - (((bandno + 1 >> i) & 1) << declvl - 1), declvl);
			}
			band->cblknx = ff_j2k_ceildiv(band->coord[0][1], band->codeblock_width) - band->coord[0][0] / band->
				codeblock_width;
			band->cblkny = ff_j2k_ceildiv(band->coord[1][1], band->codeblock_height) - band->coord[1][0] / band->
				codeblock_height;
