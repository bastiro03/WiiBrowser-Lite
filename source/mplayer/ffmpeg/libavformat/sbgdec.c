...
#undef time
		time(&now0);
		tm = localtime(&now0);
		now = tm ? tm->tm_hour * 3600 + tm->tm_min * 60 + tm->tm_sec : now0 % DAY;
	...
	s->end_ts = s->opt_duration ? s->start_ts + s->opt_duration : AV_NOPTS_VALUE;
                            ...
                            struct ws_intervals* inter)
{
	...
	{
		edata_size += inter->inter[i].type == WS_SINE ? 44 : inter->inter[i].type == WS_NOISE ? 32 : 0;
		if (edata_size < 0)
			return AVERROR(ENOMEM);
	}
...
int sbg_read_probe(AVProbeData* p)
{
	int r, score;
	struct sbg_script script = {0};

	r = parse_script(NULL, p->buf, p->buf_size, &script);
	score = r < 0 || !script.nb_def || !script.nb_tseq ? 0 : AVPROBE_SCORE_MAX / 3;
	free_script(&script);
	return score;
}
...
int sbg_read_header(AVFormatContext* avf)
{
	...
	st->duration = script.end_ts == AV_NOPTS_VALUE
		               ? AV_NOPTS_VALUE
