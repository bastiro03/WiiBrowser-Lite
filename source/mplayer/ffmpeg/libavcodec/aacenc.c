		...
		for (w = 0; w < ics->num_windows * 16; w += 16)
		{
			for (g = 0; g < ics->num_swb; g++)
			{
				...
				if (cpe->common_window && !ch && cpe->ms_mask[w + g])
				{
					...
					{
						cpe->ch[0].coeffs[start + i] = (cpe->ch[0].coeffs[start + i] + cpe->ch[1].coeffs[start + i]) /
							2.0;
...
int aac_encode_end(AVCodecContext* avctx)
{
	AACEncContext* s = avctx->priv_data;
...
int dsp_init(AVCodecContext* avctx, AACEncContext* s)
{
	int ret = 0;
...
int alloc_buffers(AVCodecContext* avctx, AACEncContext* s)
{
	int ch;
...
int aac_encode_init(AVCodecContext* avctx)
{
	AACEncContext* s = avctx->priv_data;
	int i, ret = 0;
...
static const AVOption aacenc_options[] = {
	{
		"stereo_mode", "Stereo coding method", offsetof(AACEncContext, options.stereo_mode), AV_OPT_TYPE_INT,
		{.dbl = 0}, -1, 1, AACENC_FLAGS, "stereo_mode"
	},
	{
		"auto", "Selected by the Encoder", 0, AV_OPT_TYPE_CONST, {.dbl = -1}, INT_MIN, INT_MAX, AACENC_FLAGS,
		"stereo_mode"
	},
	{
		"ms_off", "Disable Mid/Side coding", 0, AV_OPT_TYPE_CONST, {.dbl = 0}, INT_MIN, INT_MAX, AACENC_FLAGS,
		"stereo_mode"
	},
	{
		"ms_force", "Force Mid/Side for the whole frame if possible", 0, AV_OPT_TYPE_CONST, {.dbl = 1}, INT_MIN,
		INT_MAX, AACENC_FLAGS, "stereo_mode"
	},
	{
		"aac_coder", "", offsetof(AACEncContext, options.aac_coder), AV_OPT_TYPE_INT, {.dbl = 2}, 0, AAC_CODER_NB - 1,
		AACENC_FLAGS
	},
	{NULL}
};
