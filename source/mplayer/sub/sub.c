...
static void draw_alpha_buf(mp_osd_obj_t* obj, int x0, int y0, int w, int h, unsigned char* src, unsigned char* srca,
                           int stride)
{
...
// renders char to a big per-object buffer where alpha and bitmap are separated
static void tt_draw_alpha_buf(mp_osd_obj_t* obj, int x0, int y0, int w, int h, unsigned char* src, int stride, int fg,
                              int bg, int alpha)
{
	int dststride = obj->stride;
	int dstskip = obj->stride - w;
			...
			if (!tc.gfx || (tc.flh && !flashon))
			{
					...
					(font = vo_font->font[tc.unicode]) >= 0 && y + hm < dys)
				{
					tt_draw_alpha_buf(obj, x, y, vo_font->width[tc.unicode], vo_font->height,
					                  vo_font->pic_b[font]->bmp + vo_font->start[tc.unicode] - vo_font->charspace *
					                  vo_font->pic_a[font]->w,
...
static inline void vo_update_text_sub(mp_osd_obj_t* obj, int dxs, int dys)
{
...

	{
		...
		if (obj->params.subtitle.lines)
			obj->y = dys - ((obj->params.subtitle.lines - 1) * sub_font->height + sub_font->pic_a[sub_font->font[40]]->
				h);
	...
	while (obj)
	{
		...
		if (obj->flags & OSDFLAG_CHANGED)
		{
			...
			mp_msg(MSGT_OSD, MSGL_DBG2, "OSD chg: %d  V: %s  pb:%d  \n", obj->type,
			       (obj->flags & OSDFLAG_VISIBLE) ? "yes" : "no", vo_osd_progbar_type);
		}
		obj = obj->next;
	}
	return chg;
