                               ...
                               uint8_t** ref_picture)
{
	...
	if (s->flags & CODEC_FLAG_EMU_EDGE)
	{
		...
		{
			s->dsp.emulated_edge_mc(s->edge_emu_buffer, ptr, linesize, 17, 17, src_x, src_y, s->h_edge_pos,
			                        s->v_edge_pos);
	...
	if (s->flags & CODEC_FLAG_EMU_EDGE)
	{
		...
		{
			s->dsp.emulated_edge_mc(s->edge_emu_buffer, ptr, uvlinesize, 9, 9, src_x, src_y, s->h_edge_pos >> 1,
			                        s->v_edge_pos >> 1);
			ptr = s->edge_emu_buffer;
			emu = 1;
		}
...
void MPV_motion_internal(MpegEncContext* s,
                         ...
                         qpel_mc_func (*qpix_op)[16], int is_mpeg12)
{
	...
	case MV_TYPE_FIELD:
		...
		{
			if (s->picture_structure != s->field_select[dir][0] + 1 && s->pict_type != AV_PICTURE_TYPE_B && !s->
				first_field)
			{
