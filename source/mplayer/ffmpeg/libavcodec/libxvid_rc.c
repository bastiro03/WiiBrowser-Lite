...
int ff_xvid_rate_control_init(MpegEncContext* s)
{
	...
	for (i = 0; i < s->rc_context.num_entries; i++)
	{
		...
		snprintf(tmp, sizeof(tmp), "%c %d %d %d %d %d %d\n",
		         frame_types[rce->pict_type], (int)lrintf(rce->qscale / FF_QP2LAMBDA), rce->i_count,
		         s->mb_num - rce->i_count - rce->skip_count,
		         rce->skip_count, (rce->i_tex_bits + rce->p_tex_bits + rce->misc_bits + 7) / 8,
