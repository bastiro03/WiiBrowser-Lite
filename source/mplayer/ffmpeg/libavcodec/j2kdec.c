                         ...
                         int layno, uint8_t* expn, int numgbits)
{
	...
	for (bandno = 0; bandno < rlevel->nbands; bandno++)
	{
		...
		for (cblkny = prec->yi0; cblkny < prec->yi1; cblkny++)
			for (cblknx = prec->xi0, cblkno = cblkny * band->cblknx + cblknx; cblknx < prec->xi1; cblknx++, cblkno++,
			     pos++)
			{
	...
	for (compno = 0; compno < s->ncomponents; compno++)
	{
		...
		for (reslevelno = 0; reslevelno < codsty->nreslevels; reslevelno++)
		{
			...
			for (bandno = 0; bandno < rlevel->nbands; bandno++)
			{
				...
				yy0 = bandno == 0
					      ? 0
					      : comp->reslevel[reslevelno - 1].coord[1][1] - comp->reslevel[reslevelno - 1].coord[1][0];
				y0 = yy0;
						...
						else
						{
							for (y = yy0; y < yy1; y += s->cdy[compno])
							{
								...
								for (x = xx0; x < xx1; x += s->cdx[compno])
								{
									...
									comp->data[(comp->coord[0][1] - comp->coord[0][0]) * y + x] =
										tmp < 0 ? -tmp2 : tmp2;
								}
							}
						}
...
int j2kdec_init(AVCodecContext* avctx)
{
	J2kDecoderContext* s = avctx->priv_data;
