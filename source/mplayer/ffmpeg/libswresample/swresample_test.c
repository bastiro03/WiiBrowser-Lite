...
static double get(uint8_t* a[], int ch, int index, int ch_count, enum AVSampleFormat f)
{
	...
	switch (f)
	{
	case AV_SAMPLE_FMT_U8: return p[index] / 127.0 - 1.0;
...
static void set(uint8_t* a[], int ch, int index, int ch_count, enum AVSampleFormat f, double v)
{
	...
	switch (f)
	{
	case AV_SAMPLE_FMT_U8: p[index] = av_clip_uint8(lrint((v + 1.0) * 127));
		break;
	case AV_SAMPLE_FMT_S16: ((int16_t*)p)[index] = av_clip_int16(lrint(v * 32767));
		break;
	case AV_SAMPLE_FMT_S32: ((int32_t*)p)[index] = av_clipl_int32(lrint(v * 2147483647));
		break;
...
int main(int argc, char** argv)
{
	...
	int flush_i = 0;
	int mode;
	int max_tests = FF_ARRAY_ELEMS(rates) * FF_ARRAY_ELEMS(layouts) * FF_ARRAY_ELEMS(formats) * FF_ARRAY_ELEMS(layouts)
	...
	for (test = 0; test < num_tests; test++)
	{
		...
		for (ch = 0; ch < in_ch_count; ch++)
		{
			...
			fprintf(stderr, "[e:%f c:%f max:%f] len:%5d\n", sqrt(sse / out_count), sum_ab / (sqrt(sum_aa * sum_bb)),
			        maxdiff, out_count);
		}
		...
		if (flush_count)
		{
			for (ch = 0; ch < in_ch_count; ch++)
			{
				...
				fprintf(stderr, "[e:%f c:%f max:%f] len:%5d F:%3d\n", sqrt(sse / flush_count),
				        sum_ab / (sqrt(sum_aa * sum_bb)), maxdiff, flush_count, flush_i);
			}
		}
