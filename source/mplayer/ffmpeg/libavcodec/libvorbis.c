...
static const AVOption options[] = {
	{
		"iblock", "Sets the impulse block bias", offsetof(OggVorbisContext, iblock), AV_OPT_TYPE_DOUBLE, {.dbl = 0},
...
int oggvorbis_init_encoder(vorbis_info* vi,
...
int oggvorbis_encode_close(AVCodecContext* avctx)
{
	OggVorbisContext* s = avctx->priv_data;
...
int oggvorbis_encode_init(AVCodecContext* avctx)
{
	OggVorbisContext* s = avctx->priv_data;
                                  ...
                                  const AVFrame* frame, int* got_packet_ptr)
{
	...
	if (frame)
	{
		...
		{
			int i;
			int co = (channels > 8) ? c : ff_vorbis_encoding_channel_layout_offsets[channels - 1][c];
			for (i = 0; i < samples; i++)
