...
static void handle_stream(demuxer_t* demuxer, AVFormatContext* avfc, int i)
{
	...
	if (stream_type)
	{
		...
		mp_msg(MSGT_DEMUX, MSGL_INFO, "[lavf] stream %d: %s (%s), -%cid %d", i, stream_type, codec_name, *stream_type,
		       stream_id);
...
static demuxer_t* demux_open_lavf(demuxer_t* demuxer)
{
	...
	for (i = 0; i < avfc->nb_chapters; i++)
	{
		...
		uint64_t start = av_rescale_q(c->start, c->time_base, (AVRational)
		{
			1, 1000
		}
		)
		uint64_t end = av_rescale_q(c->end, c->time_base, (AVRational)
		{
			1, 1000
		}
		)
		...
		redo:
			...
			for (i = 0; i < program->nb_stream_indexes; i++)
			{
				...
				case AVMEDIA_TYPE_AUDIO:
					...
					break;
				case AVMEDIA_TYPE_SUBTITLE:
					if (prog->sid == -2 && priv->avfc->streams[program->stream_index[i]]->codec->codec_id ==
						CODEC_ID_TEXT)
