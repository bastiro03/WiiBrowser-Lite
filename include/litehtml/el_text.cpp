...
litehtml::el_text::el_text(const wchar_t* text, document* doc) : element(doc)
{
	if (text)
	{
		m_text = text;
...
void litehtml::el_text::draw_content(uint_ptr hdc, const position& pos)
{
	uint_ptr font = m_parent->get_font();
	web_color color = m_parent->get_color(L"color", true, m_doc->get_def_color());
...
void litehtml::el_text::apply_stylesheet(const css& stylesheet)
{
}
...
{
	m_text_transform = static_cast<text_transform>(value_index(get_style_property(L"text-transform", true, L"none"),
