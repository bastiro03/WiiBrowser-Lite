                                     ...
                                     void* arg, MPContext* mpctx)
{
	...
	case M_PROPERTY_PRINT:
		if (!arg)
			return M_PROPERTY_ERROR;
		*(char**)arg = strdup(frame_dropping == 1
			                      ? MSGTR_Enabled
	...
	case M_PROPERTY_PRINT:
		...
		*(char**)arg = malloc(64);
		...
		if (vo_vobsub && vobsub_id >= 0)
		{
			...
			language = vobsub_get_id(vo_vobsub, vobsub_id);
		...
		case MP_CMD_STEP_PROPERTY:
			{
				...
				r = mp_property_do(cmd->args[0].v.s,
				                   cmd->args[2].v.i < 0 ? M_PROPERTY_STEP_DOWN : M_PROPERTY_STEP_UP,
				                   arg, mpctx);
			step_prop_err:
				if (r == M_PROPERTY_UNKNOWN)
					mp_msg(MSGT_CPLAYER, MSGL_WARN,
		...
		case MP_CMD_OSD:
			{
				...
				if (v == -1 && osd_level <= 1)
					set_osd_msg(OSD_MSG_OSD_STATUS, 0, osd_duration,
					            MSGTR_OSDosd,
					            osd_level ? MSGTR_OSDenabled : MSGTR_OSDdisabled);
				else
