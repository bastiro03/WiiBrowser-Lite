...
void FreeTypeGX::unloadFont()
{
    ...
    for(itr = fontData.begin(); itr != fontData.end(); itr++)
        free(itr->second.glyphDataTexture);
...
ftgxCharData *FreeTypeGX::cacheGlyphData(wchar_t charCode)
{
    ...
    if (gIndex && !FT_Load_Glyph(ftFace, gIndex, FT_LOAD_DEFAULT | FT_LOAD_RENDER ))
    {
        if(ftSlot->format == FT_GLYPH_FORMAT_BITMAP)
        {
            ...
            loadGlyphData(glyphBitmap, &fontData[charCode]);
...
uint16_t FreeTypeGX::cacheGlyphDataComplete()
{
    ...
    while ( gIndex != 0 )
    {
        if(cacheGlyphData(charCode) != NULL)
            ++i;
...
uint16_t FreeTypeGX::drawText(int16_t x, int16_t y, const wchar_t *text, GXColor color, uint16_t textStyle, uint16_t textWidth, uint16_t widthLimit)
{
    ...
    {
        x_offset = this->getStyleOffsetWidth(fullTextWidth, textStyle);
    }
    if(textStyle & FTGX_ALIGN_MASK)
    {
        this->getOffset(text, &offset, widthLimit);
    ...
    while (text[i])
    {
        ...
        if (glyphData != NULL)
        {
            ...
            copyTextureToFramebuffer(&glyphTexture, glyphData->textureWidth, glyphData->textureHeight, x_pos + glyphData->renderOffsetX + x_offset, y - glyphData->renderOffsetY + y_offset, 0, color);
    ...
    {
        getOffset(text, &offset, widthLimit);
        drawTextFeature(x + x_offset, y + y_offset, fullTextWidth, &offset, textStyle, color);
    }
...
void FreeTypeGX::drawTextFeature(int16_t x, int16_t y, uint16_t width, ftgxDataOffset *offsetData, uint16_t format, GXColor color)
{
    ...
    if (format & FTGX_STYLE_UNDERLINE)
        this->copyFeatureToFramebuffer(width, featureHeight, x, y + 1, 0, color);

    if (format & FTGX_STYLE_STRIKE)
        this->copyFeatureToFramebuffer(width, featureHeight, x, y - ((offsetData->max) >> 1), 0, color);
}
...
uint16_t FreeTypeGX::getCharWidth(const wchar_t wChar)
{
    ...
    {
        glyphData = cacheGlyphData(wChar);
    }
