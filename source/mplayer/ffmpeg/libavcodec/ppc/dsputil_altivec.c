...
static int sad16_x2_altivec(void* v, uint8_t* pix1, uint8_t* pix2, int line_size, int h)
{
	int i;
	int s;
	const vector unsigned char zero = (const vector
unsigned char
	)
	vec_splat_u8(0);
	vector
	...
	vector
	unsigned char pix1v, pix2v, pix2iv, avgv, t5;
	vector
	unsigned int sad;
	vector
	signed int sumdiffs;
 ...
 unsigned int
	...
	sumdiffs = vec_sums((vector signed int)
	sad, (vector
 signed int
	)
	zero
...
unsigned char
	...
	unsigned char pix2l, pix2r;
	vector
	unsigned char pix1v, pix2v, pix3v, avgv, t5;
	vector
	unsigned int sad;
	vector
	signed int sumdiffs;
...
unsigned char
	...
	vec_splat_u8(0);
	const vector unsigned short two = (const vector
unsigned short
	)
	vec_splat_u16(2);
	vector
 ...
 unsigned short
	...
	vec_mergeh(zero, pix2v);
	pix2lv = (vector
 unsigned short
	)
	vec_mergel(zero, pix2v);
	pix2ihv = (vector
 unsigned short
	)
	vec_mergeh(zero, pix2iv);
	pix2ilv = (vector
 unsigned short
	)
	vec_mergel(zero, pix2iv);
 ...
 unsigned short
		...
		vec_mergeh(zero, pix3v);
		pix3lv = (vector
 unsigned short
		)
		vec_mergel(zero, pix3v);
		pix3ihv = (vector
 unsigned short
		)
		vec_mergeh(zero, pix3iv);
		pix3ilv = (vector
 unsigned short
		)
		vec_mergel(zero, pix3iv);
...
static int sad16_altivec(void* v, uint8_t* pix1, uint8_t* pix2, int line_size, int h)
{
	int i;
	int s;
	const vector unsigned int zero = (const vector
unsigned int
	)
	vec_splat_u32(0);
	vector
...
unsigned int
	...
	vec_splat_u32(0);
	const vector unsigned char permclear = (vector
 unsigned char
	)
	{
...
unsigned int
 ...
 unsigned int
	...
	/* Sum up the four partial sums, and put the result into s */
	sum = vec_sums((vector signed int)
	sv, (vector
 signed int
	)
	zero
...
unsigned int
 ...
 unsigned int
	...
	/* Sum up the four partial sums, and put the result into s */
	sumsqr = vec_sums((vector signed int)
	sum, (vector
 signed int
	)
	zero
...
unsigned char
 ...
 signed short
		...
		vec_mergeh(zero, bytes);

		// save the data to the block, we assume the block is 16-byte aligned
		vec_st(shorts, i * 16, (vector signed short*)
		block
		)
...
unsigned char
 ...
 signed short
		...
		// save the data to the block, we assume the block is 16-byte aligned
		vec_st(shorts1, 0, (vector signed short*)
		block
		)
...
static void put_pixels8_xy2_altivec(uint8_t* block, const uint8_t* pixels, int line_size, int h)
{
	...
	register const vector unsigned char vczero = (const vector
unsigned char
	)
	vec_splat_u8(0);
	register const vector unsigned short vctwo = (const vector
unsigned short
	)
	vec_splat_u16(2);
	...
	pixelssum1 = vec_add((vector unsigned short)
	pixelsv1,
		(vector
 unsigned short
	)
	...
	for (i = 0; i < h; i++)
	{
		...
		pixelsv1 = vec_mergeh(vczero, pixelsv1);
		pixelsv2 = vec_mergeh(vczero, pixelsv2);
		pixelssum2 = vec_add((vector unsigned short)
		pixelsv1,
			(vector
 unsigned short
		)
		...
		pixelsavg = vec_packsu(temp3, (vector unsigned short)
		vczero
		)
...
unsigned char
	...
	vec_splat_u8(0);
	register const vector unsigned short vcone = (const vector
unsigned short
	)
	vec_splat_u16(1);
	register const vector unsigned short vctwo = (const vector
...
unsigned short
	...
	pixelsv1 = vec_mergeh(vczero, pixelsv1);
	pixelsv2 = vec_mergeh(vczero, pixelsv2);
	pixelssum3 = vec_add((vector unsigned short)
	pixelsv3,
		(vector
 unsigned short
	)
 ...
 unsigned short
	...
	for (i = 0; i < h; i++)
	{
		...
		pixelssum4 = vec_add((vector unsigned short)
		pixelsv3,
			(vector
 unsigned short
		)
...
unsigned char
	...
	register vector signed short temp0, temp1, temp2, temp3, temp4,
	                             temp5, temp6, temp7;
	{
		register const vector signed short vprod1 = (const vector
signed short
		)
		{
			1, -1, 1, -1, 1, -1, 1, -1
		}
		register const vector signed short vprod2 = (const vector
signed short
		)
		{
			1, 1, -1, -1, 1, 1, -1, -1
		}
		register const vector signed short vprod3 = (const vector
signed short
		)
		{
			1, 1, 1, 1, -1, -1, -1, -1
		}
		register const vector unsigned char perm1 = (const vector
unsigned char
		)
		{
		...
		register const vector unsigned char perm2 = (const vector
unsigned char
		)
		{
		...
		register const vector unsigned char perm3 = (const vector
unsigned char
		)
		{
...
#undef ONEITERBUTTERFLY
	{
		...
		vsum = vec_sums(vsum, (vector signed int)
		vzero
		)
                                            ...
                                            int blocksize)
{
	...
	for (i = 0; i < blocksize; i += 4)
	{
		m = vec_ld(0, mag + i);
		a = vec_ld(0, ang + i);
		t0 = vec_cmple(m, (vector float)
		vec_splat_u32(0)
		)
		t1 = vec_cmple(a, (vector float)
		vec_splat_u32(0)
		)
		a = vec_xor(a, (vector float)
		vec_sl((vector unsigned int)
		t0, v_31
		)
		)
		t0 = (vector
 ...
 int
		...
		vec_andc(a, t1);
		a = vec_sub(m, (vector float)
		t1
		)
		m = vec_add(m, (vector float)
		t0
		)
		vec_stl(a, 0, ang + i);
		vec_stl(m, 0, mag + i);
