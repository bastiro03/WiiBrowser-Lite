...
{
	if (dp->len != offs && offs != -1) mp_msg(MSGT_DEMUX, MSGL_V,
	                                          "warning! fragment.len=%d BUT next fragment offset=%d  \n", dp->len,
	                                          offs);
...
static int demux_asf_read_packet(demuxer_t* demux, unsigned char* data, int len, int id, int seq, uint64_t time,
                                 unsigned short dur, int offs, int keyframe)
{
 ...
 *
 */
static void get_payload_extension_data(demuxer_t* demux, unsigned char** pp, unsigned char id, unsigned int seq,
                                       int* keyframe, uint64_t* seg_time)
{
	struct asf_priv* asf = demux->priv;
...
static int demux_asf_fill_buffer(demuxer_t* demux, demux_stream_t* ds)
{
	...
	mp_msg(MSGT_DEMUX, MSGL_V, "%08"PRIX64":  UNKNOWN TYPE  %02X %02X %02X %02X %02X...\n", (int64_t)demux->filepos,
	       asf->packet[0], asf->packet[1], asf->packet[2], asf->packet[3], asf->packet[4]);
	return 0;
}
...
static void demux_seek_asf(demuxer_t* demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	off_t rel_seek_packs = (flags & SEEK_FACTOR)
		                       ? // FIXME: int may be enough?
