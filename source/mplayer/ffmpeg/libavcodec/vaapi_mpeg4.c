...
static int vaapi_mpeg4_start_frame(AVCodecContext* avctx, const av_unused uint8_t* buffer, av_unused uint32_t size)
{
	...
	pic_param->vop_fields.bits.backward_reference_vop_coding_type = s->pict_type == AV_PICTURE_TYPE_B
		                                                                ? s->next_picture.f.pict_type -
		                                                                AV_PICTURE_TYPE_I
		                                                                : 0;
