...
DS_VideoDecoder* DS_VideoDecoder_Open(char* dllname, GUID* guid, BITMAPINFOHEADER* format, int flip, int maxauto)
{
	...
	/*try*/
	{
		...
		bihs = (format->biSize < (int)sizeof(BITMAPINFOHEADER)) ? sizeof(BITMAPINFOHEADER) : format->biSize;
		...
		memcpy(&this->iv.m_obh, this->iv.m_bh, sizeof(this->iv.m_obh) < (unsigned)this->iv.m_bh->biSize
			                                       ? sizeof(this->iv.m_obh)
			                                       : (unsigned)this->iv.m_bh->biSize);
		...
		if (!flip)
		{
			...
			result = this->m_pDS_Filter->m_pOutputPin->vt->QueryAccept(this->m_pDS_Filter->m_pOutputPin,
			                                                           &this->m_sDestType);
			if (result)
			{
	...
	//	case fccIV50:
		...
		default:
			...
			for (c = check; c->bits; c++)
			{
				...
				result = this->m_pDS_Filter->m_pOutputPin->vt->QueryAccept(
					this->m_pDS_Filter->m_pOutputPin, &this->m_sDestType);
				if (!result)
				{
...
*/
	...
	if (should_test)
		result = this->m_pDS_Filter->m_pOutputPin->vt->
		               QueryAccept(this->m_pDS_Filter->m_pOutputPin, &this->m_sDestType);
	else
		result = -1;
...
int DS_VideoDecoder_SetValue(DS_VideoDecoder* this, const char* name, int value)
{
	if (this->m_bIsDivX4)
	{
		...
		if (this->m_pDS_Filter->m_pFilter->vt->QueryInterface((IUnknown*)this->m_pDS_Filter->m_pFilter,
		                                                      &IID_IDivxFilterInterface, (void**)&pIDivx))
		{
