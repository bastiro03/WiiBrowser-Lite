...
static int asf_streaming(ASF_stream_chunck_t* stream_chunck, int* drop_packet)
{
	...
	*/
	...
	{
		mp_msg(MSGT_NETWORK, MSGL_ERR, MSGTR_MPDEMUX_ASF_SizeConfirmMismatch, stream_chunck->size,
...
static int asf_streaming_parse_header(int fd, streaming_ctrl_t* streaming_ctrl)
{
	...
	if (pos >= 0)
	{
		ASF_file_header_t* fileh = &buffer[pos];
		...
		*/
		...
		streaming_ctrl->prebuffer_size = (unsigned int)(((double)fileh->preroll / 1000.0) * ((double)fileh->max_bitrate
			/ 8.0));
	}
	...
	{
		ASF_stream_header_t* streamh = &buffer[pos];
...
static int asf_http_streaming_read(int fd, char* buffer, int size, streaming_ctrl_t* streaming_ctrl)
{
	...
	asf_http_streaming_ctrl_t* asf_http_ctrl = streaming_ctrl->data;
	...
	while (pragma != NULL)
	{
		...
		do
		{
			if (!strncasecmp(pragma, "features=", 9))
			{
				...
				{
					mp_msg(MSGT_NETWORK, MSGL_WARN, MSGTR_MPDEMUX_ASF_ASFHTTPParseWarnCuttedPragma, pragma, len,
					       sizeof(features) - 1);
...
static int open_s(stream_t* stream, int mode, void* opts, int* file_format)
{
	...
	if ((!strncmp(stream->url, "http", 4)) && (*file_format != DEMUXER_TYPE_ASF && *file_format !=
