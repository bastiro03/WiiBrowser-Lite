                                ...
                                const char* value)
{
	if (!strcmp(key, "WWW-Authenticate") || !strcmp(key, "Proxy-Authenticate"))
	{
			...
			state->auth_type <= HTTP_AUTH_BASIC)
		{
			...
			state->realm[0] = 0;
			state->stale = 0;
			ff_parse_key_value(p, handle_basic_params,
			                   state);
		}
			...
			state->auth_type <= HTTP_AUTH_DIGEST)
		{
			...
			state->realm[0] = 0;
			state->stale = 0;
			ff_parse_key_value(p, handle_digest_params,
			                   state);
	...
	{
		ff_parse_key_value(value, handle_digest_update,
		                   state);
	}
}
                              ...
                              const char* method)
{
	...
	ff_data_to_hex(cnonce, cnonce_buf, sizeof(cnonce_buf), 1);
