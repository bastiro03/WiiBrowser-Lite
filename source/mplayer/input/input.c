...
static const mp_cmd_t mp_cmds[] = {
  ...
  { MP_CMD_RADIO_STEP_FREQ, "radio_step_freq", 1, { {MP_CMD_ARG_FLOAT,{0}}, {-1,{0}} } },
	...
	{
		MP_CMD_OSD_SHOW_TEXT, "osd_show_text", 1,
		{{MP_CMD_ARG_STRING, {0}}, {MP_CMD_ARG_INT, {-1}}, {MP_CMD_ARG_INT, {0}}, {-1, {0}}}
	},
	{
  ...
  { MP_CMD_DVB_SET_CHANNEL, "dvb_set_channel", 2, { {MP_CMD_ARG_INT,{0}}, {MP_CMD_ARG_INT,{0}}, {-1,{0}}}},
	...
	{
		MP_CMD_OVERLAY_ADD, "overlay_add", 5,
		{
			{MP_CMD_ARG_STRING, {0}}, {MP_CMD_ARG_INT, {0}}, {MP_CMD_ARG_INT, {0}}, {MP_CMD_ARG_INT, {0}},
  ...
  { MP_CMD_CHIDE, "hide", 0, { {MP_CMD_ARG_INT,{3000}}, {-1,{0}} } },
	...
	{
		MP_CMD_STEP_PROPERTY, "step_property", 1,
		{{MP_CMD_ARG_STRING, {0}}, {MP_CMD_ARG_FLOAT, {0}}, {MP_CMD_ARG_INT, {0}}, {-1, {0}}}
	},
...
static mp_cmd_t* check_autorepeat(int paused)
{
	...
	if (ar_rate > 0 && ar_state >= 0 && num_key_down > 0 && !(key_down[num_key_down - 1] & MP_NO_REPEAT_KEY))
	{
		...
		if (ar_state == 1 && (t - last_ar) >= 1000000 / ar_rate)
		{
			last_ar = t;
			return mp_cmd_clone(ar_cmd);
		}
...
static mp_cmd_t* read_events(int time, int paused)
{
	...
	for (i = 0; i < num_key_fd; i++)
	{
		...
		if (key_fds[i].no_readfunc_retval)
		{
			// getch2 handler special-cased for now
...
mp_input_get_key_from_name(const char* name)
{
	...
	if (len > 2 && strncasecmp("0x", name, 2) == 0)
...
mp_input_parse_config(char* file)
{
	...
	while (1)
	{
		...
		if (keys[0] == 0)
		{
			...
			if (end[0] == '\0')
			{
				// Key name doesn't fit in the buffer
				if (buffer == iter)
				{
...
mp_input_set_section(char* name)
{
	...
	free(section);
	if (name) section = strdup(name);
	else section = strdup("default");
