...
void ff_write_pass1_stats(MpegEncContext* s)
{
	snprintf(s->avctx->stats_out, 256,
	         ...
	         s->current_picture_ptr->f.display_picture_number, s->current_picture_ptr->f.coded_picture_number,
	         s->pict_type,
...
int ff_rate_control_init(MpegEncContext* s)
{
	...
	res = av_expr_parse(&rcc->rc_eq_eval, s->avctx->rc_eq ? s->avctx->rc_eq : "tex^qComp", const_names, func1_names,
	                    func1, NULL, NULL, 0, s->avctx);
	if (res < 0)
	{
	...
	if (s->flags & CODEC_FLAG_PASS2)
	{
		...
		for (i = 0; i < rcc->num_entries - s->max_b_frames; i++)
		{
			...
			e += sscanf(
				...
				&rce->f_code, &rce->b_code, &rce->mc_mb_var_sum, &rce->mb_var_sum, &rce->i_count, &rce->skip_count,
				&rce->header_bits);
			if (e != 14)
			{
...
static double modify_qscale(MpegEncContext* s, RateControlEntry* rce, double q, int frame_num)
{
	...
	if (buffer_size)
	{
		...
		if (min_rate)
		{
			...
			q_limit = bits2qp(
				rce, FFMAX((min_rate - buffer_size + rcc->buffer_index) * s->avctx->rc_min_vbv_overflow_use, 1));
			if (q > q_limit)
			{
				if (s->avctx->debug & FF_DEBUG_RC)
				{
...
float ff_rate_estimate_qscale(MpegEncContext* s, int dry_run)
{
	...
	if (s->avctx->debug & FF_DEBUG_RC)
	{
		av_log(s->avctx, AV_LOG_DEBUG,
		       ...
		       av_get_picture_type_char(pict_type), qmin, q, qmax, picture_number, (int)wanted_bits / 1000,
		       (int)s->total_bits / 1000,
		       br_compensation, short_term_q, s->frame_bits, pic->mb_var_sum, pic->mc_mb_var_sum, s->bit_rate / 1000,
		       (int)fps
		);
	}
