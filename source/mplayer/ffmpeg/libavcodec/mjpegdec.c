			...
			if (!mb_x || mb_y == resync_mb_y || mb_y == resync_mb_y + 1 && mb_x < resync_mb_x || s->interlaced)
			{
				...
				for (i = 0; i < nb_components; i++)
				{
					...
					for (j = 0; j < n; j++)
					{
						...
						{
							ptr16 = (uint16_t*)(s->picture.data[c] + 2 * (linesize * (v * mb_y + y)) + 2 * (h * mb_x +
								x)); //FIXME optimize this crap
							if (y == 0 && toprow)
							{
								if (x == 0 && leftcol)
								{
...
void mjpeg_copy_block(uint8_t* dst, const uint8_t* src,
                        ...
                        const AVFrame* reference)
{
	...
	for (i = 0; i < nb_components; i++)
	{
		...
		if (s->dc_index[i] < 0 || s->ac_index[i] < 0 ||
			...
			goto out_of_range;
		if (!s->vlcs[0][s->dc_index[i]].table || !(s->progressive
			                                           ? s->vlcs[2][s->ac_index[0]].table
