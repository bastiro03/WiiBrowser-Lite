...
int HandleForm(GuiWindow *parentWindow, GuiWindow *mainWindow, ListaDiBottoni btn)
{
	...
	ListaDiBottoni bottone, choice = nullptr, button = nullptr;
	...
	for (bottone = button; !NoButton(bottone); bottone = bottone->prox)
	{
		if (!bottone->btn || bottone == choice || bottone->outline == TEXT || bottone->outline == UPLOAD)
		{
...
int inputType(ListaDiInput lista)
{
	...
	if (lista->type == "hidden")
		return HIDDEN;
	if (lista->type == "text" || lista->type == "email" || lista->type == "password" || lista->type == "search" || lista->type == "tel" || lista->type.length() == 0)
		return TEXT;
	if (lista->type == "radio")
		return RADIO;
