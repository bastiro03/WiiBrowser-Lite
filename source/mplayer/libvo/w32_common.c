...
static int createRenderingContext(void)
{
	...
	RECT r;
	int style = (vo_border && !vo_fs) ? (WS_OVERLAPPEDWINDOW | WS_SIZEBOX) : WS_POPUP;
...
int vo_w32_init(void)
{
	...
	{
		WNDCLASSEX wcex = {
			sizeof wcex, CS_OWNDC | CS_DBLCLKS, WndProc, 0, 0, hInstance, mplayerIcon, LoadCursor(0, IDC_ARROW), NULL,
			0, classname, mplayerIcon
		};
