...
static void* attribute_align_arg worker(void* v)
{
	...
	for (;;)
	{
		...
		c->rets[our_job % c->rets_count] = c->func
			                                   ? c->func(avctx, (char*)c->args + our_job * c->job_size)
...
void avcodec_thread_park_workers(ThreadContext* c, int thread_count)
{
...
static int avcodec_thread_execute(AVCodecContext* avctx, action_func* func, void* arg, int* ret, int job_count,
                                  int job_size)
{
	ThreadContext* c = avctx->thread_opaque;
	int dummy_ret;
...
void* frame_worker_thread(void* arg)
{
	...
	while (1)
	{
		...
		if (!codec->update_thread_context && (avctx->thread_safe_callbacks || avctx->get_buffer ==
