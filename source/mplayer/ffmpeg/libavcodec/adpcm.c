                          ...
                          int buf_size, int* coded_samples)
{
	...
	*coded_samples = 0;
	...
	case CODEC_ID_ADPCM_IMA_ISS: header_size = 4 * ch;
		break;
	case CODEC_ID_ADPCM_IMA_AMV: header_size = 8;
		break;
	case CODEC_ID_ADPCM_IMA_SMJPEG: header_size = 4;
		break;
	}
	if (header_size > 0)
	...
	case CODEC_ID_ADPCM_SBPRO_4:
		{
			...
			switch (avctx->codec->id)
			{
			case CODEC_ID_ADPCM_SBPRO_2: samples_per_byte = 4;
				break;
			case CODEC_ID_ADPCM_SBPRO_3: samples_per_byte = 3;
				break;
			case CODEC_ID_ADPCM_SBPRO_4: samples_per_byte = 2;
				break;
			}
	...
	case CODEC_ID_ADPCM_EA_R3:
		{
			...
			for (channel = 0; channel < avctx->channels; channel++)
				offsets[channel] = (big_endian ? bytestream2_get_be32(&gb) : bytestream2_get_le32(&gb)) +
					(avctx->channels + 1) * 4;
