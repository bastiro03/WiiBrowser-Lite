                                           ...
                                           const char* filename)
{
	...
	result = RegQueryValueExA(HKEY_LOCAL_MACHINE, fullname, NULL, NULL, buffer, &size);
	if ((size >= 0) && (size < 256))
		buffer[size] = 0;
                                              ...
                                              const char* filename)
{
	...
	int size;
	char* fullname;
	dbgprintf("GetPrivateProfileStringA('%s', '%s', def_val '%s', 0x%x, 0x%x, '%s')", appname, keyname, def_val, dest,
	          len, filename);
...
static HDRVR WINAPI expOpenDriverA(LPCSTR szDriverName, LPCSTR szSectionName,
                                   LPARAM lParam2)
{
	dbgprintf("OpenDriverA(0x%x='%s', 0x%x='%s', 0x%x) => -1\n", szDriverName, szDriverName, szSectionName,
	          szSectionName, lParam2);
	return -1;
}
...
void* LookupExternal(const char* library, int ordinal)
{
	...
	if (!strcmp(library, "tsd32.dll") || !strcmp(library, "vssh264dec.dll") || !strcmp(library, "LCMW2.dll") || !strcmp(
		library, "VDODEC32.dll"))
#endif
