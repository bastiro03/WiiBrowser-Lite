...
static void decode_mb(MpegEncContext* s, int ref)
{
	...
	s->dest[1] = s->current_picture.f.data[1] + (s->mb_y * (16 >> s->chroma_y_shift) * s->uvlinesize) + s->mb_x * (16 >>
		s->chroma_x_shift);
	s->dest[2] = s->current_picture.f.data[2] + (s->mb_y * (16 >> s->chroma_y_shift) * s->uvlinesize) + s->mb_x * (16 >>
		s->chroma_x_shift);
                           ...
                           int h, int stride, int is_luma)
{
	...
	for (b_y = 0; b_y < h; b_y++)
	{
		for (b_x = 0; b_x < w - 1; b_x++)
		{
			...
			int right_intra = IS_INTRA(
				s->current_picture.f.mb_type[((b_x + 1) >> is_luma) + (b_y >> is_luma) * s->mb_stride]);
                           ...
                           int stride, int is_luma)
{
	...
	for (b_y = 0; b_y < h - 1; b_y++)
	{
		for (b_x = 0; b_x < w; b_x++)
		{
			...
			int bottom_intra = IS_INTRA(
				s->current_picture.f.mb_type[(b_x >> is_luma) + ((b_y + 1) >> is_luma) * s->mb_stride]);
		...
		num_avail <= mb_width / 2)
	{
		for (mb_y = 0; mb_y < s->mb_height; mb_y++)
		{
			for (mb_x = 0; mb_x < s->mb_width; mb_x++)
			{
				...
				s->mv_dir = s->last_picture.f.data[0]
					            ? MV_DIR_FORWARD
					            : MV_DIR_BACKWARD;
				s->mb_intra = 0;
	...
	for (mb_y = 0; mb_y < s->mb_height; mb_y++)
	{
		for (mb_x = 0; mb_x < s->mb_width; mb_x++)
		{
			...
			if (IS_8X8(mb_type))
			{
				...
				for (j = 0; j < 4; j++)
				{
					s->mv[0][j][0] = s->current_picture.f.motion_val[dir][mb_index + (j & 1) + (j >> 1) * s->b8_stride][
						0];
					s->mv[0][j][1] = s->current_picture.f.motion_val[dir][mb_index + (j & 1) + (j >> 1) * s->b8_stride][
						1];
				}
			}
			else
