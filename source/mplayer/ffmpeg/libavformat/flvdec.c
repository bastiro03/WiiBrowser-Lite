...
static int flv_set_video_codec(AVFormatContext* s, AVStream* vstream, int flv_codecid)
{
	...
	case FLV_CODECID_H263: vcodec->codec_id = CODEC_ID_FLV1;
		break;
	case FLV_CODECID_REALH263: vcodec->codec_id = CODEC_ID_H263;
...
static int amf_parse_object(AVFormatContext* s, AVStream* astream, AVStream* vstream, const char* key, int64_t max_pos,
                            int depth)
{
...
static AVStream* create_stream(AVFormatContext* s, int stream_type)
{
	...
	switch (stream_type)
	{
	case FLV_STREAM_TYPE_VIDEO: st->codec->codec_type = AVMEDIA_TYPE_VIDEO;
		break;
	case FLV_STREAM_TYPE_AUDIO: st->codec->codec_type = AVMEDIA_TYPE_AUDIO;
		break;
...
static int flv_read_packet(AVFormatContext* s, AVPacket* pkt)
{
	...
	for (;; avio_skip(s->pb, 4))
	{
		...
		if ((st->discard >= AVDISCARD_NONKEY && !((flags & FLV_VIDEO_FRAMETYPE_MASK) == FLV_FRAME_KEY || (stream_type ==
