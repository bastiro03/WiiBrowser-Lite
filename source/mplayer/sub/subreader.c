...
static subtitle* sub_read_line_subviewer(stream_t* st, subtitle* current, int utf16)
{
	...
	{
		if (!stream_read_line(st, line, LINE_LEN, utf16)) return NULL;
		if ((len = sscanf(line, "%d:%d:%d%[,.:]%d --> %d:%d:%d%[,.:]%d", &a1, &a2, &a3, (char*)&i, &a4, &b1, &b2, &b3,
		                  (char*)&i, &b4)) < 10)
			continue;
...
static subtitle* sub_read_line_rt(stream_t* st, subtitle* current, int utf16)
{
	...
	while (!current->text[0])
	{
		...
		if (
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d.%d\" %*[Ee]nd=\"%d.%d\"%*[^<]<clear/>%n", &a3, &a4, &b3,
			               &b4, &plen)) < 4) &&
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d.%d\" %*[Ee]nd=\"%d:%d.%d\"%*[^<]<clear/>%n", &a3, &a4, &b2,
			               &b3, &b4, &plen)) < 5) &&
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d:%d\" %*[Ee]nd=\"%d:%d\"%*[^<]<clear/>%n", &a2, &a3, &b2,
			               &b3, &plen)) < 4) &&
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d:%d\" %*[Ee]nd=\"%d:%d.%d\"%*[^<]<clear/>%n", &a2, &a3, &b2,
			               &b3, &b4, &plen)) < 5) &&
			//	((len=sscanf (line, "<%*[tT]ime %*[bB]egin=\"%d:%d.%d\" %*[Ee]nd=\"%d:%d\"%*[^<]<clear/>%n",&a2,&a3,&a4,&b2,&b3,&plen)) < 5) &&
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d:%d.%d\" %*[Ee]nd=\"%d:%d.%d\"%*[^<]<clear/>%n", &a2, &a3,
			               &a4, &b2, &b3, &b4, &plen)) < 6) &&
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d:%d:%d.%d\" %*[Ee]nd=\"%d:%d:%d.%d\"%*[^<]<clear/>%n", &a1,
			               &a2, &a3, &a4, &b1, &b2, &b3, &b4, &plen)) < 8) &&
			...
			((len = sscanf(line, "<%*[tT]ime %*[bB]egin=\"%d:%d:%d.%d\"%*[^<]<clear/>%n", &a1, &a2, &a3, &a4,
			               &plen)) < 4)
		)
			continue;
		current->start = a1 * 360000 + a2 * 6000 + a3 * 100 + a4 / 10;
...
static int sub_autodetect(stream_t* st, int* uses_time, int utf16)
{
	...
	while (j < 100)
	{
		...
		if (sscanf(line, "%d:%d:%d%[,.:]%d --> %d:%d:%d%[,.:]%d", &i, &i, &i, (char*)&i, &i, &i, &i, &i, (char*)&i,
		           &i) == 10)
		{
			*uses_time = 1;
...
sub_data* sub_read_file(const char* filename, float fps)
{
	...
	fd = open_stream(filename, NULL, NULL);
	if (!fd) return NULL;
                                 ...
                                 const char* fname, int limit_fuzziness)
{
	...
	// BrowserGetSubInit();
	while (0 /*BrowserGetSub(de->d_name)*/)
	{
		//int h;
