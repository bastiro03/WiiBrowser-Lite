...
void wsCreateWindow(wsTWindow* win, int X, int Y, int wX, int hY, int bW, int cV, unsigned char D, char* label)
{
	...
	if (!(cV & wsShowMouseCursor))
		win->wsCursor = XCreatePixmapCursor(wsDisplay, win->wsCursorPixmap, win->wsCursorPixmap, &win->wsColor,
		                                    &win->wsColor, 0, 0);
...
Bool wsEvents(Display* display, XEvent* Event)
{
	...
	case ConfigureNotify:
		...
		if ((wsWindowList[l]->X != x) || (wsWindowList[l]->Y != y) || (wsWindowList[l]->Width != Event->xconfigure.
			width) || (wsWindowList[l]->Height != Event->xconfigure.height))
		{
			wsWindowList[l]->X = x;
			wsWindowList[l]->Y = y;
			...
			if (wsWindowList[l]->ReSize)
				wsWindowList[l]->ReSize(wsWindowList[l]->X, wsWindowList[l]->Y, wsWindowList[l]->Width,
				                        wsWindowList[l]->Height);
		}
	...
	buttonreleased:
		...
		if (wsWindowList[l]->MouseHandler)
			wsWindowList[l]->MouseHandler(i, Event->xbutton.x, Event->xbutton.y, Event->xmotion.x_root,
...
void wsSetIcon(Display* dpy, Window win, guiIcon_t* icon)
{
	...
	if (icon->collection)
	{
		...
		XChangeProperty(dpy, win, iconatom, XA_CARDINAL, 32, PropModeReplace, (unsigned char*)icon->collection,
		                icon->collection_size);
	}
}
