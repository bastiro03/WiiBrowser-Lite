                  ...
                  unsigned int flags, unsigned int outfmt)
{
	...
	lavc_venc_context->time_base = (AVRational)
	{
...
static int encode_frame(struct vf_instance* vf, AVFrame* pic, double pts)
{
	...
	if (lavc_param_psnr)
	{
		...
		fprintf(fvstats, "%6d, %2.2f, %6d, %2.2f, %2.2f, %2.2f, %2.2f %c\n",
		        ...
		        psnr((lavc_venc_context->coded_frame->error[0] + lavc_venc_context->coded_frame->error[1] +
			        lavc_venc_context->coded_frame->error[2]) / (f * 1.5)),
...
static void uninit(struct vf_instance* vf)
{
	if (lavc_param_psnr)
	{
		...
		mp_msg(MSGT_MENCODER, MSGL_INFO, "PSNR: Y:%2.2f, Cb:%2.2f, Cr:%2.2f, All:%2.2f\n",
		       ...
		       psnr((lavc_venc_context->error[0] + lavc_venc_context->error[1] + lavc_venc_context->error[2]) / (f *
			       1.5))
		);
	}
...
static int vf_open(vf_instance_t* vf, char* args)
{
	...
	if (!strcasecmp(lavc_param_vcodec, "mpeg1") || !strcasecmp(lavc_param_vcodec, "mpeg1video"))
		...
		mux_v->bih->biCompression = mmioFOURCC(lavc_param_vcodec[0],
		                                       lavc_param_vcodec[1], lavc_param_vcodec[2],
		                                       lavc_param_vcodec[3]); /* FIXME!!! */
