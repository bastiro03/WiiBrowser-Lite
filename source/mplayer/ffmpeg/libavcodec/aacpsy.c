...
int psy_3gpp_init(FFPsyContext* ctx)
{
	AacPsyContext* pctx;
	float bark;
	int i, j, g, start;
		...
		for (g = 0; g < num_bands; g++)
		{
			...
			if (!(wi->window_type[0] == LONG_STOP_SEQUENCE || (wi->window_type[1] == LONG_START_SEQUENCE && !w)))
				band->thr = FFMAX(PSY_3GPP_RPEMIN * band->thr, FFMIN(band->thr,
				                                                     PSY_3GPP_RPELEV * pch->prev_band[w + g].
				                                                     thr_quiet));
...
void psy_3gpp_end(FFPsyContext* apc)
{
	AacPsyContext* pctx = apc->model_priv_data;
	av_freep(&pctx->ch);
                                       ...
                                       const float* la, int channel, int prev_type)
{
	...
	if (la)
	{
		...
		for (i = 0; i < PSY_LAME_NUM_SUBBLOCKS; i++)
		{
			...
			attack_intensity[i] = energy_subshort[i] / pch->prev_energy_subshort[i + ((AAC_NUM_BLOCKS_SHORT - 2) *
