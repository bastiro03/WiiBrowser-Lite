...
static int rv34_decode_block(DCTELEM* dst, GetBitContext* gb, RV34VLC* rvlc, int fc, int sc, int q_dc, int q_ac1,
                             int q_ac2)
{
...
static inline RV34VLC* choose_vlc_set(int quant, int mod, int type)
{
	...
	return type
		       ? &inter_vlcs[rv34_quant_to_vlc_set[1][av_clip(quant, 0, 30)]]
		...
		if (IS_INTRA(next_bt) || IS_SKIP(next_bt))
		{
			...
			for (j = 0; j < 2; j++)
				for (i = 0; i < 2; i++)
					for (k = 0; k < 2; k++)
						for (l = 0; l < 2; l++)
							s->current_picture_ptr->f.motion_val[l][mv_pos + i + j * s->b8_stride][k] = calc_add_mv(
								r, l, s->next_picture_ptr->f.motion_val[0][mv_pos + i + j * s->b8_stride][k]);
 ...
 */
static void rv34_pred_4x4_block(RV34DecContext* r, uint8_t* dst, int stride, int itype, int up, int left, int down,
                                int right)
{
...
static void rv34_output_intra(RV34DecContext* r, int8_t* intra_types, int cbp)
{
	...
	for (j = 0; j < 4; j++)
	{
		...
		{
			rv34_pred_4x4_block(r, dst, s->linesize, ittrans[intra_types[i]], avail[idx - 8], avail[idx - 1],
			                    avail[idx + 7], avail[idx - 7]);
			avail[idx] = 1;
			if (!(cbp & 1)) continue;
                         ...
                         AVPacket* avpkt)
{
	...
	for (i = 0; i < slice_count; i++)
	{
		...
		if (i + 1 < slice_count)
		{
			...
			init_get_bits(&s->gb, buf + get_slice_offset(avctx, slices_hdr, i + 1),
			              (buf_size - get_slice_offset(avctx, slices_hdr, i + 1)) * 8);
