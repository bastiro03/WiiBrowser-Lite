...
static int mga_vid_ioctl(struct inode* inode, struct file* file, unsigned int cmd, unsigned long arg)
{
	...
	case MGA_VID_CONFIG:
		...
		card->src_base = (card->ram_size * 0x100000 - card->config.num_frames * card->config.frame_size - card->
			top_reserved);
		if (card->src_base < 0)
		{
...
static ssize_t mga_vid_write(struct file* file, const char* buf, size_t count, loff_t* ppos)
{
	...
	else if (memcmp(buf, PARAM_CONTRAST, min(count, strlen(PARAM_CONTRAST))) == 0)
	{
		short contrast;
	...
	else if (memcmp(buf, PARAM_BLACKIE, min(count, strlen(PARAM_BLACKIE))) == 0)
	{
		short blackie;
...
static void cards_init(mga_card_t* card, struct pci_dev* dev, int card_number, int is_g400)
{
	...
	else
	{
		...
		if (card->is_g400)
		{
			...
			// SDRAM:
			case 0x00:
			case 0x04: card->ram_size = 16;
				break;
			case 0x03:
			case 0x05: card->ram_size = 32;
				break;
			// SGRAM:
			case 0x10:
			case 0x14: card->ram_size = 32;
				break;
			case 0x11:
			case 0x12: card->ram_size = 16;
				break;
			default:
				card->ram_size = 16;
