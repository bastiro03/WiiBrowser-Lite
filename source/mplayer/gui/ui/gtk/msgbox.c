...
GtkWidget* create_MessageBox(void)
{
	...
	gtk_widget_set_events(
		MessageBox, GDK_EXPOSURE_MASK | GDK_KEY_PRESS_MASK | GDK_KEY_RELEASE_MASK | GDK_VISIBILITY_NOTIFY_MASK);
	...
	pixmapwid = gdk_pixmap_colormap_create_from_xpm_d(MessageBox->window, gdk_colormap_get_system(), &mask,
	                                                  &pixmapstyle->bg[GTK_STATE_NORMAL], (gchar**)warning_xpm);
	WarningPixmap = gtk_pixmap_new(pixmapwid, mask);
	pixmapwid = gdk_pixmap_colormap_create_from_xpm_d(MessageBox->window, gdk_colormap_get_system(), &mask,
	                                                  &pixmapstyle->bg[GTK_STATE_NORMAL], (gchar**)error_xpm);
