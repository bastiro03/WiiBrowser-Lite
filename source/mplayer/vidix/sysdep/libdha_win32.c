...
void* map_phys_mem(unsigned long base, unsigned long size)
{
	if (!IsWinNT())
	{
		...
		return req.mdr_LinearAddress;
	}
	dhahelper_t dhahelper_priv;
	DWORD dwBytesReturned;
	dhahelper_priv.size = size;
	dhahelper_priv.base = base;
	if (hDriver == INVALID_HANDLE_VALUE)hDriver = CreateFile("\\\\.\\DHAHELPER", GENERIC_READ | GENERIC_WRITE, 0, NULL,
	                                                         OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	if (!DeviceIoControl(hDriver, IOCTL_DHAHELPER_MAPPHYSTOLIN, &dhahelper_priv, sizeof(dhahelper_t), &dhahelper_priv,
	                     sizeof(dhahelper_t), &dwBytesReturned, NULL))
	{
...
void unmap_phys_mem(void* ptr, unsigned long size)
{
	if (IsWinNT())
	{
		...
		DeviceIoControl(hDriver, IOCTL_DHAHELPER_UNMAPPHYSADDR, &dhahelper_priv, sizeof(dhahelper_t), NULL, 0,
