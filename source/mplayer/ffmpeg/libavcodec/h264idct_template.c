...
void FUNCC(ff_h264_idct_add16)(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
...
void FUNCC(ff_h264_idct_add16intra)(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
                                    const uint8_t nnzc[15 * 8])
{
	int i;
	for (i = 0; i < 16; i++)
...
void FUNCC(ff_h264_idct8_add4)(uint8_t* dst, const int* block_offset, DCTELEM* block, int stride,
                               const uint8_t nnzc[15 * 8])
{
	int i;
	for (i = 0; i < 16; i += 4)
	{
		...
		{
			if (nnz == 1 && ((dctcoef*)block)[i * 16])
				FUNCC(ff_h264_idct8_dc_add)(dst + block_offset[i], block + i * 16 * sizeof(pixel), stride);
			else
				FUNCC(ff_h264_idct8_add)(dst + block_offset[i], block + i * 16 * sizeof(pixel), stride);
		}
	}
}

void FUNCC(ff_h264_idct_add8)(uint8_t** dest, const int* block_offset, DCTELEM* block, int stride,
                              const uint8_t nnzc[15 * 8])
{
	int i, j;
	for (j = 1; j < 3; j++)
...
void FUNCC(ff_h264_idct_add8_422)(uint8_t** dest, const int* block_offset, DCTELEM* block, int stride,
                                  const uint8_t nnzc[15 * 8])
{
	int i, j;
...
#define stride 16
	...
	dctcoef* input = p_input;
	dctcoef* output = p_output;
