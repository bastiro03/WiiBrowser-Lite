...
int display_openfilewindow(gui_t* gui, int add)
{
	...
	fileopen.Flags = OFN_ALLOWMULTISELECT | OFN_FILEMUSTEXIST | OFN_LONGNAMES | OFN_EXPLORER | OFN_READONLY |
		OFN_HIDEREADONLY;
...
static LRESULT CALLBACK OpenUrlWndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	/*subclass the edit box to capture the VK_RETURN key*/
		OldUrlWndProc = (WNDPROC)SetWindowLongPtr(url, GWLP_WNDPROC, SubUrlWndProc);

		if ((f = fopen(history, "r")))
		{
			char lasturl[MAX_PATH];
			fgets(lasturl, MAX_PATH, f);
			SendMessage(url, WM_SETTEXT, 0, lasturl);
	...
	case WM_COMMAND:
		{
			...
			case ID_OK:
				{
					char file[MAX_PATH];
					SendMessage(url, WM_GETTEXT, MAX_PATH, file);
	...
	case WM_CREATE:
		{
			...
			if (skinHandle != INVALID_HANDLE_VALUE)
			{
				do
				{
					...
					SendDlgItemMessage(hwnd, ID_DIR, LB_ADDSTRING, 0, finddata.cFileName);
				}
	...
	case WM_COMMAND:
		{
			...
			case ID_DEFAULTS:
				if (guiInfo.sh_video && guiInfo.Playing)
				{
					...
					SendDlgItemMessage(hwnd, ID_EQ0, TBM_SETPOS, 1, 100);
					set_video_colors(guiInfo.sh_video, "brightness", vo_gamma_brightness);

					vo_gamma_contrast = 0;
					SendDlgItemMessage(hwnd, ID_EQ1, TBM_SETPOS, 1, 100);
					set_video_colors(guiInfo.sh_video, "contrast", vo_gamma_contrast);

					vo_gamma_hue = 0;
					SendDlgItemMessage(hwnd, ID_EQ2, TBM_SETPOS, 1, 100);
					set_video_colors(guiInfo.sh_video, "hue", vo_gamma_hue);

					vo_gamma_saturation = 0;
					SendDlgItemMessage(hwnd, ID_EQ3, TBM_SETPOS, 1, 100);
					set_video_colors(guiInfo.sh_video, "saturation", vo_gamma_saturation);
				}
				break;
			}
