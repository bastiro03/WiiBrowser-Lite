...
static void PREFIX_no_rnd_vc1_chroma_mc8_altivec(uint8_t* dst, uint8_t* src, int stride, int h, int x, int y)
{
	...
	vec_u8 fperm;
	const vec_s32 vABCD = vec_ld(0, ABCD);
	const vec_s16 vA = vec_splat((vec_s16)vABCD, 
	1
	)
	const vec_s16 vB = vec_splat((vec_s16)vABCD, 
	3
	)
	const vec_s16 vC = vec_splat((vec_s16)vABCD, 
	5
	)
	const vec_s16 vD = vec_splat((vec_s16)vABCD, 
	7
	)
	LOAD_ZERO;
