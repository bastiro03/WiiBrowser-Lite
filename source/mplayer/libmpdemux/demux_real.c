...
read_index:
	...
	if (i != 0)
		mp_msg(MSGT_DEMUX, MSGL_WARN,
		       "Hmm, index table with unknown version (%d), please report it to MPlayer developers!\n", i);
	...
	if ((unsigned)stream_id < MAX_STREAMS)
	{
		...
		if (priv->index_table_size[stream_id] >= priv->index_malloc_size[stream_id])
		{
			...
			priv->index_table[stream_id] = realloc(priv->index_table[stream_id],
			                                       priv->index_malloc_size[stream_id] * sizeof(priv->index_table[0][
				                                       0]));
		}
...
double real_fix_timestamp(unsigned char* buf, unsigned int timestamp, unsigned int format, int64_t* kf_base,
                          int* kf_pts, double* pts)
{
	...
	if (format == mmioFOURCC('R', 'V', '3', '0') || format == mmioFOURCC('R', 'V', '4', '0'))
	{
		...
		mp_msg(MSGT_DEMUX, MSGL_DBG2, "\nTS: %08X -> %08X (%04X) %d %02X %02X %02X %02X %5u\n", timestamp, kf, orig_kf,
		       pict_type, s[0], s[1], s[2], s[3], pts ? kf - (unsigned int)(*pts * 1000.0) : 0);
	}
	v_pts = kf * 0.001f;
	...
	while (!stream_eof(demuxer->stream))
	{
		...
		if (priv->is_multirate && priv->stream_switch)
		{
			...
			if (!demuxer->audio->eof && demuxer->video->eof)
				...
				stream_seek(demuxer->stream, priv->video_curpos);
			// Get video
		...
		got_audio:
			...
			// parse audio chunk:
			{
					...
					(priv->intl_id[stream_id] == mmioFOURCC('s', 'i', 'p', 'r')))
				{
					...
					case mmioFOURCC('g', 'e', 'n', 'r'):
						...
						for (x = 0; x < w / sps; x++)
							stream_read(demuxer->stream,
							            priv->audio_buf + sps * (sph * x + ((sph + 1) / 2) * (spc & 1) +
								            (spc >> 1)), sps);
						break;
					...
					priv->audio_timestamp[priv->sub_packet_cnt] = (priv->a_pts == timestamp)
						                                              ? MP_NOPTS_VALUE
						                                              : (timestamp / 1000.0);
					priv->a_pts = timestamp;
			...
			if (((!demuxer->video->sh && index_mode == 2) || priv->is_multirate) && (unsigned)demuxer->audio->id <
				MAX_STREAMS)
			{
				...
				while (len > 2)
				{
					...
					else
					{
						...
						if (!(vpkg_length & 0xC000))
						{
							...
							mp_dbg(MSGT_DEMUX, MSGL_DBG2, "l+: %02X %02X ", (vpkg_length >> 8) & 0xff,
							       vpkg_length & 0xff);
							len -= 2;
						}
						else
						...
						if (!(vpkg_offset & 0xC000))
						{
							...
							mp_dbg(MSGT_DEMUX, MSGL_DBG2, "o+: %02X %02X ", (vpkg_offset >> 8) & 0xff,
							       vpkg_offset & 0xff);
							len -= 2;
						}
						else
					...
					if (ds->asf_packet)
					{
						...
						else
						{
							...
							if (0x80 == (vpkg_header & 0xc0))
							{
								...
								if (vpkg_offset > dp->len - sizeof(dp_hdr_t) - dp_hdr->len) vpkg_offset = dp->len -
									sizeof(dp_hdr_t) - dp_hdr->len;
								stream_read(demuxer->stream, dp_data + dp_hdr->len, vpkg_offset);
								if ((dp_data[dp_hdr->len] & 0x20) && (sh_video->format == 0x30335652)) --dp_hdr->chunks;
								else
							...
							if (len > dp->len - sizeof(dp_hdr_t) - dp_hdr->len) len = dp->len - sizeof(dp_hdr_t) -
								dp_hdr->len;
					...
					extra = dp->buffer + dp_hdr->chunktab;
					AV_WL32(extra, 1);
					AV_WL32(extra + 4, 0); // offset of the first chunk
		...
		if ((unsigned)stream_id < MAX_STREAMS)
		{
			if (demuxer->audio->id == -1 && demuxer->a_streams[stream_id])
			{
				...
				sh->ds = demuxer->audio;
				demuxer->audio->sh = sh;
				priv->audio_buf = calloc(priv->sub_packet_h[demuxer->audio->id],
...
static void demux_seek_real(demuxer_t* demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	if (streams & 1)
	{
		// use the video index if we have one
