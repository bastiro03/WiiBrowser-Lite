...
{
	if (static_cast<unsigned>(x - this->GetLeft()) < static_cast<unsigned>(textWidth)
		&& static_cast<unsigned>(y - this->GetTop()) < static_cast<unsigned>(size /**GetLinesCount()*/))
		return true;
	return false;
}
...
void GuiText::Draw()
{
	...
	if (wrap)
	{
		if (textDynNum == 0)
		{
			...
			while (ch < textlen && linenum < linestodraw)
			{
				...
				if (currentWidth > ((linenum == 0 || (style & FTGX_JUSTIFY_MASK) != FTGX_JUSTIFY_LEFT)
					                    ? maxWidth
					                    : screenwidth - 80))
				{
					if (lastSpace >= 0)
		...
		for (int i = 0; i < textDynNum; ++i)
		{
			...
			else (font ? font : fontSystem[currentSize])->drawText(leftPoint, top + i * lineheight, textDyn[i], c,
			                                                       style);
		}
	}
	else
	{
		...
		{
			if ((font ? font : fontSystem[currentSize])->getWidth(text) > maxWidth && (FrameTimer % textScrollDelay ==
				0))
			{
				...
				{
					++textScrollPos;
					if (static_cast<u32>(textScrollPos) > textlen - 1)
					{
						textScrollPos = 0;
						textScrollInitialDelay = TEXT_SCROLL_INITIAL_DELAY;
					}
					...
					{
						int i = 0;
						while ((font ? font : fontSystem[currentSize])->getWidth(textDyn[0]) < maxWidth && dynlen + 1 <
							textlen)
						{
