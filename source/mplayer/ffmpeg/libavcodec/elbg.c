...
static int get_closest_codebook(elbg_data* elbg, int index)
{
	...
	for (i = 0; i < elbg->numCB; i++)
		if (i != index)
		{
			diff = distance_limited(elbg->codebook + i * elbg->dim, elbg->codebook + index * elbg->dim, elbg->dim,
			                        diff_min);
			if (diff < diff_min)
			{
				pick = i;
                      ...
                      cell* cells)
{
	...
	{
		int dist[2] = {
			distance_limited(centroid[0], points + tempcell->index * dim, dim, INT_MAX),
			distance_limited(centroid[1], points + tempcell->index * dim, dim, INT_MAX)
		};
                ...
                AVLFG* rand_state)
{
	...
	do
	{
		...
		{
			best_dist = distance_limited(elbg->points + i * elbg->dim, elbg->codebook + best_idx * elbg->dim, dim,
			                             INT_MAX);
