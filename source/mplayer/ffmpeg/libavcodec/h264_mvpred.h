...
int fetch_diagonal_mv(H264Context* h, const int16_t** C,
...
void pred_motion(H264Context* const h, int n,
...
void pred_16x8_motion(H264Context* const h,
...
void pred_8x16_motion(H264Context* const h,
                      int n, int list, int ref,
                      int* const mx, int* const my)
{
	if (n == 0)
...
void pred_pskip_motion(H264Context* const h)
{
...
static void fill_decode_neighbors(H264Context* h, int mb_type)
{
	...
	static const uint8_t left_block_options[4][32] = {
		{
			0, 1, 2, 3, 7, 10, 8, 11, 3 + 0 * 4, 3 + 1 * 4, 3 + 2 * 4, 3 + 3 * 4, 1 + 4 * 4, 1 + 8 * 4, 1 + 5 * 4,
			1 + 9 * 4
		},
		{
			2, 2, 3, 3, 8, 11, 8, 11, 3 + 2 * 4, 3 + 2 * 4, 3 + 3 * 4, 3 + 3 * 4, 1 + 5 * 4, 1 + 9 * 4, 1 + 5 * 4,
			1 + 9 * 4
		},
		{
			0, 0, 1, 1, 7, 10, 7, 10, 3 + 0 * 4, 3 + 0 * 4, 3 + 1 * 4, 3 + 1 * 4, 1 + 4 * 4, 1 + 8 * 4, 1 + 4 * 4,
			1 + 8 * 4
		},
		{
			0, 2, 0, 2, 7, 10, 7, 10, 3 + 0 * 4, 3 + 2 * 4, 3 + 0 * 4, 3 + 2 * 4, 1 + 4 * 4, 1 + 8 * 4, 1 + 4 * 4,
			1 + 8 * 4
		}
	};
				...
				for (i = 0; i < 2; i++)
				{
					...
					else
					{
						h->intra4x4_pred_mode_cache[3 + 8 * 1 + 2 * 8 * i] =
							h->intra4x4_pred_mode_cache[3 + 8 * 2 + 2 * 8 * i] = 2 - 3 * !(left_type[LEFT(i)] &
								type_mask);
					}
				}
			}
			...
			if (mb_type & (MB_TYPE_16x8 | MB_TYPE_8x8))
			{
				for (i = 0; i < 2; i++)
				{
					...
					else
					{
						...
						ref_cache[cache_idx] =
							ref_cache[cache_idx + 8] = (left_type[LEFT(i)])
								                           ? LIST_NOT_USED
								                           : PART_NOT_AVAILABLE;
					}
				}
