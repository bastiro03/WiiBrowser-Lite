...
void mc_dir_part(H264Context* h, Picture* pic,
...
void mc_part_std(H264Context* h, int n, int square,
...
void mc_part_weighted(H264Context* h, int n, int square,
                      int height, int delta,
                      uint8_t* dest_y, uint8_t* dest_cb,
                      uint8_t* dest_cr,
                      int x_offset, int y_offset,
...
void mc_part(H264Context* h, int n, int square,
             int height, int delta,
             uint8_t* dest_y, uint8_t* dest_cb,
             uint8_t* dest_cr,
             int x_offset, int y_offset,
...
void prefetch_motion(H264Context* h, int list,
                     int pixel_shift, int chroma_idc)
{
...
void hl_motion(H264Context* h, uint8_t* dest_y,
...
void hl_motion_420(H264Context* h, uint8_t* dest_y,
                   uint8_t* dest_cb, uint8_t* dest_cr,
                   qpel_mc_func (*qpix_put)[16],
                   h264_chroma_mc_func (*chroma_put),
                   qpel_mc_func (*qpix_avg)[16],
...
void hl_motion_422(H264Context* h, uint8_t* dest_y,
                   uint8_t* dest_cb, uint8_t* dest_cr,
                   qpel_mc_func (*qpix_put)[16],
                   h264_chroma_mc_func (*chroma_put),
                   qpel_mc_func (*qpix_avg)[16],
...
void common_init(H264Context* h)
{
	MpegEncContext* const s = &h->s;
                                        ...
                                        const AVCodecContext* src)
{
	...
	// SPS/PPS
	copy_parameter_set(h->sps_buffers, h1->sps_buffers,
	                   MAX_SPS_COUNT, sizeof(SPS));
	h->sps = h1->sps;
	copy_parameter_set(h->pps_buffers, h1->pps_buffers,
...
int ff_h264_frame_start(H264Context* h)
{
	...
	for (i = 0; i < 16; i++)
	{
		h->block_offset[i] = (4 * ((scan8[i] - scan8[0]) & 7) << pixel_shift) + 4 * s->linesize * ((scan8[i] - scan8[0])
			>> 3);
		h->block_offset[48 + i] = (4 * ((scan8[i] - scan8[0]) & 7) << pixel_shift) + 8 * s->linesize * ((scan8[i] -
			scan8[0]) >> 3);
	}
	for (i = 0; i < 16; i++)
	{
		h->block_offset[16 + i] =
			h->block_offset[32 + i] = (4 * ((scan8[i] - scan8[0]) & 7) << pixel_shift) + 4 * s->uvlinesize * ((scan8[i]
				- scan8[0]) >> 3);
		h->block_offset[48 + 16 + i] =
			h->block_offset[48 + 32 + i] = (4 * ((scan8[i] - scan8[0]) & 7) << pixel_shift) + 8 * s->uvlinesize * ((
				scan8[i] - scan8[0]) >> 3);
	}
...
static void decode_postinit(H264Context* h, int setup_finished)
{
	...
	if (cur->f.pict_type == AV_PICTURE_TYPE_B
		|| (h->last_pocs[MAX_DELAYED_PIC_COUNT - 2] > INT_MIN && h->last_pocs[MAX_DELAYED_PIC_COUNT - 1] - h->last_pocs[
			MAX_DELAYED_PIC_COUNT - 2] > 2))
...
void backup_mb_border(H264Context* h, uint8_t* src_y,
...
void xchg_mb_border(H264Context* h, uint8_t* src_y,
                    uint8_t* src_cb, uint8_t* src_cr,
                    int linesize, int uvlinesize,
...
int dctcoef_get(DCTELEM* mb, int high_bit_depth,
                int index)
{
	if (high_bit_depth)
	{
...
void dctcoef_set(DCTELEM* mb, int high_bit_depth,
                 int index, int value)
{
	if (high_bit_depth)
	{
...
void hl_decode_mb_predict_luma(H264Context* h,
...
void hl_decode_mb_idct_luma(H264Context* h, int mb_type,
...
void hl_decode_mb_internal(H264Context* h, int simple,
                           int pixel_shift)
{
	MpegEncContext* const s = &h->s;
			...
			else
			{
				...
				else if (CONFIG_SVQ3_DECODER)
				{
					h->h264dsp.h264_chroma_dc_dequant_idct(h->mb + 16 * 16 * 1,
					                                       h->dequant4_coeff[IS_INTRA(mb_type) ? 1 : 4][h->chroma_qp[0]]
					                                       [0]);
					h->h264dsp.h264_chroma_dc_dequant_idct(h->mb + 16 * 16 * 2,
					                                       h->dequant4_coeff[IS_INTRA(mb_type) ? 2 : 5][h->chroma_qp[1]]
					                                       [0]);
					for (j = 1; j < 3; j++)
					{
						for (i = j * 16; i < j * 16 + 4; i++)
...
void hl_decode_mb_444_internal(H264Context* h,
                               int simple,
                               int pixel_shift)
{
	MpegEncContext* const s = &h->s;
...
void hl_decode_mb_complex(H264Context* h)
{
...
void hl_decode_mb_444_complex(H264Context* h)
{
...
void hl_decode_mb_444_simple(H264Context* h)
{
...
static int decode_slice_header(H264Context* h, H264Context* h0)
{
	...
	if (!s->context_initialized)
	{
		...
		s->avctx->height -= (1 << s->chroma_y_shift) * FFMIN(h->sps.crop_bottom, (16 >> s->chroma_y_shift) - 1) * (2 - h
		...
		{
			s->avctx->color_range = h->sps.full_range > 0
				                        ? AVCOL_RANGE_JPEG
				                        : AVCOL_RANGE_MPEG;
		...
		case 8:
			if (CHROMA444)
			{
				s->avctx->pix_fmt = s->avctx->color_range == AVCOL_RANGE_JPEG
					                    ? PIX_FMT_YUVJ444P
					                    : PIX_FMT_YUV444P;
				if (s->avctx->colorspace == AVCOL_SPC_RGB)
				{
		...
		if (s0->first_field)
		{
			...
			else
			{
				...
				else
				{
					...
					if (last_pic_dropable != s->dropable)
					{
						av_log(s->avctx, AV_LOG_ERROR,
...
void fill_filter_caches_inter(H264Context* h,
                              ...
                              int mb_xy, int list)
{
	...
	if (IS_INTER(mb_type) || IS_DIRECT(mb_type))
	{
		if (USES_LIST(top_type, list))
		{
			...
			int (*ref2frm)[64] = (void*)(h->ref2frm[h->slice_table[top_xy] & (MAX_SLICES - 1)][0] + (
				MB_MBAFF ? 20 : 2));
		...
		if (!IS_INTERLACED(mb_type ^ left_type[LTOP]))
		{
			if (USES_LIST(left_type[LTOP], list))
			{
				...
				int (*ref2frm)[64] = (void*)(h->ref2frm[h->slice_table[left_xy[LTOP]] & (MAX_SLICES - 1)][0] + (
					MB_MBAFF ? 20 : 2));
		...
		for (;;)
		{
			...
			if (s->avctx->debug & FF_DEBUG_STARTCODE)
				av_log(h->s.avctx, AV_LOG_DEBUG, "NAL %d/%d at %d/%d length %d pass %d\n", hx->nal_unit_type,
				       hx->nal_ref_idc, buf_index, buf_size, dst_length, pass);
		...
		again:
			...
			case NAL_SLICE:
				...
				if (h->sei_recovery_frame_cnt >= 0
					&& (h->recovery_frame < 0
						|| ((h->recovery_frame - h->frame_num) & ((1 << h->sps.log2_max_frame_num) - 1)) > h->
                        ...
                        int* data_size, AVPacket* avpkt)
{
	...
	if (h->is_avc && buf_size >= 9 && buf[0] == 1 && buf[2] == 0 && (buf[4] & 0xFC) == 0xFC && (buf[5] & 0x1F) && buf[8]
		== 0x67)
	{
		int cnt = buf[5] & 0x1f;
...
static const AVOption h264_options[] = {
	...
	{
		"nal_length_size", "nal_length_size", offsetof(H264Context, nal_length_size), FF_OPT_TYPE_INT, {.dbl = 0}, 0, 4,
		0
	},
	{NULL}
};
