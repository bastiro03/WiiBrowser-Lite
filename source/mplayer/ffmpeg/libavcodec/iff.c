                          ...
                          const AVPacket* const avpkt)
{
	...
	if (buf_size > 8)
	{
		...
		if (s->ham)
		{
			...
			ham_count = 8 * (1 << s->ham);
			s->ham_palbuf = av_malloc(
				(ham_count << !!(s->masking == MASK_HAS_MASK)) * sizeof(uint32_t) + FF_INPUT_BUFFER_PADDING_SIZE);
			if (!s->ham_palbuf)
			{
				av_freep(&s->ham_buf);
				return AVERROR(ENOMEM);
			...
			if (count)
			{
				// HAM with color palette attached
			...
			{
				// HAM with grayscale color palette
				count = 1 << s->ham;
				for (i = 0; i < count; i++)
				{
