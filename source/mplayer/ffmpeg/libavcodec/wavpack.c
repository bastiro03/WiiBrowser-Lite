...
int get_tail(GetBitContext* gb, int k)
{
	int p, e, res;
...
int wv_alloc_frame_context(WavpackContext* c)
{
...
int wavpack_decode_init(AVCodecContext* avctx)
{
...
int wavpack_decode_end(AVCodecContext* avctx)
{
	WavpackContext* s = avctx->priv_data;
	int i;
                                ...
                                const uint8_t* buf, int buf_size)
{
	...
	while (buf < buf_end)
	{
		...
		case WP_ID_CHANINFO:
			...
			case 0: chmask = *buf;
				break;
			case 1: chmask = AV_RL16(buf);
				break;
			case 2: chmask = AV_RL24(buf);
				break;
			case 3: chmask = AV_RL32(buf);
				break;
			case 5:
				chan |= (buf[1] & 0xF) << 8;
