...
static int mpeg4_decode_sprite_trajectory(MpegEncContext* s, GetBitContext* gb)
{
	...
	virtual_ref[0][0] = 16 * (vop_ref[0][0] + w2)
		+ ROUNDED_DIV(
			((w - w2) * (r * sprite_ref[0][0] - 16 * vop_ref[0][0]) + w2 * (r * sprite_ref[1][0] - 16 * vop_ref[1][0])),
			w);
	virtual_ref[0][1] = 16 * vop_ref[0][1]
		+ ROUNDED_DIV(
			((w - w2) * (r * sprite_ref[0][1] - 16 * vop_ref[0][1]) + w2 * (r * sprite_ref[1][1] - 16 * vop_ref[1][1])),
			w);
	virtual_ref[1][0] = 16 * vop_ref[0][0]
		+ ROUNDED_DIV(
			((h - h2) * (r * sprite_ref[0][0] - 16 * vop_ref[0][0]) + h2 * (r * sprite_ref[2][0] - 16 * vop_ref[2][0])),
			h);
	virtual_ref[1][1] = 16 * (vop_ref[0][1] + h2)
		+ ROUNDED_DIV(
			((h - h2) * (r * sprite_ref[0][1] - 16 * vop_ref[0][1]) + h2 * (r * sprite_ref[2][1] - 16 * vop_ref[2][1])),
			h);
                           ...
                           DCTELEM block[6][64])
{
	...
	if (s->codec_id == CODEC_ID_MPEG4)
	{
		...
		if (next)
		{
			...
			if (s->mb_x + s->mb_y * s->mb_width + 1 >= next)
				return SLICE_END;
...
static int decode_vop_header(MpegEncContext* s, GetBitContext* gb)
{
	...
	if (s->pict_type == AV_PICTURE_TYPE_B && s->low_delay && s->vol_control_parameters == 0 && !(s->flags &
	...
	if (s->pict_type == AV_PICTURE_TYPE_S && (s->vol_sprite_usage == STATIC_SPRITE || s->vol_sprite_usage ==
		GMC_SPRITE))
	{
	...
	if (s->shape != BIN_ONLY_SHAPE)
	{
		...
		if (s->avctx->debug & FF_DEBUG_PICT_INFO)
		{
			av_log(s->avctx, AV_LOG_DEBUG,
			       "qp:%d fc:%d,%d %s size:%d pro:%d alt:%d top:%d %spel part:%d resync:%d w:%d a:%d rnd:%d vot:%d%s dc:%d ce:%d/%d/%d time:%d tincr:%d\n",
			       s->qscale, s->f_code, s->b_code,
			       s->pict_type == AV_PICTURE_TYPE_I
				       ? "I"
				       : (s->pict_type == AV_PICTURE_TYPE_P ? "P" : (s->pict_type == AV_PICTURE_TYPE_B ? "B" : "S")),
			       gb->size_in_bits, s->progressive_sequence, s->alternate_scan, s->top_field_first,
			       s->quarter_sample ? "q" : "h", s->data_partitioning, s->resync_marker, s->num_sprite_warping_points,
			       s->sprite_warping_accuracy, 1 - s->no_rounding, s->vo_type,
...
int ff_mpeg4_decode_picture_header(MpegEncContext* s, GetBitContext* gb)
{
	...
	for (;;)
	{
		...
		{
			if (gb->size_in_bits == 8 && (s->divx_version >= 0 || s->xvid_build >= 0) || s->codec_tag ==
				AV_RL32("QMP4"))
			{
...
static const AVOption mpeg4_options[] = {
	...
	{
		"divx_packed", "divx style packed b frames", offsetof(MpegEncContext, divx_packed), FF_OPT_TYPE_INT, {.dbl = 0},
		0, 1, 0
	},
	{NULL}
};
