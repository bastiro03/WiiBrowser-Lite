...
static void store_slice2_c(uint8_t* dst, int16_t* src, int dst_stride, int src_stride, int width, int height,
                           int log2_scale)
{
	int y, x;
#define STORE2(pos)                                                        \
                   ...
                   uint8_t* qp_store, int qp_stride, int is_luma)
{
	...
	int32_t __attribute__((aligned(32)))
	block_align[4 * 8 * BLOCKSZ + 4 * 8 * BLOCKSZ];
	...
	for (y = step; y < height + 8; y += step)
	{
		...
		for (x0 = 0; x0 < width + 8 - 8 * (BLOCKSZ - 1); x0 += 8 * (BLOCKSZ - 1))
		{
			...
			if (p->qp)
				column_fidct_s((int16_t*)(&p->threshold_mtx[0]), block + 0 * 8, block3 + 0 * 8, 8 * (BLOCKSZ - 1));
				...
				for (x = 0; x < 8 * (BLOCKSZ - 1); x += 8)
				{
					...
					column_fidct_s((int16_t*)(&p->threshold_mtx[0]), block + x * 8, block3 + x * 8, 8);
					//yes, this is a HOTSPOT
				}
		...
		row_idct_s(block3 + 0 * 8, p->temp + (y & 15) * stride + x0 + 2 - (y & 1), stride, es >> 2);
		{
			...
			{
				if (y1 & 8)
					store_slice_s(dst + (y1 - 8) * dst_stride, p->temp + 8 + 8 * stride,
					              dst_stride, stride, width, 8, 5 - p->log2_count);
				else
					store_slice2_s(dst + (y1 - 8) * dst_stride, p->temp + 8 + 0 * stride,
					               dst_stride, stride, width, 8, 5 - p->log2_count);
			}
		}
	}
	...
	if (y & 7)
	{
		// == height & 7
		if (y & 8)
			store_slice_s(dst + ((y - 8) & ~7) * dst_stride, p->temp + 8 + 8 * stride,
			              dst_stride, stride, width, y & 7, 5 - p->log2_count);
		else
			store_slice2_s(dst + ((y - 8) & ~7) * dst_stride, p->temp + 8 + 0 * stride,
			               dst_stride, stride, width, y & 7, 5 - p->log2_count);
	}
}
...
static void column_fidct_c(int16_t* thr_adr, DCTELEM* data, DCTELEM* output, int cnt)
{
	...
	{
		//start positions
		threshold = thr_adr; //threshold_mtx
