...
static void (*store_slice)(uint8_t* dst, int16_t* src, int dst_stride, int src_stride, int width, int height,
                           int log2_scale) = store_slice_c;

static void (*requantize)(DCTELEM dst[64], DCTELEM src[64], int qp, uint8_t* permutation) = hardthresh_c;

static void filter(struct vf_priv_s* p, uint8_t* dst, uint8_t* src, int dst_stride, int src_stride, int width,
                   int height, uint8_t* qp_store, int qp_stride, int is_luma)
{
	int x, y, i;
	...
	for (y = 0; y < height + 8; y += 8)
	{
		...
		if (y)
			store_slice(dst + (y - 8) * dst_stride, p->temp + 8 + y * stride, dst_stride, stride, width,
			            XMIN(8, height + 8 - y), 6 - p->log2_count);
	}
#if 0
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (vf->priv->log2_count || !(mpi->flags & MP_IMGFLAG_DIRECT))
	{
		...
		{
			filter(vf->priv, dmpi->planes[0], mpi->planes[0], dmpi->stride[0], mpi->stride[0], mpi->w, mpi->h, qp_tab,
			       mpi->qstride, 1);
...
static int vf_open(vf_instance_t* vf, char* args)
{
	...
	{
	default:
	case 0: requantize = hardthresh_c;
		break;
	case 1: requantize = softthresh_c;
		break;
	}
