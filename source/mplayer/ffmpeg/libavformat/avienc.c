...
static int avi_write_header(AVFormatContext* s)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		case AVMEDIA_TYPE_VIDEO: ffio_wfourcc(pb, "vids");
			break;
		case AVMEDIA_TYPE_AUDIO: ffio_wfourcc(pb, "auds");
			break;
		//      case AVMEDIA_TYPE_TEXT : ffio_wfourcc(pb, "txts"); break;
		case AVMEDIA_TYPE_DATA: ffio_wfourcc(pb, "dats");
			break;
		}
		if (stream->codec_type == AVMEDIA_TYPE_VIDEO ||
...
static int avi_write_ix(AVFormatContext* s)
{
	...
	for (i = 0; i < s->nb_streams; i++)
	{
		...
		for (j = 0; j < avist->indexes.entry; j++)
		{
			...
			avio_wl32(pb, (ie->len & ~0x80000000) |
...
static int avi_write_packet(AVFormatContext* s, AVPacket* pkt)
{
	...
	while (enc->block_align == 0 && pkt->dts != AV_NOPTS_VALUE && pkt->dts > avist->packet_count && enc->codec_id !=
		CODEC_ID_XSUB)
	{
