...
static int sis_config_playback(vidix_playback_t* info)
{
	...
	overlay.keyOP = (sis_grkey.ckey.op == CKEY_TRUE ? VI_ROP_DestKey : VI_ROP_Always);
...
static int sis_frame_select(unsigned int frame)
{
	...
	/* set Y start address */
	setvideoreg(Index_VI_Disp_Y_Buf_Start_Low, PSY);
	setvideoreg(Index_VI_Disp_Y_Buf_Start_Middle, PSY >> 8);
	setvideoreg(Index_VI_Disp_Y_Buf_Start_High, PSY >> 16);
	...
	if ((sis_format == IMGFMT_YV12) || (sis_format == IMGFMT_I420))
	{
		...
		setvideoreg(Index_VI_U_Buf_Start_Low, PSU);
		setvideoreg(Index_VI_U_Buf_Start_Middle, PSU >> 8);
		setvideoreg(Index_VI_U_Buf_Start_High, PSU >> 16);

		setvideoreg(Index_VI_V_Buf_Start_Low, PSV);
		setvideoreg(Index_VI_V_Buf_Start_Middle, PSV >> 8);
		setvideoreg(Index_VI_V_Buf_Start_High, PSV >> 16);
...
static void set_overlay(SISOverlayPtr pOverlay, int index)
{
	...
	/* Set Y buf pitch */
	setvideoreg(Index_VI_Disp_Y_Buf_Pitch_Low, pitch);
	setvideoregmask(Index_VI_Disp_Y_UV_Buf_Pitch_Middle,
	                pitch >> 8, 0x0f);

	/* Set Y start address */
	setvideoreg(Index_VI_Disp_Y_Buf_Start_Low, pOverlay->PSY);
	...
	setvideoreg(Index_VI_Hor_Post_Up_Scale_High,
	            pOverlay->HUSF >> 8);
	setvideoreg(Index_VI_Ver_Up_Scale_Low, pOverlay->VUSF);
	...
	setvideoreg(Index_VI_Win_Hor_Disp_Start_Low, left);
	setvideoreg(Index_VI_Win_Hor_Disp_End_Low, right);
	setvideoreg(Index_VI_Win_Hor_Over, h_over);

	setvideoreg(Index_VI_Win_Ver_Disp_Start_Low, top);
	setvideoreg(Index_VI_Win_Ver_Disp_End_Low, bottom);
	setvideoreg(Index_VI_Win_Ver_Over, v_over);
...
static void set_colorkey(void)
{
	...
	b = sis_grkey.ckey.blue;
	g = sis_grkey.ckey.green;
	r = sis_grkey.ckey.red;

	/* set color key mode */
	setvideoregmask(Index_VI_Key_Overlay_OP,
	                sis_grkey.ckey.op == CKEY_TRUE ? VI_ROP_DestKey : VI_ROP_Always, 0x0F);

	/* set colorkey values */
	setvideoreg(Index_VI_Overlay_ColorKey_Blue_Min, b);
	setvideoreg(Index_VI_Overlay_ColorKey_Green_Min, g);
	setvideoreg(Index_VI_Overlay_ColorKey_Red_Min, r);

	setvideoreg(Index_VI_Overlay_ColorKey_Blue_Max, b);
	setvideoreg(Index_VI_Overlay_ColorKey_Green_Max, g);
	setvideoreg(Index_VI_Overlay_ColorKey_Red_Max, r);
}
