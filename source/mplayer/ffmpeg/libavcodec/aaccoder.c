...
int quant(float coef, const float Q)
{
	float a = coef * Q;
...
static float (* const quantize_and_encode_band_cost_arr[])(
	...
	PutBitContext* pb, const float* in,
	const float* scaled, int size, int scale_idx,
	int cb, float lambda, float uplim,
	int* bits) = {
		...
		for (g = 0; g < sce->ics.num_swb; g++)
		{
			...
			if (nz)
			{
				...
				for (q = minscale; q < maxscale; q++)
				{
					...
					{
						FFPsyBand* band = &s->psy.ch[s->cur_channel].psy_bands[(w + w2) * 16 + g];
						dist += quantize_band_cost(s, coefs + w2 * 128, s->scoefs + start + w2 * 128,
						                           sce->ics.swb_sizes[g],
	...
	for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w])
	{
		...
		for (g = 0; g < sce->ics.num_swb; g++)
		{
			...
			for (;;)
			{
				...
				quant_max = quant(maxq[w * 16 + g],
				                  ff_aac_pow2sf_tab[POW_SF2_ZERO - scf + SCALE_ONE_POS - SCALE_DIV_512]);
				if (quant_max >= 8191)
				{
	...
	for (w = 0; w < sce->ics.num_windows; w += sce->ics.group_len[w])
	{
		for (g = 0; g < sce->ics.num_swb; g++)
		{
			for (w2 = 0; w2 < sce->ics.group_len[w]; w2++)
			{
				...
				{
					sce->sf_idx[(w + w2) * 16 + g] = av_clip(
						SCALE_ONE_POS - SCALE_DIV_512 + log2f(band->threshold), 80, 218);
					sce->zeroes[(w + w2) * 16 + g] = 0;
				}
