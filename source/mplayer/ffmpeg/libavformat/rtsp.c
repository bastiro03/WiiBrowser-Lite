...
const AVOption ff_rtsp_options[] = {
	{
		"initial_pause", "Don't start playing the stream immediately", OFFSET(initial_pause), AV_OPT_TYPE_INT, {0}, 0,
		1, DEC
	},
	FF_RTP_FLAG_OPTS(RTSPState, rtp_muxer_flags)
	{
		"rtsp_transport", "RTSP transport protocols", OFFSET(lower_transport_mask), AV_OPT_TYPE_FLAGS, {0}, INT_MIN,
		INT_MAX, DEC | ENC, "rtsp_transport"
	},
	...
	{
		"udp_multicast", "UDP multicast", 0, AV_OPT_TYPE_CONST, {1 << RTSP_LOWER_TRANSPORT_UDP_MULTICAST}, 0, 0, DEC,
		"rtsp_transport"
	},
	...
	{
		"min_port", "Minimum local UDP port", OFFSET(rtp_port_min), AV_OPT_TYPE_INT, {RTSP_RTP_PORT_MIN}, 0, 65535,
		DEC | ENC
	},
	{
		"max_port", "Maximum local UDP port", OFFSET(rtp_port_max), AV_OPT_TYPE_INT, {RTSP_RTP_PORT_MAX}, 0, 65535,
		DEC | ENC
	},
	{NULL},
};
...
static int rtsp_open_transport_ctx(AVFormatContext* s, RTSPStream* rtsp_st)
{
	...
	if (rt->transport != RTSP_TRANSPORT_RDT && CONFIG_RTPDEC)
	{
		if (rtsp_st->dynamic_handler)
		{
