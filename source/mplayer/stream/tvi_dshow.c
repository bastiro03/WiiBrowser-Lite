                                  ...
                                  IPin* pOutPin, IPin* pInPin)
{
	...
	if (FAILED(hr))
		mp_msg(MSGT_TV, MSGL_DBG2, "tvi_dshow: pInPin->QueryInterface(IID_IAMBufferNegotiation) Error: 0x%x",
		       (unsigned int)hr);
	else
	{
		hr = OLE_CALL_ARGS(pBN, SuggestAllocatorProperties, &prop);
                               ...
                               int* pnFirst)
{
	...
	mp_msg(MSGT_TV, MSGL_DBG4, "tvi_dshow: load_freq_table called %d (%s)\n", nCountry,
	       nInputType == TunerInputAntenna ? "broadcast" : "cable");
...
static HRESULT set_nearest_freq(priv_t* priv, long lFreq)
{
	...
	if (priv->freq_table_len == -1 && !priv->freq_table)
	{
		...
		if (load_freq_table(chanlist2country(priv->tv_param->chanlist), tunerInput, &(priv->freq_table),
		                    &(priv->freq_table_len), &(priv->first_channel)) != S_OK)
		{
			//FIXME
	...
	mp_msg(MSGT_TV, MSGL_V, "tvi_dshow: set_nearest_freq #%d (%ld)\n", nChannel,
	       priv->freq_table[nChannel - priv->first_channel]);
...
static HRESULT build_sub_graph(priv_t* priv, chain_t* chain, const GUID* ppin_category)
{
	...
	do
	{
		...
		hr = CoCreateInstance((GUID*)&CLSID_SampleGrabber, NULL, CLSCTX_INPROC_SERVER, &IID_IBaseFilter,
		                      (void*)&chain->pSGF);
		if (FAILED(hr))
		{
		...
		hr = OLE_CALL_ARGS(priv->pBuilder, FindPin, (IUnknown*)chain->pSGF, PINDIR_INPUT, NULL, NULL, FALSE, 0,
		                   &chain->pSGIn);
		if (FAILED(hr))
		{
		...
		hr = OLE_CALL_ARGS(priv->pBuilder, FindPin, (IUnknown*)chain->pSGF, PINDIR_OUTPUT, NULL, NULL, FALSE, 0,
		                   &pSGOut);
		if (FAILED(hr))
		{
		...
		{
			set_buffer_preference(20, (WAVEFORMATEX*)(chain->arpmt[nFormatProbed]->pbFormat), chain->pCapturePin,
			                      chain->pSGIn);
		}
...
static int get_device_name(IMoniker* pM, char* pBuf, int nLen)
{
	...
	var.vt = VT_BSTR;
	hr = OLE_CALL_ARGS(pPropBag, Read, L"Description", &var,
	                   NULL);
	if (FAILED(hr))
	{
		hr = OLE_CALL_ARGS(pPropBag, Read, L"FriendlyName", &var,
		                   NULL);
	}
...
static int init(priv_t* priv)
{
	...
	do
	{
		...
		if (priv->adev_index != -1)
		{
			priv->chains[1]->pCaptureFilter = find_capture_device(priv->adev_index, &CLSID_AudioInputDeviceCategory);
		...
		{
			IAMTVAudio* pTVAudio;
			hr = OLE_CALL_ARGS(priv->pBuilder, FindInterface, NULL, NULL, priv->chains[0]->pCaptureFilter,
			                   &IID_IAMTVAudio, (void*)&pTVAudio);
			if (hr == S_OK)
			{
				...
				if (FAILED(hr))
					mp_msg(MSGT_TV, MSGL_WARN, MSGTR_TVI_DS_UnableSetAudioMode, priv->tv_param->amode,
					       (unsigned int)hr);
			}
		}
...
static int control(priv_t* priv, int cmd, void* arg)
{
	...
	case TVI_CONTROL_VID_GET_SATURATION:
		return get_control(priv, VideoProcAmp_Saturation, arg);
	case TVI_CONTROL_VID_SET_BRIGHTNESS:
		return set_control(priv, VideoProcAmp_Brightness, *(int*)arg);
	case TVI_CONTROL_VID_GET_BRIGHTNESS:
		return get_control(priv, VideoProcAmp_Brightness, arg);
