...
static char* vbeErrToStr(int err)
{
	...
	if ((err & VBE_VESA_ERROR_MASK) == VBE_VESA_ERROR_MASK)
	{
		...
		switch (err)
		{
		case VBE_OK: retval = "No error";
			break;
		case VBE_VM86_FAIL: retval = "vm86() syscall failed";
			break;
		case VBE_OUT_OF_DOS_MEM: retval = "Out of DOS memory";
			break;
		case VBE_OUT_OF_MEM: retval = "Out of memory";
			break;
		case VBE_BROKEN_BIOS: retval = "Broken BIOS or DOS TSR";
			break;
		default: sprintf(sbuff, "Unknown or internal error: %i", err);
			retval = sbuff;
			break;
		}
	return retval;
...
static int draw_frame(uint8_t* src[])
{
	...
	if (sws)
	{
		...
		if (srcFourcc == IMGFMT_RGB32 || srcFourcc == IMGFMT_BGR32)
			srcStride[0] = srcW * 4;
		else if (srcFourcc == IMGFMT_RGB24 || srcFourcc == IMGFMT_BGR24)
			srcStride[0] = srcW * 3;
		else
			srcStride[0] = srcW * 2;
		if (HAS_DGA()) dst[0] += y_offset * SCREEN_LINE_SIZE(PIXEL_SIZE()) + x_offset * PIXEL_SIZE();
...
static uint32_t parseSubDevice(const char* sd)
{
	uint32_t flags;
	flags = 0;
	if (strcmp(sd, "nodga") == 0)
	{
		flags |= SUBDEV_NODGA;
		flags &= ~(SUBDEV_FORCEDGA);
	}
	else if (strcmp(sd, "dga") == 0)
	{
		flags &= ~(SUBDEV_NODGA);
		flags |= SUBDEV_FORCEDGA;
	}
	else if (strcmp(sd, "neotv_pal") == 0)
	{
		neomagic_tvout = 1;
		neomagic_tvnorm = NEO_PAL;
	}
	else if (strcmp(sd, "neotv_ntsc") == 0)
	{
		neomagic_tvout = 1;
		neomagic_tvnorm = NEO_NTSC;
	}
	else if (memcmp(sd, "lvo:", 4) == 0) lvo_name = &sd[4]; /* lvo_name will be valid within init() */
#ifdef CONFIG_VIDIX
					else
	...
	{
		mp_msg(MSGT_VO, MSGL_WARN, MSGTR_LIBVO_VESA_UnknownSubdevice, sd);
		return 0xFFFFFFFFUL;
	}
	return flags;
}
...
static char* model2str(unsigned char type)
{
	...
	switch (type)
	{
	case memText: retval = "Text";
		break;
	case memCGA: retval = "CGA";
		break;
	case memHercules: retval = "Hercules";
		break;
	case memPL: retval = "Planar";
		break;
	case memPK: retval = "Packed pixel";
		break;
	case mem256: retval = "256";
		break;
	case memRGB: retval = "Direct color RGB";
		break;
	case memYUV: retval = "Direct color YUV";
		break;
       ...
       uint32_t format)
{
	...
	{
	case IMGFMT_BGR8:
	case IMGFMT_RGB8: bpp = 8;
		break;
	case IMGFMT_BGR15:
	case IMGFMT_RGB15: bpp = 15;
		break;
	case IMGFMT_BGR16:
	case IMGFMT_RGB16: bpp = 16;
		break;
	case IMGFMT_BGR24:
	case IMGFMT_RGB24: bpp = 24;
		break;
	case IMGFMT_BGR32:
	case IMGFMT_RGB32: bpp = 32;
		break;
	default: bpp = 16;
		break;
	}
	...
	for (i = 0; i < num_modes; i++)
	{
		...
		if (mp_msg_test(MSGT_VO, MSGL_V))
		{
			mp_msg(MSGT_VO, MSGL_V, "vo_vesa: Mode (%03u): mode=%04X %ux%u@%u attr=%04X\n"
			       ...
			       , vmib.WinASegment, vmib.WinAAttributes, vmib.WinBSegment, vmib.WinBAttributes, vmib.WinSize,
			       vmib.WinGranularity
	...
	if (best_mode_idx != UINT_MAX)
	{
		...
		if ((video_mode_info.WinAAttributes & FRAME_MODE) == FRAME_MODE)
			win.idx = 0; /* frame A */
		else if ((video_mode_info.WinBAttributes & FRAME_MODE) == FRAME_MODE)
			win.idx = 1; /* frame B */
		else win.idx = -2;
		/* Try use DGA instead */
		...
		*/
		...
		if (lvo_name)
		{
			...
			mp_msg(MSGT_VO, MSGL_INFO, MSGTR_LIBVO_VESA_UsingVideoOverlay, lvo_name);
			lvo_opened = 1;
		}
#ifdef CONFIG_VIDIX
		else
