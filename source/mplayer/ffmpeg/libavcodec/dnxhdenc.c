...
static const AVOption options[] = {
	{
		"nitris_compat", "encode with Avid Nitris compatibility", offsetof(DNXHDEncContext, nitris_compat),
		AV_OPT_TYPE_INT, {.dbl = 0}, 0, 1, VE
	},
	{NULL}
};
...
void dnxhd_10bit_get_pixels_8x4_sym(DCTELEM* restrict block, const uint8_t* pixels, int line_size)
{
	int i;
...
void dnxhd_encode_dc(DNXHDEncContext* ctx, int diff)
{
	int nbits;
	if (diff < 0)
	{
...
void dnxhd_encode_block(DNXHDEncContext* ctx, DCTELEM* block, int last_index, int n)
{
	int last_non_zero = 0;
	int slevel, i, j;
...
void dnxhd_unquantize_c(DNXHDEncContext* ctx, DCTELEM* block, int n, int qscale, int last_index)
{
...
int dnxhd_ssd_block(DCTELEM* qblock, DCTELEM* block)
{
	int score = 0;
	int i;
	for (i = 0; i < 64; i++)
...
int dnxhd_calc_ac_bits(DNXHDEncContext* ctx, DCTELEM* block, int last_index)
{
	int last_non_zero = 0;
	int bits = 0;
	int i, j, level;
		...
		for (x = 0; x < ctx->m.mb_width; x++)
		{
			...
			if (!RC_VARIANCE)
			{
				...
				ctx->mb_cmp[mb].value = delta_bits
					                        ? ((ctx->mb_rc[ctx->qscale][mb].ssd - ctx->mb_rc[ctx->qscale + 1][mb].ssd) *
						                        100) / delta_bits
