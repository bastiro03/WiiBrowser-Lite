...
static int config(uint32_t s_width, uint32_t s_height, uint32_t width, uint32_t height, uint32_t flags, char* title,
                  uint32_t format)
{
	int arg;
...
static int preinit(const char* arg)
{
	...
	if (ucode)
		...
		for (n = 0; ucodesearchpath[n] != NULL; n++)
		{
