	...
	case AF_CONTROL_REINIT:
		{
			...
			mp_msg(MSGT_AFILTER, MSGL_DBG2, "[scaletempo] "
			       ...
			       s->bytes_stride / nch / bps,
			       s->bytes_standing / nch / bps,
			       s->bytes_overlap / nch / bps,
			       s->frames_search,
			       s->bytes_queue / nch / bps,
			       (use_int ? "s16" : "float"));
	...
	case AF_CONTROL_COMMAND_LINE:
		{
			...
			s->scale = s->speed * s->scale_nominal;
			mp_msg(MSGT_AFILTER, MSGL_DBG2,
			       "[scaletempo] %6.3f scale, %6.2f stride, %6.2f overlap, %6.2f search, speed = %s\n",
			       s->scale_nominal, s->ms_stride, s->percent_overlap, s->ms_search,
			       (s->speed_tempo
				        ? (s->speed_pitch ? "tempo and speed" : "tempo")
				        : (s->speed_pitch ? "pitch" : "none")));
			return AF_OK;
		}
	}
	return AF_UNKNOWN;
