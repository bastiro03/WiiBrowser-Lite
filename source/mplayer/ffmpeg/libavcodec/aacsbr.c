                             ...
                             SpectrumParameters* spectrum)
{
	...
	else
	{
		...
		{
			int16_t vk1[49];
			float invwarp = spectrum->bs_alter_scale
...
void get_bits1_vector(GetBitContext* gb, uint8_t* vec,
                      int elements)
{
	int i;
	...
	for (i = 0; i < ch_data->bs_num_env; i++)
	{
		if (ch_data->bs_df_env[i])
		{
			...
			if (ch_data->bs_freq_res[i + 1] == ch_data->bs_freq_res[i])
			{
				for (j = 0; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++)
					ch_data->env_facs[i + 1][j] = ch_data->env_facs[i][j] + delta * (get_vlc2(gb, t_huff, 9, 3) -
						t_lav);
			}
			else if (ch_data->bs_freq_res[i + 1])
			{
				for (j = 0; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++)
				{
					...
					ch_data->env_facs[i + 1][j] = ch_data->env_facs[i][k] + delta * (get_vlc2(gb, t_huff, 9, 3) -
						t_lav);
				}
			}
			else
		...
		else
		{
			...
			for (j = 1; j < sbr->n[ch_data->bs_freq_res[i + 1]]; j++)
				ch_data->env_facs[i + 1][j] = ch_data->env_facs[i + 1][j - 1] + delta * (get_vlc2(gb, f_huff, 9, 3) -
					f_lav);
		}
	}
                           ...
                           SBRData* ch_data, int ch)
{
	...
	for (i = 0; i < ch_data->bs_num_noise; i++)
	{
		...
		{
			for (j = 0; j < sbr->n_q; j++)
				ch_data->noise_facs[i + 1][j] = ch_data->noise_facs[i][j] + delta * (get_vlc2(gb, t_huff, 9, 2) -
					t_lav);
		}
		else
		{
			...
			for (j = 1; j < sbr->n_q; j++)
				ch_data->noise_facs[i + 1][j] = ch_data->noise_facs[i + 1][j - 1] + delta * (get_vlc2(gb, f_huff, 9, 3)
					- f_lav);
		}
	}
