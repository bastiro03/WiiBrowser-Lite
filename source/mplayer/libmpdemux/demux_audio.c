...
static int demux_audio_open(demuxer_t* demuxer)
{
	...
	case MP3:
				...
				if (!memcmp(hdr, "3DI", 3) && hdr[3] >= 4 && hdr[3] != 0xff)
				{
					...
					if (len > 0)
					{
						...
						else
						{
							...
							stream_read(s, hdr, 4);
							if (memcmp(hdr, "ID3", 3) || hdr[3] < 4 || hdr[3] == 0xff || id3v2_tag_size(hdr[3], s) !=
								len)
							{
		...
		if (duration && demuxer->movi_end && demuxer->movi_end > demuxer->movi_start) sh_audio->wf->nAvgBytesPerSec = (
...
static void demux_audio_seek(demuxer_t* demuxer, float rel_seek_secs, float audio_delay, int flags)
{
	...
	case WAV:
		pos -= (pos - demuxer->movi_start) %
			(sh_audio->wf->nBlockAlign ? sh_audio->wf->nBlockAlign : (sh_audio->channels * sh_audio->samplesize));
		break;
	}
