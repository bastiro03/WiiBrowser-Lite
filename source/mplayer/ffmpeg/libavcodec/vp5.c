	...
	for (pt = 0; pt < 2; pt++)
		for (ctx = 0; ctx < 36; ctx++)
			for (node = 0; node < 5; node++)
				model->coeff_dcct[pt][ctx][node] = av_clip(
					((model->coeff_dccv[pt][node] * vp5_dccv_lc[node][ctx][0] + 128) >> 8) + vp5_dccv_lc[node][ctx][1],
					1, 254);
	...
	for (ct = 0; ct < 3; ct++)
		for (pt = 0; pt < 2; pt++)
			for (cg = 0; cg < 3; cg++)
				for (ctx = 0; ctx < 6; ctx++)
					for (node = 0; node < 5; node++)
						model->coeff_acct[pt][ct][cg][ctx][node] = av_clip(
							((model->coeff_ract[pt][ct][cg][node] * vp5_ract_lc[ct][cg][node][ctx][0] + 128) >> 8) +
							vp5_ract_lc[ct][cg][node][ctx][1], 1, 254);
	return 0;
}
...
int vp5_decode_init(AVCodecContext* avctx)
{
