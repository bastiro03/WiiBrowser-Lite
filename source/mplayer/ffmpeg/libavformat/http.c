...
static const AVOption options[] = {
	{
		"chunked_post", "use chunked transfer-encoding for posts", OFFSET(chunked_post), AV_OPT_TYPE_INT, {.dbl = 1}, 0,
		1, E
	},
	{
	...
	{
		"multiple_requests", "use persistent connections", OFFSET(multiple_requests), AV_OPT_TYPE_INT, {.dbl = 0}, 0, 1,
		D | E
	},
...
static int64_t http_seek(URLContext* h, int64_t off, int whence)
{
	...
	if (whence == AVSEEK_SIZE)
		return s->filesize;
	if ((s->filesize == -1 && whence == SEEK_END) || h->is_streamed)
		return -1;
