	...
	case IRP_MJ_DEVICE_CONTROL:
		...
		case IOCTL_DHAHELPER_MAPPHYSTOLIN:
			...
			if (dwInputBufferLength)
			{
				...
				ntStatus = MapPhysicalMemoryToLinearSpace(dhahelper_priv.base, dhahelper_priv.size,
				                                          &dhahelper_priv.ptr);
				if (NT_SUCCESS(ntStatus))
				{
...
static STDCALL void dhahelperunload(IN PDRIVER_OBJECT DriverObject)
{
	...
	while (alloccount && (ntStatus == STATUS_SUCCESS))ntStatus = UnmapPhysicalMemory(
		alloclist[alloccount - 1].UserVirtualAddress);
	RtlInitUnicodeString(&DeviceLinkUnicodeString, L"\\DosDevices\\DHAHELPER");
...
#ifdef _WIN64
		UserVirtualAddress = (
			PVOID)(((ULONGLONG)PAGE_ALIGN(MmMapLockedPages(Mdl, UserMode))) + MmGetMdlByteOffset(Mdl));
#else
