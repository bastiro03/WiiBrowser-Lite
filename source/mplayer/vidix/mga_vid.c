...
static void mga_vid_write_regs(int restore)
{
	...
	writel(((readl(mga_mmio_base + C2DATACTL) & ~0x000000ff) + (cregs.c2datactl & 0x000000ff)),
	       mga_mmio_base + C2DATACTL);
	// ctrc2
...
static int mga_init(void)
{
	...
	else
	{
		...
		if (is_g400)
		{
			...
			// SDRAM:
			case 0x00:
			case 0x04: mga_ram_size = 16;
				break;
			case 0x03: mga_ram_size = 32;
				break;
			// SGRAM:
			case 0x10:
			case 0x14: mga_ram_size = 32;
				break;
			case 0x11:
			case 0x12: mga_ram_size = 16;
				break;
			default:
				mga_ram_size = 16;
	...
	if (mga_verbose > 1)
		printf("[mga] hardware addresses: mmio: %#x, framebuffer: %#x\n",
