...
#else
using vcd_tocentry = struct ioc_read_toc_single_entry;
#endif

using mp_vcd_priv_t = struct mp_vcd_priv_st
{
	int fd;
	vcd_tocentry entry;
#ifdef VCD_NETBSD
...
#else
			       static_cast<int>(vcd->entry.track),
			       static_cast<int>(vcd->entry.entry.addr_type),
			       static_cast<int>(vcd->entry.entry.control),
#endif
			       static_cast<int>(vcd->entry.address_format),
			       static_cast<int>(TOCADDR(vcd->entry).msf.minute),
			       static_cast<int>(TOCADDR(vcd->entry).msf.second),
			       static_cast<int>(TOCADDR(vcd->entry).msf.frame)
			);
