...
int imc_decode_init(AVCodecContext* avctx)
{
	int i, j, ret;
                          ...
                          int flag)
{
	...
	if (freebits < summer)
	{
		for (i = 0; i < BANDS; i++)
		{
			workT[i] = q->bitsBandT[i]
				           ? (q->bitsBandT[i] * -2 + q->flcoeffs4[i] + 1.585)
				           : 1.e20;
		}
		if (stream_format_code & 0x2)
		{
...
int imc_decode_close(AVCodecContext* avctx)
{
	IMCContext* q = avctx->priv_data;
