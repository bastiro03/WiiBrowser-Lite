...
static void init(struct vf_priv_s* p, mp_image_t* mpi)
{
	...
	chroma_plane_size = mpi->flags & MP_IMGFLAG_PLANAR ? mpi->chroma_height * p->chroma_stride : 0;
	p->memory_allocated =
...
static int put_image(struct vf_instance* vf, mp_image_t* mpi, double pts)
{
	...
	if (flags & MP_IMGFIELD_ORDERED)
	{
		...
		p->chflag = (flags & MP_IMGFIELD_REPEAT_FIRST ? '|' : flags & MP_IMGFIELD_TOP_FIRST ? ':' : '.');
	}
	p->swapped = swapped;
	...
	default:
		if (keep_rate)
			...
			p->iosync >= (breaks == 1 ? -p->in_inc : p->in_inc << (p->num_fields == 1)))
		{
