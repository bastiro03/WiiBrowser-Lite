...
static GtkWidget* AddMenuCheckItem(GtkWidget* window1, const char* immagine_xpm, GtkWidget* Menu, const char* label,
                                   gboolean state, int Number)
{
...
GtkWidget* create_PopUpMenu(void)
{
		...
		if (c > 1)
		{
			...
			for (i = 0; i < MAX_A_STREAMS; i++)
				if (demuxer->a_streams[i])
				{
					...
					int selected_id = (audio_id == aid || (audio_id == -1 && aid == demuxer_default_audio_track(
						mpctx_get_demuxer(guiInfo.mpcontext))));
					char tmp[32];
					snprintf(tmp, 32, MSGTR_MENU_Track, aid);
					AddMenuCheckItem(window1, (const char*)empty1px_xpm, SubMenu, tmp, selected_id,
					                 (aid << 16) + ivSetAudio);
				}
		}
		...
		if (c > 1)
		{
			...
			for (i = 0; i < MAX_V_STREAMS; i++)
				if (demuxer->v_streams[i])
				{
					...
					AddMenuCheckItem(window1, (const char*)empty1px_xpm, SubMenu, tmp, video_id == vid,
					                 (vid << 16) + ivSetVideo);
				}
		}
	}
	...
	if (global_sub_size && guiInfo.StreamType != STREAMTYPE_DVD)
	{
		...
		for (i = 0; i < global_sub_size; i++)
		{
			...
			AddMenuCheckItem(window1, (const char*)empty1px_xpm, SubMenu, tmp, guiInfo.mpcontext->global_sub_pos == i,
			                 (i << 16) + ivSetSubtitle);
		}
	}
	...
	if (guiInfo.VideoWindow)
	{
		...
		if (!guiApp.videoWindow.isFullScreen && guiInfo.Playing)
		{
			if ((guiApp.videoWindow.Width == guiInfo.VideoWidth * 2) &&
				(guiApp.videoWindow.Height == guiInfo.VideoHeight * 2))
				b2 = 1;
		...
		F = AddMenuCheckItem(window1, (const char*)full_xpm, Menu, MSGTR_MENU_FullScreen,
		                     guiApp.videoWindow.isFullScreen, evFullScreen);
		if (!guiInfo.Playing)
		{
