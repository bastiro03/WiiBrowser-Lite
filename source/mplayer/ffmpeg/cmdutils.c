                 ...
                 const OptionDef* options)
{
	...
	{
		int ret = po->flags & OPT_FUNC2
			          ? po->u.func2_arg(optctx, opt, arg)
...
int check_stream_specifier(AVFormatContext* s, AVStream* st, const char* spec)
{
		...
		*spec == 't')
	{
		...
		switch (*spec++)
		{
		case 'v': type = AVMEDIA_TYPE_VIDEO;
			break;
		case 'a': type = AVMEDIA_TYPE_AUDIO;
			break;
		case 's': type = AVMEDIA_TYPE_SUBTITLE;
			break;
		case 'd': type = AVMEDIA_TYPE_DATA;
			break;
		case 't': type = AVMEDIA_TYPE_ATTACHMENT;
			break;
