...
static int init_zoran(zr_info_t* zr, int stretchx, int stretchy)
{
	...
	if (zr->xdoff == -1)
	{
		zr->p.img_x = (zr->vc.maxwidth -
			zr->p.HorDcm * zr->image_width / zr->hdec) / 2;
	}
	else
	{
		zr->p.img_x = zr->xdoff;
	...
	mp_msg(MSGT_VO, MSGL_V, "zr: geometry (after 'scaling'): %dx%d+%d+%d fields=%d, w=%d, h=%d\n", zr->p.img_width,
	       (3 - zr->fields) * zr->p.img_height, zr->p.img_x, zr->p.img_y, zr->fields, zr->image_width / zr->hdec,
	       zr->image_height);
	...
	mp_msg(MSGT_VO, MSGL_V, "zr: got %ld buffers of size %ld (wanted %d buffers of size %d)\n", zr->zrq.count,
	       zr->zrq.size, MJPEG_NBUFFERS, MJPEG_SIZE);
                  ...
                  uint32_t d_height, uint32_t flags, char* title, uint32_t format)
{
	...
	for (i = 0; i < zr_count; i++)
	{
		...
		mp_msg(MSGT_VO, MSGL_V, "zr: input: %dx%d, cropped: %dx%d, output: %dx%d, off_y=%d, off_c=%d\n", width, height,
		       g->width, g->height, zr->image_width, zr->image_height, zr->off_y, zr->off_c);
...
static void flip_page(void)
{
	...
	for (j = 0; j < zr_count; j++)
	{
		...
		if (k > zr->zrq.size) mp_msg(MSGT_VO, MSGL_WARN,
...
vo_zr_parseoption(const m_option_t* conf, const char* opt, const char* param)
{
	...
	if (!strcasecmp(opt, "zrxdoff"))
	{
		i = atoi(param);
		zr->xdoff = i;
		return 1;
	}
	if (!strcasecmp(opt, "zrydoff"))
	{
		i = atoi(param);
		zr->ydoff = i;
		return 1;
	}
	if (!strcasecmp(opt, "zrquality"))
	{
		i = atoi(param);
